...
export class FlowReadingDocument {
  ...
  find(query, options = {}) {
    this.assertCurrent();
    record(options, ["asciiCaseInsensitive", "maxMatches"]);
    const max = options.maxMatches ?? 1000,
      insensitive = options.asciiCaseInsensitive ?? false;
    if (
      !integer(max, 1000) ||
      max < 1 ||
      typeof insensitive !== "boolean" ||
      typeof query !== "string" ||
      query.length > 1024 ||
      !wellFormed(query)
    )
      fail("INVALID_ARGUMENT", "invalid literal reading search");
