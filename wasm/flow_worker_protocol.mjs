...
const fail = (code, message) => {
  throw new FlowWorkerError(code, message);
};
...
export function flowToken(value) {
  if (!value || typeof value !== "object")
    fail("INVALID_IDENTITY", "a source/layout token is required");
...
function editOptions(value) {
  ...
  return {
    expectedRevision: identity(value.expectedRevision),
    reuseAssets: flag(value.reuseAssets ?? false, "reuseAssets"),
  };
}
function pageOptions(value = {}, snapshot = false) {
  fields(value, ["offset", "limit", "token", ...(snapshot ? ["glyphs"] : [])], "page options");
  const result = {
    offset: uint(value.offset ?? 0, "offset"),
    limit: uint(value.limit ?? 512, "limit", 1, 2048),
  };
...
function asset(value) {
  ...
  const result = {
    ...
    width: uint(value.width, "width", 1, 32768),
    height: uint(value.height, "height", 1, 32768),
  };
  if (value.bytes !== undefined) {
    ...
    if (data.byteLength > FLOW_ASSET_LIMIT)
      fail("BUDGET_EXCEEDED", "asset exceeds the 8 MiB payload limit");
    // Creating a view also detects detached buffers, including zero-length ones.
    try {
      result.bytes = new Uint8Array(data.buffer, data.byteOffset, data.byteLength);
    } catch {
      fail("INVALID_ARGUMENT", "asset buffer is detached");
    }
  }
  return result;
}
...
export function normalizeFlowRequest(method, args) {
  ...
  switch (method) {
    ...
    case "getSource":
      return [];
    case "exportDocument":
      return normalizeFlowExport(args[0], args[1], args[2]);
    case "edit":
    case "editBytes":
      range(args[0], args[1]);
    ...
    case "reflow": {
      ...
      for (const key of LAYOUT_KEYS) {
        ...
        if (value <= 0 || value > 1000000)
          fail("INVALID_OPTIONS", "layout values must be positive and at most 1000000");
        partial[key] = value;
      }
    ...
    case "provideAsset":
      return [asset(args[0])];
    case "provideAssets":
      return [normalizeAssetBatch(args[0])];
    case "reloadAssets":
    case "fontBytes":
      return [identity(args[0])];
    case "glyphOutlines":
      return [identity(args[0]), outlineGlyphIds(args[1])];
    case "assetBytes":
      return args.map((value) => identity(value));
    case "snapshot":
      return [pageOptions(args[0], true)];
    case "viewport":
      return [viewportOptions(args[0])];
    case "readingOrder":
    case "pendingAssets":
      return [pageOptions(args[0])];
    case "hitTest":
      return [finite(args[0], "x"), finite(args[1], "y"), flowToken(args[2])];
    case "selectText":
      uint(args[0], "itemIndex");
      range(args[1], args[2]);
      return [args[0], args[1], args[2], flowToken(args[3])];
    case "copySource":
      range(args[0], args[1]);
      return [args[0], args[1], identity(args[2])];
    default:
      fail("UNKNOWN_METHOD", "unsupported flow-worker method");
  }
}
...
export function requestWeight(args) {
  const size = (value) => {
    ...
    if (value && typeof value === "object")
      return 32 + Object.values(value).reduce((sum, item) => sum + size(item), 0);
    return 8;
  };
...
export function acknowledgedState(value) {
  ...
  if (
    !LAYOUT_KEYS.every((key) => Object.hasOwn(value.layout, key)) ||
    typeof value.token?.revision !== "string" ||
    typeof value.token?.layoutRevision !== "string"
  ) {
