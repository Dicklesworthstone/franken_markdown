//! TOML-specific resumable lexer: bounded incremental classification with
//! sound safe-cut release (FCB-022, TOML route / fcb-9vx.18).
//!
//! Same architecture as [`crate::lex_c`]: the batch engine is the classifier,
//! chunks are appended to a held suffix, and only spans ending at a safe-cut
...
/// the span is a complete literal.
fn is_safe_cut_span(pending: &str, span: &Span) -> bool {
    ...
    is_safe_cut_byte(*last)
}

/// A resumable, bounded TOML lexer.
#[derive(Debug)]
pub struct ResumableTomlLexer {
    pending: String,
    base: usize,
    spans: Vec<Span>,
    finished: bool,
...
impl ResumableTomlLexer {
    ...
    /// Feed one chunk of TOML source.
    pub fn feed(&mut self, chunk: &str) -> Result<FeedReportToml, LexTomlError> {
        if self.finished {
            return Err(LexTomlError::AlreadyFinished);
        }
        self.pending.push_str(chunk);
        if self.pending.len() > self.max_pending_bytes {
            return Err(LexTomlError::SuffixTooLong {
    ...
    pub fn finish(&mut self) -> Result<(), LexTomlError> {
        if self.finished {
            return Err(LexTomlError::AlreadyFinished);
        }
        let tail = highlight("toml", &self.pending.clone());
        for span in tail {
    ...
    fn release_through_safe_cut(&mut self) {
        let spans = highlight("toml", &self.pending);
...
/// What one [`ResumableTomlLexer::feed`] produced.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct FeedReportToml {
...
/// The versioned TOML capability row (FCB-022 capability publication).
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct TomlCapabilityV1 {
    /// Capability row format version.
    pub version: u32,
    /// Incremental (chunk-safe) classification is supported for TOML.
    pub incremental: bool,
    /// Literal (single-quote, no escape) and basic (double-quote) strings.
    pub basic_and_literal_strings: bool,
    /// Dotted keys, arrays, inline tables and table headers.
    pub structured_keys: bool,
}

/// The TOML capability row published by this module.
pub const TOML_CAPABILITY_V1: TomlCapabilityV1 = TomlCapabilityV1 {
    version: 1,
    incremental: true,
