...
export function createReadingControls({
  ...
  const view = new FlowReaderView(root, { onLink: activateLink }),
    listeners = [];
  let snapshot = null,
  ...
  const search = () => {
    ...
    if (enabled && snapshot) {
      try {
        result = snapshot.find(query.value, { asciiCaseInsensitive: insensitive.checked });
        summary();
      } catch (failure) {
        error(failure);
      }
  ...
  function activateLink(activation) {
    ...
    try {
      ...
      if (!destination) {
        status.textContent =
          "No matching internal destination. External links are not opened by this reader.";
        return;
      }
      const index = destination.nodeIndex,
        location = getLocation(index, snapshot);
  ...
  listen(sourceButton, "click", () => {
    ...
    try {
      ...
      status.textContent =
        "Selected the enclosing original Markdown block, not an inferred inline range.";
    } catch (failure) {
      error(failure);
    }
  });
  ...
  return Object.freeze({
    update(state) {
      ...
      if (state.status === "idle" || state.status === "disposed") {
        ...
        status.textContent =
          "Preview changing or unavailable; navigation is paused. Previously shown text remains selectable.";
      buttons();
    },
    dispose() {
      if (disposed) return;
