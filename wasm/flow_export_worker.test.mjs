...
function endpoint(data) {
  const worker = new Worker(new URL("./tests/flow_export_worker_fixture.mjs", import.meta.url), { workerData: data });
  const listeners = new Map();
  return {
    postMessage(message, transfer) { worker.postMessage(message, transfer); },
    terminate() { return worker.terminate(); },
    addEventListener(kind, fn) { const listener = value => fn(kind === "message" ? { data: value } : value); listeners.set(fn, listener); worker.on(kind, listener); },
    removeEventListener(kind, fn) { worker.off(kind, listeners.get(fn)); listeners.delete(fn); }
  };
}
async function setup(t, data = {}, runtime = {}) {
  const session = await createWorkerFlowSessionWith(() => endpoint(data), "# Source 😀", { font: "serif" }, { timeoutMs: 5000, ...runtime });
  t.after(() => session.dispose()); return session;
}
...
test("real worker awaits exports, transfers owned document bytes and retains native storage", async t => {
  ...
  const options = { title: "before" }, token = session.token;
  const pending = session.exportDocument("pdf", options, token); options.title = "after";
  const result = await pending;
  assert(result.bytes instanceof Uint8Array); assert.equal(result.revision, token.revision);
  assert.deepEqual(decode(result), { markdown: "# Source 😀", font: "serif", title: "before", epoch: 0, images: [["local.png", [1, 2, 3]]] });
  const second = await session.exportDocument("pdf", { title: "before" }, token);
  assert.deepEqual(second.bytes, result.bytes); assert.equal(session.pendingOperations, 0); assert.equal(session.pendingBytes, 0);
});
...
test("a queued edit cannot race ahead of an asynchronous export", async t => {
  ...
  assert.equal(decode(result).markdown, "# Source 😀"); assert.equal(result.revision, token.revision);
  const next = await editing; assert.notEqual(next.revision, token.revision);
...
test("queued export cancellation leaves active export and the session usable", async t => {
  ...
  controller.abort(); await assert.rejects(queued, code("ABORTED"));
  assert.equal((await first).format, "pdf"); assert.equal(session.disposed, false);
...
test("in-flight export cancellation terminates the worker and never replays queued edits", async t => {
  const session = await setup(t, { delayMs: 1000 }), controller = new AbortController();
  ...
  controller.abort(); await assert.rejects(exporting, code("ABORTED")); await lost;
  assert.equal(session.disposed, true); assert.equal(session.pendingOperations, 0); assert.equal(session.pendingBytes, 0);
});
...
test("export deadlines kill only the owned worker and settle every request", async t => {
  ...
  await assert.rejects(pending, code("TIMEOUT")); assert.equal(session.disposed, true); assert.equal(session.pendingBytes, 0);
});
...
test("renderer rejection is recoverable; invalid options are rejected before worker ingress", async t => {
  ...
  await assert.rejects(session.exportDocument("pdf", { pdfImages: [] }, session.token), code("INVALID_OPTIONS"));
  assert.equal(session.pendingOperations, 0);
  await assert.rejects(session.exportDocument("pdf", { title: "reject-render" }, session.token), code("EXPORT_FAILED"));
  assert.equal(session.disposed, false);
  const result = await session.exportDocument("html", {}, session.token); assert.equal(result.format, "html");
});
...
test("export acknowledgments must match the requested revision, format and lower output limit", async t => {
  for (const corrupt of ["revision", "format", "size"]) {
    ...
    await assert.rejects(session.exportDocument("pdf", options, session.token), code("WORKER_PROTOCOL_ERROR"));
...
test("worker admission limits still cover queued exports and retain no caller options", async t => {
  ...
  const first = session.exportDocument("pdf", {}, session.token);
  await assert.rejects(session.exportDocument("html", {}, session.token), code("WORKER_QUEUE_FULL"));
  await first; assert.equal(session.pendingOperations, 0); assert.equal(session.disposed, false);
});
