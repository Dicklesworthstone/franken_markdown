...
async function equivalent(label) {
  ...
  assert.deepEqual((await remote.snapshot()).totalBounds, direct.snapshot().totalBounds, `${label}: bounds`);
  assert.deepEqual((await remote.readingOrder({ limit: 2048 })).nodes,
    direct.readingOrder({ limit: 2048 }).nodes, `${label}: accessible reading semantics`);
  assert.deepEqual((await remote.pendingAssets()).requests, direct.pendingAssets().requests, `${label}: pending assets`);
...
try {
  ...
  assert(text?.fontRun);
  assert.deepEqual(await remote.fontBytes(text.fontRun.fontId), direct.fontBytes(text.fontRun.fontId));
  assert.deepEqual(await remote.selectText(text.index, 0, 1, remote.token), direct.selectText(text.index, 0, 1, direct.token));
  const shown = remote.token;
  const requests = (await remote.pendingAssets()).requests;
  for (const request of requests.slice().reverse()) {
    const result = { requestId: request.id, generation: request.generation, width: 640, height: 240, bytes: new Uint8Array([1, 2, 3]) };
    await remote.provideAsset(result); direct.provideAsset(result);
  ...
  await remote.reflow(resized, remote.token); direct.reflow(resized, direct.token);
  ...
  await assert.rejects(remote.provideAsset({ requestId: requests[0].id, generation: oldRevision, width: 1, height: 1 }),
  ...
  console.log("flow-worker: generated WASM, worker/synchronous equivalence, edits, fonts, selection and assets: PASS");
} finally {
  direct.dispose(); remote.dispose();
}
