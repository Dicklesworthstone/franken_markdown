...
import { readFlowDocument, FlowReaderView } from "@franken-suite/franken-markdown/flow-reader";
...
test("public package export loads without DOM/WASM side effects and searches engine data", async () => {
  ...
  assert.equal(document.find("text").matches.length, 1); assert.equal(typeof FlowReaderView, "function");
  assert.deepEqual(manifest.exports["./flow-reader"], { types: "./flow-reader.d.ts", import: "./flow-reader.js" });
});
test("manifest ships all reading runtime and live-control dependencies", async () => {
  for (const file of [...runtime, "demo/flow_reading_controls.mjs"]) {
    assert(manifest.files.includes(file), `unshipped ${file}`); assert((await readFile(new URL(file, import.meta.url))).length > 0);
  }
  for (const path of ["flow-reader.js", "demo/flow_reading_controls.mjs", "demo/flow_preview_controller.mjs"]) {
    ...
    for (const [, dependency] of code.matchAll(/from\s+"(\.{1,2}\/[^"]+)"/g)) {
      ...
      assert(manifest.files.includes(target.slice(root.length)), `${path} has an unshipped dependency ${dependency}`);
    }
  }
});
test("both assemblers ship the new public reader and the actual demo controls", async () => {
  for (const script of ["dsr-wasm-package.sh", "check-wasm-package.sh"]) {
    ...
    assert(source.split("\n").some(line => line.startsWith("cp ") && line.includes("wasm/demo/flow_reading_controls.mjs")), `${script} misses UI`);
  }
});
