...
export function createBookControls({ root, worker, confirm = () => true, urls = URL,
  ...
  function revoke() {
    ...
    el.download.removeAttribute("href"); el.download.removeAttribute("download");
    if (url !== null) { const previous = url; url = null; urls.revokeObjectURL(previous); }
  }
  function invalidate() {
    generation++; preparing = false; worker.cancel(); revoke();
    if (!disposed) buttons();
  }
  ...
  function showOptions() {
    ...
    el["dark-mode"].value = value.darkMode; el["font-scale"].value = String(value.fontScale); el.toc.checked = value.toc; el["page-numbers"].checked = value.pageNumbers;
  }
  ...
  function publish(output, ticket, revision, view) {
    if (disposed || ticket !== generation || revision !== collection.revision || view !== signature()) {
      throw bookError("STALE_SOURCE", "The collection changed; this output was not published.");
    }
    revoke(); url = urls.createObjectURL(output.blob);
    published = { revision, view }; el.download.href = url; el.download.download = output.filename;
    el.download.textContent = `Download ${output.filename}`; el.download.hidden = false;
  ...
  async function prepare(format) {
    let ticket = null;
    try {
      alive(); if (reading) throw bookError("BOOK_BUSY", "A local import is in progress.");
      invalidate(); capture();
      const input = collection.snapshot(), revision = collection.revision, view = signature();
      ticket = ++generation;
      preparing = true; buttons(); el.status.textContent = `Preparing ${format.toUpperCase()} in the worker. Editing or cancelling terminates this export.`;
      const result = await worker.render(input.files, format, input.options);
      const name = (input.options.title.replace(/[^a-z0-9_-]+/gi, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "book") + "." + result.extension;
      publish({ filename: name, blob: result.blob() }, ticket, revision, view);
    } catch (error) { if (ticket === null || ticket === generation) report(error); throw error; }
    finally { if (!disposed && ticket === generation) { preparing = false; buttons(); } }
  }

  function prepareSource(project) {
    alive(); invalidate(); capture(project);
  ...
  async function importFiles(files, mode = "files") {
    alive(); if (reading) throw bookError("BOOK_BUSY", "A local import is already in progress.");
    capture(); invalidate();
    const revision = collection.revision, view = signature(); reading = true; buttons();
    const fence = () => {
      alive();
      if (revision !== collection.revision || view !== signature()) throw bookError("STALE_SOURCE", "The collection changed during import; nothing was installed.");
    };
    try {
      if (mode === "project") {
        const project = await readBookProject(files[0]); fence();
  ...
  on("chapters", "change", () => invoke(() => { const next = Number(el.chapters.value); capture(); active = next; showChapter(); }));
  on("add-chapter", "click", () => invoke(() => {
  ...
  on("save-project", "click", () => invoke(() => prepareSource(true))); on("save-chapter", "click", () => invoke(() => prepareSource(false)));
  for (const format of ["pdf", "epub", "site"]) on(`export-${format}`, "click", () => { void prepare(format).catch(() => {}); });
  on("cancel-export", "click", () => { invalidate(); el.status.textContent = "Export cancelled. All source remains editable."; });
  on("download", "click", event => {
    if (disposed || !published || published.revision !== collection.revision || published.view !== signature()) {
      event.preventDefault(); if (!disposed) { invalidate(); el.status.textContent = "Output is stale. Prepare it again from the current collection."; }
    }
  });
