...
export class FlowAssetError extends Error {
  constructor(code, message, options) {
    super(message, options); this.name = "FlowAssetError"; this.code = code;
  }
}
export const assetFailure = (code, message) => new FlowAssetError(code, message);
const invalid = () => { throw assetFailure("INVALID_IMAGE", "invalid or unsupported raster image structure"); };
...
export function rasterInfo(bytes, limits) {
  if (!ArrayBuffer.isView(bytes) || Object.prototype.toString.call(bytes) !== "[object Uint8Array]"
      || Object.prototype.toString.call(bytes.buffer) !== "[object ArrayBuffer]") invalid();
  ...
  let view;
  try { view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength); } catch { invalid(); }
  ...
  else throw assetFailure("UNSUPPORTED_IMAGE", "only static PNG and 8-bit baseline/progressive JPEG are supported");
  const pixels = info.width * info.height;
  if (!info.width || !info.height || info.width > limits.maxDimension || info.height > limits.maxDimension
      || !Number.isSafeInteger(pixels) || pixels > limits.maxImagePixels) {
...
function png(bytes, view) {
  let offset = 8, info = null, data = false, dataEnded = false, palette = false;
  while (offset + 12 <= bytes.length) {
    ...
    if (["acTL", "fcTL", "fdAT"].includes(kind)) {
      throw assetFailure("UNSUPPORTED_IMAGE", "animated PNG is not admitted by the static-image loader");
    }
    if (kind === "IHDR") {
      if (info || offset !== 8 || length !== 13) invalid();
      const color = bytes[body + 9], depth = bytes[body + 8];
      const depths = { 0: [1, 2, 4, 8, 16], 2: [8, 16], 3: [1, 2, 4, 8], 4: [8, 16], 6: [8, 16] };
      if (!depths[color]?.includes(depth) || bytes[body + 10] !== 0 || bytes[body + 11] !== 0 || bytes[body + 12] > 1) invalid();
      info = { mime: "image/png", width: view.getUint32(body), height: view.getUint32(body + 4), color };
    } else if (kind === "PLTE") {
      if (palette || data || !length || length > 768 || length % 3 || [0, 4].includes(info.color)) invalid();
      palette = true;
    } else if (kind === "IDAT") {
      ...
      if (["iCCP", "zTXt", "iTXt"].includes(kind)) {
        throw assetFailure("UNSUPPORTED_IMAGE", "compressed PNG metadata is not admitted by this bounded decoder profile");
      }
    }
    offset = end;
  }
...
function jpeg(bytes, view) {
  let offset = 2, info = null, scan = false, sawScan = false;
  while (offset < bytes.length) {
    ...
    if (marker === 0xc0 || marker === 0xc2) {
      ...
      info = { mime: "image/jpeg", height: view.getUint16(offset + 3), width: view.getUint16(offset + 5) };
    } else if (marker === 0xda) {
      ...
      scan = true; sawScan = true;
        ...
        || (marker >= 0xe0 && marker <= 0xef))) invalid();
    offset = end;
  }
  invalid();
}
...
export async function decodeRaster(blob) {
  if (typeof globalThis.createImageBitmap !== "function") {
    throw assetFailure("IMAGE_DECODER_UNAVAILABLE", "createImageBitmap is unavailable; supply a host decoder");
  }
