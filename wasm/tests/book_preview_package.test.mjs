...
const modules = [
  "book_site_preview.mjs",
  "book_preview_frame.mjs",
  "demo/book_preview_controls.mjs",
];
...
test("both assemblers include the preview worker dependencies, controller and guide", () => {
  for (const path of ["scripts/check-wasm-package.sh", "scripts/dsr-wasm-package.sh"]) {
    ...
    for (const name of ["book_site_preview.mjs", "book_preview_frame.mjs", "PREVIEW.md"])
...
test("actual worker entry delegates preview to the same book engine, not an alternate renderer", () => {
  ...
  assert(entry.includes('import * as book from "./book.js"'));
  assert(entry.includes('from "./book_site_preview.mjs"'));
...
test("actual publisher starts preview independently and tears it down before its source owner", () => {
  ...
  assert(entry.includes('from "./book_preview_controls.mjs"'));
  assert(entry.includes("worker: createBookWorker({ maxOutputBytes:"));
  assert(entry.includes("preview?.dispose(); library?.dispose(); controls.dispose();"));
  assert(entry.includes("preview?.suspend();"));
  assert(entry.includes("preview?.resume();"));
  const iframe = html.match(/<iframe\b[^>]*id="preview-frame"[^>]*>/)?.[0];
  assert(iframe);
  assert(iframe.includes('sandbox="allow-scripts"'));
  assert(!iframe.includes("allow-same-origin"));
  for (const id of [
    "preview-build",
    "preview-auto",
    "preview-cancel",
