...
export function outlineGlyphIds(value) {
  const typed =
    ArrayBuffer.isView(value) && Object.prototype.toString.call(value) === "[object Uint16Array]";
  if (!Array.isArray(value) && !typed)
    fail("INVALID_ARGUMENT", "glyphIds must be an array or Uint16Array");
  if (value.length > MAX_OUTLINE_GLYPHS)
    fail("BUDGET_EXCEEDED", "at most 256 glyph outlines per request");
  if (typed) {
    ...
    try {
      new Uint16Array(value.buffer, value.byteOffset, value.length);
    } catch {
      fail("INVALID_ARGUMENT", "glyphIds buffer is detached");
    }
  }
  return Array.from(value, (id) => {
    if (!Number.isInteger(id) || id < 0 || id > 65535)
      fail("INVALID_ARGUMENT", "glyph IDs must be integers in 0..=65535");
    return id;
  });
}
...
export function validateOutlineBatch(batch, fontId, requested) {
  ...
  if (
    ...
    batch.fontId !== fontId ||
    !Number.isInteger(batch.unitsPerEm) ||
    batch.unitsPerEm < 1 ||
    batch.unitsPerEm > 65535 ||
    ![batch.ascent, batch.descent, batch.lineGap].every(
      (n) => Number.isInteger(n) && n >= -32768 && n <= 32767,
    ) ||
    batch.ascent <= batch.descent ||
    !Array.isArray(batch.glyphs) ||
    batch.glyphs.length !== requested.length
  )
    bad();
  let commands = 0;
  for (let i = 0; i < requested.length; i++) {
    ...
    for (const command of glyph.commands) {
      ...
      if (
        ...
        !command
          .slice(1)
          .every((n) => typeof n === "number" && Number.isFinite(n) && Math.abs(n) <= 100000000)
      )
        bad();
      if (op === "M") {
        if (open) bad();
...
export function withGlyphOutlines(session, read) {
  ...
  Object.defineProperty(api, "glyphOutlines", {
    enumerable: true,
    value(fontId, glyphIds) {
      ...
      try {
        ...
        try {
          ...
          fail("INVALID_WASM_RESPONSE", "glyph-outline response is not JSON");
        }
