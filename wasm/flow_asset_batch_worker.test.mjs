...
function endpoint(workerData = {}) {
  const worker = new Worker(new URL("./tests/flow_asset_batch_worker.mjs", import.meta.url), {
    workerData,
  });
  ...
  return {
    ...
    removeEventListener(type, listener) {
      worker.off(type, listeners.get(listener));
...
async function session(t, data, runtime) {
  ...
  const remote = await createWorkerFlowSessionWith(
    () => worker,
    "original source",
    {},
...
test("real worker negotiates batching and acknowledges one mutation without detaching inputs", async (t) => {
  ...
  assert(remote.supportsAssetBatches);
  assert.equal(remote.supportsViewport, false);
  const input = new Uint8Array([9, 1, 2, 8]);
  const pending = remote.provideAssets([
    result("2"),
    result("1", input.subarray(1, 3)),
    result("3", new Uint8Array()),
  ]);
...
test("lower worker queue budgets remain effective and rejection leaves the session usable", async (t) => {
  ...
  await assert.rejects(
    remote.provideAssets([result("1", new Uint8Array(2048))]),
    code("WORKER_QUEUE_FULL"),
  );
  assert.equal(remote.pendingBytes, 0);
  assert.equal(remote.layoutRevision, "1");
  assert.equal((await remote.pendingAssets()).total, 3);
  await remote.provideAssets([result()]);
  assert.equal(remote.layoutRevision, "2");
});
...
test("unknown tail IDs reject one remote transaction and can be retried", async (t) => {
  const { remote } = await session(t);
  await assert.rejects(
    remote.provideAssets([result(), result("9")]),
  ...
  assert.equal(remote.layoutRevision, "2");
  assert.equal((await remote.pendingAssets()).total, 1);
});
...
test("legacy native capabilities reject batch calls locally without losing the worker", async (t) => {
  ...
  assert.equal(remote.pendingOperations, 0);
...
test("inconsistent batch acknowledgment terminates the session instead of publishing a false token", async (t) => {
  ...
  assert.equal(remote.disposed, true);
