...
const files = ["demo/book_library_store.mjs", "demo/book_library_session.mjs", "demo/book_library_controls.mjs"];
test("library runtime modules and their direct imports are declared for publication", () => {
  ...
  assert(manifest.files.includes("LIBRARY.md")); assert(read("wasm/LIBRARY.md").includes("Autosave"));
});
test("both package assemblers copy each library dependency and its usage guide", () => {
  for (const script of ["scripts/check-wasm-package.sh", "scripts/dsr-wasm-package.sh"]) {
    ...
    assert(demo, script); assert(root, script);
    for (const file of files) assert(demo[1].split(/\s+/).includes(posix.basename(file)), `${script}: ${file}`);
...
test("publisher entry starts library independently and tears it down before source disposal", () => {
  ...
  for (const id of ["library-save", "library-copy", "library-open", "library-new", "library-delete", "library-autosave", "library-books", "library-versions"]) {
