...
| `svg` | `font`, `maxWidthPt` (144..14400 points), and explicit resources |

`images` and `fonts` work for all targets. They contain explicit bytes; destinations
never authorize ambient file reads or network requests. The current SVG poster's
fixed type ladder does not consume `fontScale`, so that field is refused rather
than presented as an implemented feature. SVG warnings retain the existing
per-occurrence response channel.
