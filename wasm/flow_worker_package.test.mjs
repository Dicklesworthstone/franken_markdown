...
test("package subpath resolves without loading generated WASM on the calling thread", async () => {
  ...
  await assert.rejects(
    entry.createWorkerFlowSession("source"),
...
test("worker runtime files and type entry are explicitly shipped", async () => {
  const required = [
    ...
    "flow-worker.d.ts",
    "flow_worker.js",
    "flow_worker_session.mjs",
    "flow_worker_protocol.mjs",
    "worker_transport.mjs",
    "WORKER.md",
  ];
  assert.deepEqual(manifest.exports["./flow-worker"], {
    types: "./flow-worker.d.ts",
    import: "./flow-worker.js",
  });
  assert.equal(manifest.exports["./flow-worker/worker"], "./flow_worker.js");
  assert(
    manifest.sideEffects.includes("./flow_worker.js"),
    "bundlers must retain the worker's message handler",
  );
...
test("both assembly routes include all worker runtime files and the real-WASM proof", async () => {
  for (const name of ["check-wasm-package.sh", "dsr-wasm-package.sh"]) {
    ...
    assert(
      script.includes("node wasm/flow_worker_smoke.mjs"),
      `${name}: generated-WASM worker smoke`,
    );
  }
});
  ...
  await assert.rejects(
    createWorkerFlowSession(
      ...
      {},
      {
        workerFactory() {
          called++;
          throw new Error("policy blocked worker");
        },
      },
    ),
