  ...
  const session = await createWorkerFlowSessionWith(
    () => {
      const worker = new Worker(
        new URL("./tests/flow_viewport_worker.fixture.mjs", import.meta.url),
        { workerData: mode },
      );
    ...
    "source",
    {},
    { timeoutMs: 3000, startupTimeoutMs: 3000 },
  );
...
test("viewport request shape is normalized on both protocol boundaries", () => {
  const input = {
    ...
    afterIndex: 99,
    glyphs: true,
    token: { revision: 1n, layoutRevision: 2n },
  };
  ...
  assert.throws(() =>
    acknowledgedState({
      ...
      layout: { viewportWidth: 800, bodySize: 14, codeSize: 13, lineHeight: 20 },
...
test("public glyph/export wrappers preserve live viewport capability getters", () => {
  ...
  const wrapped = withFlowExports(
    withGlyphOutlines(session, () => "{}"),
    { html() {}, pdf() {} },
  );
