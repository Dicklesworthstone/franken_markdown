//! HTML5 named character reference table (WHATWG named character references,
//! the semicolon-terminated forms required by CommonMark). Generated offline
//! from Python's `html.entities.html5`; do not edit by hand. Sorted by name so
//! lookups can binary-search. Values may be one or two code points.

/// `(name, replacement)` pairs (name is the text between `&` and `;`), sorted by
