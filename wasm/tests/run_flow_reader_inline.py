#!/usr/bin/env python3
"""Real Chromium styled DOM/selection tests; native sessions are explicit doubles."""
import argparse
...
sources = {name: (root / name).read_text(encoding="utf-8") for name in [
    "flow_reading.mjs", "flow-reader.js", "tests/flow_reading_fixtures.mjs",
    "tests/flow_reader_inline.browser.mjs", "tests/flow_reader.browser.mjs",
]}
    ...
    try:
        ...
        results = page.evaluate("""async sources => {
          ...
          try {
            ...
            module('tests/flow_reader_inline.browser.mjs', {'../flow-reader.js': 'flow-reader.js',
            ...
            return [...await (await import(urls.get('tests/flow_reader.browser.mjs'))).run(),
              ...await (await import(urls.get('tests/flow_reader_inline.browser.mjs'))).runTests()];
