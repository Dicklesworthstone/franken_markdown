import { readFlowDocument, FlowReaderView, sourceSpanToUtf16, type ReadingFlowSession }
  from "@franken-suite/franken-markdown/flow-reader";
import type { FlowSession, FlowPageOptions, FlowReadingPage } from "./flow.js";
declare const sync: FlowSession;
declare const asyncSession: { readonly disposed: boolean; readonly token: FlowSession["token"];
...
for (const session of sessions) {
  const document = await readFlowDocument(session, { token: { revision: 1n, layoutRevision: "1" }, limits: { maxNodes: 200 } });
  const view = new FlowReaderView(element); view.render(document);
  ...
  for (const match of result.matches) {
    ...
    const range: Readonly<{ start: number; end: number }> = sourceSpanToUtf16(sync.source, location.enclosingSourceSpan);
    void copied; void range;
  }
