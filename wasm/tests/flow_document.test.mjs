...
function controls(normalizeNewlines = false) {
  const sourceEditor = new Element(),
    ...
    status = new Element();
  sourceEditor.value = "# Keep me";
  filename.value = "old.md";
  if (normalizeNewlines) {
    let value = sourceEditor.value;
    Object.defineProperty(sourceEditor, "value", {
      get: () => value,
      set: (next) => {
        value = next.replace(/\r\n?/g, "\n");
      },
  ...
  const api = createSourceControls({
    sourceEditor,
    filename,
    open,
    prepare,
    download,
    status,
    onReplace: (value) => replacements.push(value),
    confirmReplace: (value) => confirmation(value),
    urls: {
...
test("malformed and oversized files reject without lossy decoding or starting an oversized read", async () => {
  await assert.rejects(
    readMarkdownFile(new File([new Uint8Array([0xc3, 0x28])], "bad.md")),
  ...
  await assert.rejects(
    readMarkdownFile(new LargeFile([new Uint8Array(4 * 1024 * 1024 + 1)], "large.md")),
...
test("filenames are bounded basenames and source downloads never use an HTML extension", () => {
  for (const name of [
    ...
    assert.throws(() => documentName(name));
  assert.equal(
    markdownDownload({ filename: "page.html", source: "<script>hi</script>" }).filename,
    "page.html.md",
  );
  assert.equal(markdownDownload({ filename: "REPORT.MD", source: "" }).filename, "REPORT.MD");
  assert.throws(
    () => documentSnapshot({ filename: "a.md", source: "\ud800" }),
    code("INVALID_UNICODE"),
  );
});
test("snapshots strip non-source state rather than persisting assets or renderer authority", () => {
  assert.deepEqual(
    documentSnapshot({ filename: "a.md", source: "x", imageGrants: ["secret"], worker: {} }),
    { filename: "a.md", source: "x" },
  );
});
test("opening a file replaces source only after confirmation and invokes grant revocation", async (t) => {
  const f = controls();
  t.after(() => f.api.dispose());
  let inputs = 0;
  f.sourceEditor.addEventListener("input", () => {
  ...
  assert.equal(f.filename.value, "new.md");
...
test("declined open and decode failure leave the original source, name and image authority intact", async (t) => {
  ...
  assert.equal(f.sourceEditor.value, "# Keep me");
  assert.equal(f.filename.value, "old.md");
...
test("source downloads need no renderer and are invalidated on edit, rename or replacement", async (t) => {
  ...
  assert.equal(await f.urls[0].text(), "# Keep me");
  assert.equal(f.download.hidden, false);
  f.sourceEditor.dispatchEvent(new Event("input"));
  assert.equal(f.download.hidden, true);
  assert.deepEqual(f.revoked, ["blob:1"]);
  f.api.prepareDownload();
  f.filename.value = "changed.md";
  const click = new Event("click", { cancelable: true });
  f.download.dispatchEvent(click);
  assert.equal(click.defaultPrevented, true);
  assert.equal(f.download.hidden, true);
  f.api.prepareDownload();
  f.api.replace({ filename: "new.md", source: "new" });
  assert.equal(f.download.hidden, true);
});
test("disposal revokes the sole URL and removes element listeners", async () => {
  ...
  assert.deepEqual(f.revoked, ["blob:1"]);
  assert.equal(f.download.hidden, true);
  f.prepare.dispatchEvent(new Event("click"));
...
test("untouched imports preserve original mixed line endings despite textarea API normalization", async (t) => {
  ...
  f.api.prepareDownload();
  assert.deepEqual(
    new Uint8Array(await f.urls[0].arrayBuffer()),
...
test("source byte preservation can survive controller recreation without replacing the editor", () => {
  ...
  assert.throws(
    () =>
      createSourceControls({ ...options, initialDocument: { source: "stale", filename: "a.md" } }),
    code("STALE_SOURCE"),
  );
});
