import {
  ...
  type FlowSnapshot,
  type FlowToken,
  type FlowWorkerEndpoint,
} from "@franken-suite/franken-markdown/flow-worker";

const worker: FlowWorkerEndpoint = new Worker(new URL("./flow_worker.js", import.meta.url), {
  type: "module",
});
worker.terminate();
const session = await createWorkerFlowSession(
  "# Guide",
  { viewportWidth: 360 },
  { maxPendingOperations: 8 },
);
const control = { signal: new AbortController().signal, timeoutMs: 10000 };
const token: FlowToken = await session.edit(
  0,
  0,
  "Before\n\n",
...
for await (const next of session.pages({ limit: 16 }, control)) {
  next.items.forEach((item) => item.bounds);
}
