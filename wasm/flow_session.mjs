...
const DEFAULT_LAYOUT = Object.freeze({ viewportWidth: 800, bodySize: 14, codeSize: 13, lineHeight: 20 });
...
export function normalizeFlowError(error) {
  ...
  if (typeof error === "string" && error.length <= 4096) {
    ...
    } catch { /* A generated binding or a trap may throw a non-JSON diagnostic. */ }
  }
  return new FlowError("WASM_ERROR", error instanceof Error ? error.message : "WASM flow operation failed", { cause: error });
}
...
export function identity(value, name = "identity") {
  ...
  if (typeof value !== "string" || !/^(0|[1-9][0-9]{0,19})$/.test(value) || BigInt(value) > U64_MAX) {
    fail("INVALID_IDENTITY", `${name} must be a canonical u64 decimal string or bigint, never Number`);
  }
  return value;
}
...
export function sourceText(value, name = "source") {
  ...
  if (value.length > FLOW_SOURCE_LIMIT) fail("BUDGET_EXCEEDED", `${name} exceeds the 4 MiB source limit`);
  let bytes = 0;
  for (let i = 0; i < value.length; i++) {
    const c = value.charCodeAt(i);
    if (c >= 0xd800 && c <= 0xdbff) {
      const next = value.charCodeAt(i + 1);
      if (!(next >= 0xdc00 && next <= 0xdfff)) fail("INVALID_UNICODE", `${name} contains an unpaired high surrogate`);
      i++;
      bytes += 4;
    } else if (c >= 0xdc00 && c <= 0xdfff) {
      fail("INVALID_UNICODE", `${name} contains an unpaired low surrogate`);
    } else bytes += c < 0x80 ? 1 : c < 0x800 ? 2 : 3;
    if (bytes > FLOW_SOURCE_LIMIT) fail("BUDGET_EXCEEDED", `${name} exceeds the 4 MiB source limit`);
  }
  return value;
}
export function layoutOptions(value = {}, defaults = DEFAULT_LAYOUT) {
  ...
  for (const key of Object.keys(DEFAULT_LAYOUT)) {
    ...
    if (next[key] <= 0 || next[key] > 1000000) fail("INVALID_OPTIONS", `${key} must be positive and at most 1000000`);
  }
  if (next.lineHeight < Math.max(next.bodySize, next.codeSize)) fail("INVALID_OPTIONS", "lineHeight must accommodate bodySize and codeSize");
...
function textRange(start, end) {
  integer(start, "start"); integer(end, "end");
  if (start > end) fail("INVALID_SELECTION", "selection start exceeds end");
}
function token(value) {
  if (!value || typeof value !== "object") fail("INVALID_IDENTITY", "a source/layout revision token is required");
  return Object.freeze({ revision: identity(value.revision, "revision"), layoutRevision: identity(value.layoutRevision, "layoutRevision") });
}
...
function response(value, expected) {
  ...
  identity(parsed.revision); identity(parsed.layoutRevision);
...
function checkedPage(value, key, offset, limit) {
  if (!Number.isInteger(value.total) || value.total < offset || value.total > 0xffffffff || value.offset !== offset
      || !Array.isArray(value[key]) || value[key].length !== Math.min(limit, value.total - offset)) {
...
export function createFlowAdapter(raw, initialLayout = DEFAULT_LAYOUT) {
  ...
  const alive = () => { if (backend === null) fail("SESSION_DISPOSED", "flow session has been disposed"); };
  const call = f => { alive(); try { return f(backend); } catch (error) { throw normalizeFlowError(error); } };
  const current = () => call(b => {
    if (typeof b.revision !== "string" || typeof b.layoutRevision !== "string") fail("INVALID_WASM_RESPONSE", "WASM returned non-string revision IDs");
    return token({ revision: b.revision, layoutRevision: b.layoutRevision });
  });
  const fence = value => {
    const expected = token(value);
    const actual = current();
    if (expected.revision !== actual.revision) fail("STALE_REVISION", "document revision changed since this operation was prepared");
    if (expected.layoutRevision !== actual.layoutRevision) fail("STALE_LAYOUT", "layout revision changed since this operation was prepared");
    return expected;
  };
  const editOptions = value => {
    record(value, ["expectedRevision", "reuseAssets"], "edit options");
    return { revision: identity(value.expectedRevision, "expectedRevision"), reuse: boolean(value.reuseAssets ?? false, "reuseAssets") };
  };
  const page = (kind, value = {}) => {
    alive();
    record(value, ["offset", "limit", "token", ...(kind === "snapshot" ? ["glyphs"] : [])], "page options");
    ...
    const data = call(b => kind === "snapshot"
      ? b.snapshotJson(expected.revision, expected.layoutRevision, offset, limit, boolean(value.glyphs ?? false, "glyphs"))
      : kind === "reading" ? b.readingJson(expected.revision, expected.layoutRevision, offset, limit)
        : b.pendingAssetsJson(expected.revision, offset, limit));
    return checkedPage(response(data, expected), kind === "snapshot" ? "items" : kind === "reading" ? "nodes" : "requests", offset, limit);
  };
  const edit = (method, start, end, replacement, options) => {
    alive(); textRange(start, end);
  ...
  const api = {
    ...
    get source() { return call(b => b.source); },
    get layoutOptions() { alive(); return { ...layout }; },
    edit(startUtf16, endUtf16, replacement, options) { return edit("editUtf16", startUtf16, endUtf16, replacement, options); },
    editBytes(startByte, endByte, replacement, options) { return edit("editBytes", startByte, endByte, replacement, options); },
    replaceSource(source, options) {
      alive(); const opts = editOptions(options); sourceText(source);
    ...
    reflow(options, expectedToken) {
      ...
      call(b => b.reflow(expected.revision, expected.layoutRevision, next.viewportWidth, next.bodySize, next.codeSize, next.lineHeight));
      // Do not publish new JS options until the Rust transaction succeeds.
      layout = next;
      return current();
    },
    provideAsset(result) {
      alive(); record(result, ["requestId", "generation", "width", "height", "bytes"], "asset result");
    ...
    pages(options = {}) {
      alive(); record(options, ["limit", "glyphs", "token"], "iterator options");
    ...
    hitTest(x, y, expectedToken) {
      const expected = fence(expectedToken);
      const px = finite(x, "x"), py = finite(y, "y");
      return response(call(b => b.hitTestJson(expected.revision, expected.layoutRevision, px, py)), expected);
    },
    selectText(itemIndex, startUtf16, endUtf16, expectedToken) {
      const expected = fence(expectedToken); integer(itemIndex, "itemIndex"); textRange(startUtf16, endUtf16);
      return response(call(b => b.selectItemJson(expected.revision, expected.layoutRevision, itemIndex, startUtf16, endUtf16)), expected);
    },
    copySource(startByte, endByte, expectedRevision) {
      alive(); textRange(startByte, endByte);
      return call(b => b.copySource(identity(expectedRevision, "expectedRevision"), startByte, endByte));
    },
    fontBytes(fontId) { return call(b => bytes(b.fontBytes(identity(fontId, "fontId")), 32 * 1024 * 1024).slice()); },
    assetBytes(requestId, expectedRevision) {
      const data = call(b => b.assetBytes(identity(requestId, "requestId"), identity(expectedRevision, "expectedRevision")));
    ...
    dispose() {
      ...
      try { owned.free(); } catch (error) { throw normalizeFlowError(error); }
    }
  };
