    ...
    F: FnMut(&str, f32, FlowTextRole, FlowInlineStyle) -> Result<OwnedTextRun, String>,
{
    ...
    pub(super) fn inline_text(
        ...
        // Preserve the legacy layout byte-for-byte when no rich metadata is
        // requested or when the entire text is genuinely unstyled.
        if runs.is_empty() || runs.iter().all(|r| r.style == FlowInlineStyle::default() && r.link.is_none()) {
        ...
        for physical in text.split_terminator('\n') {
            ...
            // Retained cluster work for a physical line is bounded even when
            // many tiny style spans would individually fit the shaper limit.
            if physical.len() > self.options.max_shape_bytes {
                return Err(FlowLayoutError::BudgetExceeded("bytes per styled physical line"));
            }
            let physical_end = offset + physical.len();
            let first_style = runs.partition_point(|run| run.range.end <= offset);
