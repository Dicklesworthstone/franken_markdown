  ...
  const session = {
    ...
    snapshot({ offset, limit }) {
      const items = Array.from({ length: count }, (_, i) => ({
        ...
        destination: "local.png",
        isResolved: resolved.has(String(i + 1)),
      }));
...
test("retention is explicit: default still sends dimensions only and export rejects missing bytes", async (t) => {
  ...
  assert.equal(Object.hasOwn(f.calls[0], "bytes"), false);
  const api = withFlowExports(f.session, {
    html() {
...
test("retains the immutable admitted bytes, not later loader mutations, and exports them", async (t) => {
  ...
  t.after(() => f.manager.dispose());
  assert.equal((await f.manager.loadPending()).loaded, 1);
  assert.deepEqual(f.calls[0].bytes, expected);
  assert.notEqual(f.calls[0].bytes.buffer, original.buffer);
  const render = async (_, settings) => {
    assert.deepEqual(settings.pdfImages[0].bytes, expected);
    return {
      format: "pdf",
  ...
  const result = await withFlowExports(f.session, { html: render, pdf: render }).exportDocument(
    "pdf",
    {},
    f.session.token,
  );
  assert.equal(result.assetCount, 1);
  assert.equal(result.assetBytes, expected.length);
...
test("source changes close old bitmaps and require fresh payload authorization", async (t) => {
  ...
  t.after(() => f.manager.dispose());
  await f.manager.loadPending();
  f.session.edit();
  f.manager.synchronize();
  assert.equal(f.images[0].closed, true);
  assert.equal(f.payloads.size, 0);
  await f.manager.loadPending();
  assert.equal(loads, 2);
  assert.equal(f.calls[1].generation, "2");
});
test("invalid retention options do not get coerced", () => {
  for (const value of [null, 1, "true", {}, []])
    assert.throws(() => fixture({ retainSourceBytes: value }), code("INVALID_OPTIONS"));
});
test("revocation during the extra Blob copy prevents native payload delivery", async (t) => {
  const hold = gate(),
    entered = gate(),
    original = Blob.prototype.arrayBuffer;
  const f = fixture({ retainSourceBytes: true });
  t.after(() => f.manager.dispose());
  ...
  const pending = f.manager.loadPending(),
    rejected = assert.rejects(pending, code("ASSET_REVOKED"));
  await entered.promise;
  f.manager.clear();
  hold.resolve();
  await rejected;
  await f.manager.whenIdle();
  assert.equal(f.calls.length, 0);
  assert.equal(f.images[0].closed, true);
  assert.equal(f.manager.stats.inFlightBytes, 0);
});
test("serialized payload publication does not overlap native delivery", async (t) => {
  const f = fixture({ retainSourceBytes: true }, 4);
  t.after(() => f.manager.dispose());
  const original = f.session.provideAsset.bind(f.session);
  let active = 0,
    maximum = 0;
  f.session.provideAsset = async (result) => {
    maximum = Math.max(maximum, ++active);
    await new Promise((resolve) => setTimeout(resolve, 5));
    const token = original(result);
    active--;
    return token;
  };
  const report = await f.manager.loadPending();
  assert.equal(report.loaded, 4);
  assert.equal(maximum, 1);
  assert.equal(f.calls.length, 4);
  assert(f.calls.every((call) => call.bytes.length === png().length));
});
test("native budget rejection closes the unpublished bitmap and leaves the image unresolved", async (t) => {
  const f = fixture({ retainSourceBytes: true });
  t.after(() => f.manager.dispose());
  f.session.provideAsset = () => {
    throw new Error("native payload budget exceeded");
  };
  const report = await f.manager.loadPending();
  assert.equal(report.failed, 1);
  assert.equal(f.images[0].closed, true);
  assert.equal(f.manager.stats.reservedPixels, 0);
  assert.equal(f.manager.stats.inFlightBytes, 0);
  assert.equal(f.payloads.size, 0);
});
