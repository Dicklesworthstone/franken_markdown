...
import { deferred, image, png, Session, tick } from "./tests/flow_image_fixtures.mjs";
...
test("gathers all pending pages before serial, dimension-only deliveries; sync and async sessions", async () => {
  for (const asyncMode of [false, true]) {
    const session = new Session(5),
      calls = [];
    const original = session.provideAsset.bind(session);
    let writing = 0;
    session.provideAsset = asyncMode
      ? async (result) => {
      ...
      : original;
    const assets = new FlowImageAssets(
      session,
      options({
        load: (r) => {
          calls.push(r.id);
          assert.deepEqual(session.pages, [0, 2, 4]);
          return png();
        },
      }),
    );
    const report = await assets.loadPending();
    assert.equal(report.loaded, 5);
    assert.equal(report.failed, 0);
    assert.equal(session.writes.length, 5);
    assert(session.writes.every((r) => r.bytes === undefined && r.width === 2 && r.height === 3));
    assert.equal(assets.stats.retainedPixels, 30);
    ...
    session.reflow();
    assert.equal(assets.resolveImage(descriptor(), session.token).width, 2);
    assert.equal((await assets.loadPending()).loaded, 0);
...
test("out-of-order loaders are limited, immutable input views are snapshotted", async () => {
  ...
  let started = 0;
  const bytes = png(),
    pool = new Uint8Array(bytes.length + 30);
  pool.set(bytes, 10);
  ...
  const assets = new FlowImageAssets(
    session,
    options({
      limits: { maxConcurrentLoads: 2 },
      load: () => gates[started++].promise,
      decode: async (blob, info) => {
  ...
  const work = assets.loadPending();
  await tick();
  assert.equal(started, 2);
  gates[1].resolve(view);
  await tick();
  view.fill(9);
  decodeGate.resolve();
  await tick();
  ...
  gates[2].resolve(png());
  await work;
  assert.deepEqual(decoded[0], bytes);
  assert.equal(session.writes[0].requestId, "2");
  assets.dispose();
});
...
test("a source change while decoding closes the late bitmap without a native write", async () => {
  ...
  const work = assets.loadPending();
  const refused = assert.rejects(work, code("STALE_REVISION"));
  await tick();
  assert(began);
  session.edit();
  assets.synchronize();
  await refused;
  assert.equal(assets.busy, true);
  gate.resolve(late);
  await idle(assets);
...
test("aborting a dispatched native mutation is not rollback and never signals the worker", async () => {
  ...
  const original = session.provideAsset.bind(session);
  session.provideAsset = async (...args) => {
  ...
  const work = assets.loadPending({ signal: controller.signal });
  const refused = assert.rejects(work, code("ABORTED"));
  await tick();
  assert(dispatched);
  controller.abort();
  await refused;
...
test("revocation during native delivery discards its eventual bitmap and does not replay", async () => {
  ...
  const assets = new FlowImageAssets(session, options({ decode: () => bitmap }));
  const work = assets.loadPending(),
    refused = assert.rejects(work, code("ASSET_REVOKED"));
  await tick();
  assets.clear();
  await refused;
  gate.resolve();
  ...
  const assets = new FlowImageAssets(
    ...
    options({
      load: (r) => {
        ...
        if (r.id === "2") throw new Error("private credentials must not leak");
        return png();
      },
    }),
  );
...
test("malformed inventories are rejected before granting any loader authority", async () => {
  for (const mutate of [
    (p) => {
      p.requests.push(p.requests[0]);
      p.total++;
      p.nextOffset = null;
    },
    (p) => {
      p.requests[0].generation = "2";
    },
    (p) => {
      p.nextOffset = 0;
    },
    ...
    const session = new Session(),
      original = session.pendingAssets.bind(session);
    let loads = 0;
    session.pendingAssets = (args) => {
...
test("bad decoder results and failed layout transactions close owned bitmaps", async () => {
  for (const badDimensions of [true, false]) {
    const session = new Session(),
      bitmap = image(badDimensions ? 20 : 2, 3);
    if (!badDimensions)
      session.provideAsset = () => {
        throw new Error("layout refused");
      };
    const assets = new FlowImageAssets(session, options({ decode: () => bitmap }));
    const report = await assets.loadPending();
    assert.equal(report.failed, 1);
...
test("dispose closes retained and late decoded images exactly once without owning the session", async () => {
  ...
  const assets = new FlowImageAssets(
    session,
    options({ decode: () => (calls++ ? gate.promise : first) }),
  );
  const work = assets.loadPending(),
    refused = assert.rejects(work, code("ASSET_REVOKED"));
  await tick();
...
test("limits, callback types, and already aborted signals reject before loading", async () => {
  ...
  await assert.rejects(assets.loadPending({ signal: AbortSignal.abort() }), code("ABORTED"));
  assert.equal(loads, 0);
  assets.dispose();
});
