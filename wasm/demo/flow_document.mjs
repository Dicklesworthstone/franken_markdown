...
import { FLOW_SOURCE_LIMIT, FlowError, sourceText } from "../flow_session.mjs";

const fail = (code, message, cause) => {
  throw new FlowError(code, message, { cause });
};
...
export function documentName(value) {
  if (
    ...
    fail(
      "INVALID_FILENAME",
      "Use a nonempty filename without path separators or control characters.",
    );
  }
  sourceText(value, "filename");
  if (new TextEncoder().encode(value).length > 240)
    fail("INVALID_FILENAME", "Filename exceeds 240 UTF-8 bytes.");
  return value;
}

export function documentSnapshot(value) {
  if (!value || typeof value !== "object")
    fail("INVALID_ARGUMENT", "A source document is required.");
  ...
  return Object.freeze({
    filename: documentName(value.filename),
...
export async function readMarkdownFile(file) {
  ...
  const filename = documentName(file.name),
    size = file.size;
  if (size > FLOW_SOURCE_LIMIT)
    fail("BUDGET_EXCEEDED", "Markdown file exceeds the 4 MiB source limit.");
  let bytes;
  try {
    bytes = await file.arrayBuffer();
  } catch (error) {
    fail("FILE_READ_FAILED", "The local file could not be read; the editor is unchanged.", error);
  }
  if (
  ...
  try {
    ...
    fail("INVALID_UNICODE", "The file is not valid UTF-8 Markdown.", error);
  }
...
export function markdownDownload(document) {
  ...
  const filename = /\.(md|markdown)$/i.test(value.filename)
    ? value.filename
    : `${value.filename}.md`;
  return {
    filename,
    blob: new Blob([value.source], {
 ...
 */
export function createSourceControls({
  sourceEditor,
  filename,
  open,
  prepare,
  ...
  let originalSource = sourceEditor.value,
    originalView = sourceEditor.value;
  if (initialDocument !== undefined) {
    const initial = documentSnapshot(initialDocument);
    if (
      initial.filename !== filename.value ||
  ...
  const matches = (value) =>
    value.source === sourceEditor.value && value.filename === filename.value;
  function revoke() {
    published = null;
  ...
  function alive() {
    if (disposed) fail("SESSION_DISPOSED", "Source controls are disposed.");
  }
  function replace(value) {
    alive();
    const next = documentSnapshot(value),
      previous = current();
    const previousSource = originalSource,
      previousView = originalView;
    sourceEditor.value = next.source;
    filename.value = next.filename;
    originalSource = next.source;
    originalView = sourceEditor.value;
    try {
      onReplace(next);
    } catch (error) {
      sourceEditor.value = previous.source;
      filename.value = previous.filename;
      originalSource = previousSource;
      originalView = previousView;
      throw error;
    }
    changed();
  ...
  function message(error) {
    if (!disposed)
      status.textContent = `${error?.code ?? "DOCUMENT_ERROR"}: ${error instanceof Error ? error.message : "Document operation failed."}`;
  }
  async function openFile(file) {
    ...
    const fence = () => {
      alive();
      if (ticket !== epoch || !matches(before))
        fail("STALE_SOURCE", "The editor changed during file loading; the file was not opened.");
    };
    try {
  ...
  function prepareDownload() {
    ...
    const value = snapshot(),
      output = markdownDownload(value);
    revoke();
    url = urls.createObjectURL(output.blob);
    published = current();
    download.href = url;
    download.download = output.filename;
    download.textContent = `Download ${output.filename}`;
    download.hidden = false;
    status.textContent =
      "Original Markdown prepared. Click Download to save it; no file has been overwritten.";
  }
  const onOpen = () => {
  ...
  prepare.addEventListener("click", onPrepare);
  ...
  return Object.freeze({
    ...
    dispose() {
      ...
      sourceEditor.removeEventListener("input", changed);
      filename.removeEventListener("input", changed);
      open.removeEventListener("change", onOpen);
      prepare.removeEventListener("click", onPrepare);
...
export function createSourceHistory(initialSource, options = {}) {
  ...
  const maxBytes = editInteger(
    options.maxBytes ?? HISTORY_PAYLOAD_LIMIT,
    128,
    HISTORY_PAYLOAD_LIMIT,
    "maxBytes",
  );
  let current = sourceText(initialSource),
    past = [],
    future = [],
    payloadBytes = 0,
    disposed = false;
  const alive = () => {
    if (disposed) fail("SESSION_DISPOSED", "Source history is disposed.");
  };
  ...
  function record(next, selections = {}) {
    alive();
    sourceText(next);
    editingOptions(selections, ["before", "after"]);
    const before = editSelection(selections.before, current.length),
      after = editSelection(selections.after, next.length);
    if (next === current) return false;
    let start = 0,
      oldEnd = current.length,
      newEnd = next.length;
    while (start < oldEnd && start < newEnd && current.charCodeAt(start) === next.charCodeAt(start))
      start++;
    ...
    if (lowSurrogate(current.charCodeAt(oldEnd)) || lowSurrogate(next.charCodeAt(newEnd))) {
      oldEnd++;
      newEnd++;
    }
    ...
    if (cost > maxBytes)
      fail(
        "BUDGET_EXCEEDED",
        "This edit exceeds the undo-history payload limit; nothing was recorded.",
      );
    const entry = {
      start,
      removed: own(current.slice(start, oldEnd)),
      inserted: own(next.slice(start, newEnd)),
      before,
      after,
      cost,
    };
  ...
  function travel(backward) {
    alive();
    const from = backward ? past : future,
      to = backward ? future : past;
    const entry = from.at(-1);
    if (!entry) return null;
    const remove = backward ? entry.inserted : entry.removed,
      insert = backward ? entry.removed : entry.inserted;
    const source =
      current.slice(0, entry.start) + insert + current.slice(entry.start + remove.length);
    from.pop();
    to.push(entry);
    current = source;
...
export function findSourceMatches(source, query, options = {}) {
  ...
  sourceText(source);
  sourceText(query, "query");
  const fold = (code) => (ignoreCase && code >= 65 && code <= 90 ? code + 32 : code);
  const needle = new Uint16Array(query.length),
    prefix = new Uint16Array(query.length);
  ...
  for (let i = 0, matched = 0; i < source.length; i++) {
    ...
    if (matched === needle.length) {
      if (matches.length === SOURCE_MATCH_LIMIT)
...
export function replaceSourceMatches(source, query, replacement, options = {}) {
  ...
  if (found.truncated)
    fail(
      "TOO_MANY_MATCHES",
      "More than 10000 matches; narrow the search before replacing all. The source is unchanged.",
    );
  ...
  const bytes =
    ...
    count * (encoder.encode(replacement).length - encoder.encode(query).length);
  if (bytes > FLOW_SOURCE_LIMIT)
    fail(
      "BUDGET_EXCEEDED",
      "Replacement would exceed the 4 MiB source limit; the source is unchanged.",
    );
  const chunks = [];
  let offset = 0;
  for (const match of found.matches) {
    chunks.push(source.slice(offset, match.start), replacement);
    offset = match.end;
  }
 ...
 */
export function createSourceEditingControls({
  sourceEditor,
  undo,
  redo,
  query,
  ...
  const bounded = (value, length) => ({
    start: Math.min(value?.start ?? 0, length),
    end: Math.min(value?.end ?? 0, length),
    direction: value?.direction ?? "none",
  });
  const report = (error) => {
    status.textContent = `${error?.code ?? "EDIT_ERROR"}: ${error instanceof Error ? error.message : "Source operation failed."}`;
  };
  const writable = () =>
    !disposed && !composing && !sourceEditor.disabled && !sourceEditor.readOnly;
  const ready = () => {
    if (disposed) fail("SESSION_DISPOSED", "Source editing controls are disposed.");
    if (!writable())
      fail(
        "EDITOR_BUSY",
        "Finish composing text or unlock the editor before using source editing commands.",
      );
  };
  function inventory() {
    const source = sourceEditor.value,
      text = query.value,
      fold = ignoreCase.checked;
  ...
  function refresh(announce = false) {
    ...
    try {
      ...
      if (announce)
        status.textContent =
          query.value === ""
            ? "Source editing ready. Undo history is local to this document."
            : found.truncated
              ? "More than 10000 matches. Navigation shows the first 10000; narrow the query before replacing all."
  ...
  function synchronize() {
    ...
    if (history === null) history = createSourceHistory(sourceEditor.value);
      ...
      history.record(sourceEditor.value, {
        before: bounded(before ?? last, history.source.length),
        after,
      });
    last = after;
    before = null;
  ...
  function travel(backward) {
    ...
    status.textContent = value
      ? `${backward ? "Undid" : "Redid"} one source edit.`
      : `No source edit to ${backward ? "undo" : "redo"}.`;
    refresh();
    return value !== null;
  }
  function navigate(backward) {
    ...
    let index = backward
      ? found.matches.findLastIndex((match) => match.end <= range.start)
  ...
  function commit(source, after) {
    ...
    publish({ source, selection: after });
    return true;
  }
  function replaceSelected() {
    ...
    synchronize();
    const found = inventory(),
      index = selectedIndex(found);
    if (index < 0)
      fail(
        "NO_SELECTED_MATCH",
        "Select a current source match with Previous or Next before replacing it.",
      );
    const match = found.matches[index],
      text = sourceText(replacement.value, "replacement").replace(/\r\n?/g, "\n");
    const source = sourceEditor.value;
    const changed = commit(source.slice(0, match.start) + text + source.slice(match.end), {
      start: match.start,
      end: match.start + text.length,
    });
    status.textContent = changed
      ? "Replaced the selected source match. Undo restores it."
      : "The selected match already equals the replacement.";
    return changed;
  }
  function replaceEvery() {
    ready();
    synchronize();
    const result = replaceSourceMatches(
      sourceEditor.value,
      query.value,
    ...
    status.textContent = changed
      ? `Replaced ${result.count} source matches in one undoable edit.`
  ...
  const selectionChanged = () => {
    if (!disposed && !composing && before === null && history?.source === sourceEditor.value)
      last = selection();
    refresh();
  };
  ...
  listen(sourceEditor, "beforeinput", (event) => {
    ...
    if (event.cancelable && ["historyUndo", "historyRedo"].includes(event.inputType)) {
      event.preventDefault();
      run(() => travel(event.inputType === "historyUndo"))();
    } else before = selection();
  });
  listen(sourceEditor, "compositionstart", () => {
    before = selection();
    composing = true;
    refresh();
  });
  listen(sourceEditor, "compositionend", () => {
    composing = false;
    input();
  });
  ...
  listen(sourceEditor, "keydown", (event) => {
    ...
    if (key === "z" || (key === "y" && !event.shiftKey)) {
      event.preventDefault();
      run(() => travel(key === "z" && !event.shiftKey))();
    } else if (key === "f") {
      event.preventDefault();
      const range = selection(),
        selected = sourceEditor.value.slice(range.start, range.end);
  ...
  listen(replace, "click", run(replaceSelected));
  listen(replaceAll, "click", run(replaceEvery));
  reset();
  return Object.freeze({
    undo: () => travel(true),
    redo: () => travel(false),
    findNext: () => navigate(false),
    findPrevious: () => navigate(true),
    replaceSelected,
