...
const fail = message => { throw new FlowAssetError("INVALID_LOCAL_IMAGES", message); };
...
export function createLocalImageSources(files) {
  ...
  for (const file of files) {
    ...
    if (!(file instanceof Blob) || typeof file.name !== "string" || !file.name.length || file.name.length > 512
        || /[\\/:?#<>\u0000-\u001f\u007f]/.test(file.name) || file.name === "." || file.name === "..") fail("Select files with plain local names");
    if (!file.size || file.size > 8 * 1024 * 1024 || (total += file.size) > 32 * 1024 * 1024) fail("Selected files exceed the 8 MiB per-file or 32 MiB total limit");
    const encoded = encodeURIComponent(file.name).replace(/[!'()*]/g, ch => `%${ch.charCodeAt(0).toString(16).toUpperCase()}`);
  ...
  return Object.freeze({ count, references: Object.freeze(references),
