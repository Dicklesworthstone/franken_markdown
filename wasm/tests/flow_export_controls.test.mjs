...
function fixture(readOptions) {
  const html = new Element(),
    ...
    status = new Element(),
    sourceEditor = new Element();
  const calls = [],
    created = [],
    revoked = [],
    token = { revision: "1", layoutRevision: "2" };
  let hold = null,
    error = null;
  ...
  const controls = createExportControls({
    ...
    async exportDocument(format, options, source) {
      calls.push({ format, options, source });
...
test("prepares an owned Blob, never auto-clicks, and makes a separate download available", async (t) => {
  ...
  let clicks = 0;
  f.download.addEventListener("click", () => clicks++);
  f.pdf.click();
  await tick();
  assert.equal(clicks, 0);
  assert.equal(f.download.hidden, false);
  assert.equal(f.download.download, "document.pdf");
  assert.equal(f.created[0].type, "application/pdf");
  assert.deepEqual(new Uint8Array(await f.created[0].arrayBuffer()), f.result.bytes);
  assert.deepEqual(f.calls[0], {
    format: "pdf",
    options: { pageNumbers: true, metadataEpochSeconds: 0, maxOutputBytes: 64 * 1024 * 1024 },
    source: "old",
  });
...
test("new exports revoke the previous object URL and keep only one downloadable document", async (t) => {
  ...
  assert.equal(f.download.href, "blob:test-2");
...
test("typing immediately revokes the old download, before any preview update", async (t) => {
  ...
  f.pdf.click();
  await tick();
  f.sourceEditor.value = "new";
  f.sourceEditor.dispatchEvent(new Event("input"));
...
test("programmatic source edits are caught again on download click", async (t) => {
  ...
  f.pdf.click();
  await tick();
  f.sourceEditor.value = "new without input event";
  assert.equal(f.download.click().defaultPrevented, true);
  assert.equal(f.download.hidden, true);
...
test("late export after typing/restart/disposal is discarded without creating a URL", async () => {
  for (const change of [
    ...
    (f) => f.controls.update({ status: "idle", frame: null }),
    (f) => f.controls.dispose(),
  ]) {
    const f = fixture(),
      hold = gate();
    f.hold(hold);
...
test("one physical export at a time, even after invalidation; image loading disables admission", async (t) => {
  ...
  await tick();
  assert.equal(f.created.length, 0);
  f.controls.update({ ...f.state, images: { status: "loading" } });
  f.pdf.click();
  assert.equal(f.calls.length, 1);
});
test("scroll reuses downloads; new geometry revokes them", async (t) => {
  ...
  f.pdf.click();
  await tick();
  f.controls.update({ ...f.state, status: "busy" });
  assert.equal(f.download.hidden, false);
...
test("errors/diagnostics become inert text and disposal removes listeners and URLs", async () => {
  const f = fixture();
  f.error(Object.assign(new Error("<script>native</script>"), { code: "EXPORT_FAILED" }));
  f.pdf.click();
  await tick();
  assert(f.status.textContent.includes("<script>native</script>"));
  assert.equal(f.created.length, 0);
  f.error(null);
  f.result.diagnostics = [{ severity: "warning", start: 0, end: 1, message: "Review image" }];
  f.pdf.click();
  await tick();
  assert(f.status.textContent.includes("Review image"));
  f.controls.dispose();
  assert.equal(f.revoked.length, 1);
...
test("applied publishing options are captured and frozen before renderer work", async (t) => {
  ...
  f.hold(hold);
  f.pdf.click();
  settings.title = "Later title";
  settings.baseFontSize = 18;
  assert.equal(f.calls[0].options.title, "Current title");
  assert.equal(f.calls[0].options.baseFontSize, 13);
  assert.equal(f.calls[0].options.pageNumbers, false);
  assert.equal(Object.isFrozen(f.calls[0].options), true);
...
test("HTML receives its own options, not PDF-only typography", async (t) => {
  const f = fixture((format) =>
    format === "html"
      ? { darkMode: "disabled", title: "HTML title" }
      : { author: "PDF author", baseFontSize: 12 },
  );
  t.after(() => f.controls.dispose());
  f.html.click();
  await tick();
  f.pdf.click();
  await tick();
  assert.equal(f.calls[0].options.darkMode, "disabled");
  assert.equal(f.calls[0].options.author, undefined);
  assert.equal(f.calls[1].options.author, "PDF author");
  assert.equal(f.calls[1].options.darkMode, undefined);
});
test("unknown and unsafe publishing options fail before invoking the renderer", async () => {
  for (const options of [
    ...
    assert.equal(f.created.length, 0);
    assert.match(f.status.textContent, /INVALID_OPTIONS/);
...
test("applying new settings revokes prepared downloads and rejects older pending exports", async (t) => {
  const f = fixture(() => ({ title: "Applied title" }));
  t.after(() => f.controls.dispose());
  f.pdf.click();
  await tick();
  f.controls.invalidate("Publishing settings changed.");
  assert.equal(f.download.hidden, true);
  assert.equal(f.revoked.length, 1);
  const hold = gate();
  f.hold(hold);
  f.pdf.click();
  f.controls.invalidate("New settings applied.");
  hold.resolve();
  await tick();
  assert.equal(f.created.length, 1);
  assert.equal(f.status.textContent, "New settings applied.");
});
...
test("unapplied settings appearing during preparation suppress download publication", async (t) => {
  let pending = false;
  const f = fixture(() => {
    if (pending) throw new Error("unapplied");
    return {};
  });
  t.after(() => f.controls.dispose());
  const hold = gate();
