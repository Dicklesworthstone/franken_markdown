...
function fixture(document = null) {
  ...
  const restore = new Element(),
    ...
    save = new Element(),
    status = new Element();
  sourceEditor.value = "local";
  filename.value = "local.md";
  let record = { schemaVersion: 1, version: document ? 1 : 0, document },
    unavailable = false,
    openGate = null;
  let allowRestore = true,
    ...
    opens = 0,
    closes = 0;
  const controls = createDraftControls({
    sourceEditor,
    filename,
    remember,
    refresh,
    ...
    async openStore() {
      ...
      if (unavailable)
        throw Object.assign(new Error("Unavailable storage"), { code: "STORAGE_UNAVAILABLE" });
      let closed = false;
      const check = () => {
      ...
      return {
        ...
        async save(document, version) {
          ...
          if (version !== record.version)
            throw Object.assign(new Error("other tab"), { code: "DRAFT_CONFLICT" });
          writes++;
          record = { schemaVersion: 1, version: version + 1, document };
          return structuredClone(record);
        },
        async clear(version) {
          check();
          if (version !== record.version)
            throw Object.assign(new Error("other tab"), { code: "DRAFT_CONFLICT" });
          record = { schemaVersion: 1, version: version + 1, document: null };
          return structuredClone(record);
        },
        dispose() {
          if (!closed) {
  ...
  return {
    ...
    external(value) {
      record = {
        ...
        document: { source: value, filename: "external.md" },
      };
    },
    enable(value = true) {
...
test("initial inspection never changes source or consents to saving; checkbox is the opt-in", async (t) => {
  ...
  assert.equal(f.sourceEditor.value, "local");
  assert.equal(f.remember.checked, false);
  assert.equal(f.writes, 0);
  f.enable();
  assert.equal(f.remember.checked, true);
  f.save.click();
  await f.controls.whenIdle();
  assert.equal(f.record.document.source, "local");
  assert.equal(f.controls.state.dirty, false);
  assert(f.status.textContent.includes("acknowledged"));
...
test("offered recovery blocks autosave; declined restore keeps editor and grant unchanged", async (t) => {
  ...
  t.after(() => f.controls.dispose());
  await f.controls.whenIdle();
  assert.equal(f.remember.disabled, true);
  assert.equal(f.restore.disabled, false);
  assert.equal(f.forget.disabled, false);
  f.approveRestore(false);
  f.restore.click();
  await f.controls.whenIdle();
  assert.equal(f.sourceEditor.value, "local");
  assert.equal(f.replacements, 0);
  assert.equal(f.remember.checked, false);
  f.approveRestore(true);
  f.restore.click();
  await f.controls.whenIdle();
  assert.equal(f.sourceEditor.value, "saved");
...
test("forget requires confirmation, removes only stored source, and never silently reenables saving", async (t) => {
  ...
  t.after(() => f.controls.dispose());
  await f.controls.whenIdle();
  f.approveForget(false);
  f.forget.click();
  await f.controls.whenIdle();
  assert.equal(f.record.document.source, "saved");
  f.approveForget(true);
  f.forget.click();
  await f.controls.whenIdle();
...
test("source and filename input invalidate the saved acknowledgment independently of preview", async (t) => {
  ...
  await f.controls.whenIdle();
  f.enable();
  await f.controls.flush();
  f.sourceEditor.value = "not renderable 😀";
  f.sourceEditor.dispatchEvent(new Event("input"));
  assert.equal(f.controls.state.dirty, true);
  await f.controls.flush();
  f.filename.value = "renamed.md";
  f.filename.dispatchEvent(new Event("input"));
  await f.controls.flush();
...
test("conflict visibly pauses saving and refresh offers the other version without replacing local text", async (t) => {
  ...
  assert.equal(f.remember.checked, false);
  assert.equal(f.remember.disabled, true);
  assert(f.status.textContent.includes("DRAFT_CONFLICT"));
  await f.controls.refresh();
  assert.equal(f.sourceEditor.value, "local");
  assert.equal(f.controls.state.available, true);
  assert.equal(f.restore.disabled, false);
...
test("unavailable storage leaves source untouched and explicit Refresh can reconnect", async (t) => {
  ...
  t.after(() => f.controls.dispose());
  f.block(true);
  await assert.rejects(f.controls.whenIdle());
  assert.equal(f.sourceEditor.value, "local");
  assert.equal(f.refresh.disabled, false);
  assert.equal(f.remember.disabled, true);
  assert(f.status.textContent.includes("Download Markdown"));
  f.block(false);
  await f.controls.refresh();
  assert.equal(f.remember.disabled, false);
  assert.equal(f.writes, 0);
});
test("one physical connection is retained; late open is closed after disposal", async () => {
  const f = fixture(),
    hold = gate();
  f.gate(hold);
  const pending = f.controls.whenIdle();
  assert.throws(
    () => f.controls.refresh(),
    (error) => error.code === "DRAFT_BUSY",
  );
...
test("disposing during recovery confirmation cannot replace the document", async () => {
  const f = fixture({ source: "saved", filename: "saved.md" }),
    hold = gate();
  await f.controls.whenIdle();
  f.approveRestore(hold.promise);
  const pending = f.controls.restore();
  await tick();
  f.controls.dispose();
  hold.resolve(true);
  await assert.rejects(pending, (error) => error.code === "SESSION_DISPOSED");
  assert.equal(f.replacements, 0);
  assert.equal(f.sourceEditor.value, "local");
});
