...
export function mountBookPreview(doc, scope, markup, channel, fragment) {
  ...
  const send = (kind, detail = {}) => scope.parent.postMessage({ schemaVersion: 1, channel, kind, ...detail }, "*");
  const htmlTags = new Set("a abbr article aside b bdi bdo blockquote br caption cite code col colgroup dd del details dfn div dl dt em figcaption figure footer h1 h2 h3 h4 h5 h6 header hr i img input kbd li main mark nav ol p pre q rp rt ruby s samp section small span strong style sub summary sup table tbody td tfoot th thead time tr u ul var wbr".split(" "));
  const svgTags = new Set("svg a g path rect circle ellipse line polyline polygon text tspan defs clippath mask pattern lineargradient radialgradient stop use image title desc symbol".split(" "));
  const mathTags = new Set("math semantics annotation mi mn mo mtext mspace ms mrow mfrac msqrt mroot mstyle merror mpadded mphantom mfenced menclose msub msup msubsup munder mover munderover mmultiscripts mprescripts none mtable mlabeledtr mtr mtd maligngroup malignmark".split(" "));
  ...
  try {
    ...
    if (nodes.length > 50000) throw new Error("Preview contains too many elements; export the site instead.");
    let restricted = 0;
    for (const node of nodes) {
      const tag = node.localName.toLowerCase(), ns = node.namespaceURI;
      const allowed = ns === "http://www.w3.org/1999/xhtml" ? htmlTags.has(tag)
        : ns === "http://www.w3.org/2000/svg" ? svgTags.has(tag)
        : ns === "http://www.w3.org/1998/Math/MathML" && mathTags.has(tag);
      if (!allowed || (tag === "input" && node.getAttribute("type")?.toLowerCase() !== "checkbox")) {
        node.remove(); restricted++; continue;
      }
      for (const attribute of Array.from(node.attributes)) {
        const name = attribute.localName.toLowerCase(), value = attribute.value;
        if (name === "href") {
          node.removeAttributeNode(attribute);
          if (tag === "a") { if (!links.has(node)) links.set(node, value); }
          else if (tag === "use" && /^#[^\s]+$/.test(value)) node.setAttribute("href", value);
          else if (tag === "image" && dataImage(value)) node.setAttribute("href", value);
          else restricted++;
        } else if (name.startsWith("on") || ["srcdoc", "srcset", "ping", "target", "download", "action", "autofocus", "contenteditable", "is"].includes(name)
            || name.startsWith("form") || (name === "src" && !(tag === "img" && dataImage(value)))) {
          node.removeAttributeNode(attribute); restricted++;
        }
      }
    ...
    doc.addEventListener("click", anchor, true); doc.addEventListener("auxclick", anchor, true);
    ...
    if (fragment) {
      ...
      if (destination) { destination.scrollIntoView(); destination.setAttribute("tabindex", "-1"); destination.focus({ preventScroll: true }); }
      else missing = true;
    }
...
export function bookPreviewFrame(html, channel, fragment = "") {
  if (typeof html !== "string" || html.length > 8 * 1024 * 1024 || !/^[a-f0-9]{32}$/.test(channel)
      || typeof fragment !== "string" || fragment.length > 4096) throw new TypeError("Invalid book preview frame input.");
