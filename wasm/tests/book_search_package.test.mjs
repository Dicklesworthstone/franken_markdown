...
const modules = ["demo/book_controls.mjs", "demo/book_source_search.mjs", "demo/book_search_controls.mjs"];
test("the new authoring modules and all direct imports are declared for publication", () => {
  for (const path of modules) {
    ...
    for (const match of read(`wasm/${path}`).matchAll(/from\s+["']([^"']+)["']/g)) {
      assert(manifest.files.includes(posix.normalize(posix.join(posix.dirname(path), match[1]))), `${path}: ${match[1]}`);
    }
  }
...
test("both package assemblers copy the real authoring modules and guide", () => {
  for (const path of ["scripts/check-wasm-package.sh", "scripts/dsr-wasm-package.sh"]) {
    ...
    for (const name of ["book_controls.mjs", "book_source_search.mjs", "book_search_controls.mjs"]) assert(demo?.[1].split(/\s+/).includes(name), `${path}: ${name}`);
  }
});
test("actual publisher entry initializes search and tears it down before the source owner", () => {
  ...
  assert(entry.includes("search?.suspend(); preview?.suspend(); library?.suspend(); controls.suspend();"));
  assert(entry.includes("search?.dispose(); preview?.dispose(); library?.dispose(); controls.dispose();"));
  assert(entry.includes("search?.resume();"));
});
test("all actual search controller elements exist once in the publisher markup", () => {
  const html = read("wasm/demo/book.html"), controller = read("wasm/demo/book_search_controls.mjs");
  ...
  for (const id of ["search-query", "search-replacement", "search-scope", "search-results"]) assert(html.includes(`for="${id}"`), id);
...
test("source tools do not introduce a renderer, persist source or start a worker", () => {
  const code = read("wasm/demo/book_search_controls.mjs") + read("wasm/demo/book_source_search.mjs");
  for (const forbidden of [/innerHTML/, /new Worker\(/, /fetch\(/, /localStorage/, /indexedDB/, /replaceProject\(/]) assert(!forbidden.test(code), String(forbidden));
});
