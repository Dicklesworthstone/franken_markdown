...
import { box, node, ReadingSession } from "./tests/flow_reading_fixtures.mjs";

const code = (value) => (error) => error.code === value;
const heading = (id, text = "Same heading", extra = {}) =>
  node("heading", text, { level: 2, anchorId: id, ...extra });
...
test("local navigation uses engine IDs, not reconstructed heading slugs or source spans", async () => {
  const roots = [
    ...
    heading("repeat-2"),
    heading("repeat-3"),
    heading("fmd:note:1", "[1]", {
      level: 6,
      bounds: box(500),
      enclosingSourceSpan: { startByte: 100, endByte: 120 },
    }),
  ];
...
test("legacy snapshots remain readable without guessing missing destinations", async () => {
  const document = await readFlowDocument(
    new ReadingSession([
      node("heading", "Known title", { level: 1 }),
      heading(null),
      node("paragraph", "body"),
    ]),
  );
...
test("duplicate IDs are rejected even when they cross reading-page boundaries", async () => {
  ...
  assert.deepEqual(
    session.calls.map((call) => call.offset),
    [0, 256],
  );
});
...
test("destination metadata is bounded and validated before publication", async () => {
  for (const value of ["", 3, {}, "bad\n", "bad\u0085", "\udc00"]) {
    await assert.rejects(
      readFlowDocument(new ReadingSession([heading(value)])),
      code("INVALID_READING_DATA"),
    );
  }
  await assert.rejects(
    readFlowDocument(new ReadingSession([node("paragraph", "x", { anchorId: "wrong-role" })])),
    code("INVALID_READING_DATA"),
  );
  ...
  await assert.rejects(
    readFlowDocument(session, { limits: { maxAnchorUnits: 2 } }),
    code("READING_LIMIT"),
  );
  for (const limit of [0, -1, 1048577, 1.5, NaN]) {
    await assert.rejects(
      readFlowDocument(session, { limits: { maxAnchorUnits: limit } }),
...
test("navigation is fenced after source edits, reflow, and session disposal", async () => {
  for (const action of ["edit", "reflow", "dispose"]) {
    const session = new ReadingSession([heading("target")]),
      document = await readFlowDocument(session);
...
test("anchor index is immutable, session-local, and based on preorder node indices", async () => {
  ...
  roots[1].anchorId = "mutated";
  roots[1].bounds.y = 999;
  roots[1].text = "Changed";
  assert.equal(document.locateFragment("#target").bounds.y, 70);
  assert.equal(document.locateFragment("#mutated"), null);
  assert.throws(() => {
    document.nodes[3].anchorId = "forged";
  }, TypeError);
  const other = await readFlowDocument(
    new ReadingSession([heading("target", "Other", { bounds: box(20) })]),
  );
...
test("sync and async paginated sessions resolve all engine destinations identically", async () => {
  for (const asyncMode of [false, true]) {
    const roots = Array.from({ length: 520 }, (_, i) =>
      heading(`note-${i}`, `[${i}]`, { level: 6, bounds: box(i * 20) }),
    );
    const session = new ReadingSession(roots);
    if (asyncMode) {
