...
export function createDraftSession({
  ...
  if (!Number.isInteger(delayMs) || delayMs < 0 || delayMs > 60000)
    fail("INVALID_OPTIONS", "Invalid autosave delay.");
  let disposed = false,
    ready = false,
    enabled = false,
    revision = null,
  ...
  let saved = null,
    ...
    failure = null;
  const alive = () => {
    if (disposed) fail("SESSION_DISPOSED", "Draft session is disposed.");
  };
  ...
  const state = () =>
    Object.freeze({
      ...
      error: failure
        ? Object.freeze({
            code: failure.code ?? "STORAGE_UNAVAILABLE",
            message: failure.message ?? "Draft storage failed.",
          })
        : null,
    });
  const notify = () => {
    try {
      ...
      /* UI observers cannot undo a committed save. */
    }
  };
  const stopTimer = () => {
  ...
  const validate = (result) => {
    if (
      ...
      fail("STORAGE_PROTOCOL_ERROR", "Invalid draft acknowledgment.");
    return {
      version: result.version,
      document: result.document === null ? null : documentSnapshot(result.document),
    };
  };
  function schedule() {
    stopTimer();
    if (!disposed && enabled && dirty && operation === null)
      timer = setTimeout(() => {
        timer = null;
        void flush().catch(() => {});
      }, delayMs);
  ...
  async function flush() {
    ...
    const ticket = epoch,
      expected = revision;
    if (!ready || candidate !== null || expected === null)
      fail("RECOVERY_PENDING", "Inspect and restore or forget the existing draft first.");
    await launch("save", async () => {
      const result = validate(await store.save(value, expected));
      if (result.version !== expected + 1 || !same(result.document, value))
        fail("STORAGE_PROTOCOL_ERROR", "Save acknowledgment does not match the submitted source.");
      if (disposed) return;
  ...
  function restore() {
    ...
    return launch("restore", async () => {
      const result = validate(await store.read());
      alive();
      if (ticket !== epoch || !same(before, snapshot()))
        fail("STALE_SOURCE", "Editor changed while reading recovery; nothing was replaced.");
      if (result.document === null)
        fail("NO_DRAFT", "The stored draft was removed; current Markdown is unchanged.");
      const approved = await confirmRestore(result.document);
      alive();
      if (ticket !== epoch || !same(before, snapshot()))
        fail("STALE_SOURCE", "Editor changed during recovery confirmation; nothing was replaced.");
  ...
  function forget() {
    pause();
    if (!ready || revision === null)
      fail("RECOVERY_PENDING", "Inspect the stored revision before removing it.");
    const expected = revision;
    return launch("forget", async () => {
      const result = validate(await store.clear(expected));
      if (result.version !== expected + 1 || result.document !== null)
        fail(
          "STORAGE_PROTOCOL_ERROR",
          "Removal acknowledgment does not match the requested revision.",
        );
      if (disposed) return;
      revision = result.version;
      candidate = null;
  ...
  return Object.freeze({
    ...
    setEnabled(value) {
      ...
      if (operation !== null)
        fail("DRAFT_BUSY", "Wait for the current draft operation before enabling autosave.");
      if (!ready || candidate !== null)
        fail(
          "RECOVERY_PENDING",
          "Restore or explicitly forget the stored draft before enabling autosave.",
        );
      if (failure?.code === "DRAFT_CONFLICT")
        fail("DRAFT_CONFLICT", "Refresh recovery before resolving another tab's changes.");
