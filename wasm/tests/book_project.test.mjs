...
import { createBookCollection, normalizeBookProject, serializeBookProject } from "../demo/book_collection.mjs";
...
test("sparse chapter arrays and forbidden project fields fail validation", () => {
  ...
  for (const value of [{ ...project(), images: [] }, { ...project(), options: { fontAssets: [] } }, { ...project(), schemaVersion: 2 }]) {
    assert.throws(() => normalizeBookProject(value));
  }
});
test("invalid typed source remains in the editor but cannot overwrite recovery", () => {
  const c = createBookCollection(); c.replaceProject(project());
  c.edit(0, "a.md", "bad\ud800"); assert.throws(() => c.project(), /Unicode/);
  assert.equal(c.files[0].source, "bad\ud800");
  c.edit(0, "../incomplete", "valid"); assert.throws(() => c.project(), /relative book path/);
  assert.equal(c.files[0].path, "../incomplete"); c.dispose();
});
test("project serialization enforces escaped JSON size, not just source size", () => {
  const source = "\0".repeat(4 * 1024 * 1024);
  assert.throws(() => serializeBookProject({ ...project(), files: [0, 1, 2].map(i => ({ path: `${i}.md`, source })) }), /budget/);
});
test("storage unavailability is explicit and project validation precedes initialization", async () => {
  ...
  await assert.rejects(store.save({ name: "bad", project: { ...project(), images: [] } }), { code: "INVALID_PROJECT" });
  store.close(); await assert.rejects(store.list(), { code: "LIBRARY_CLOSED" });
});
