...
function harness(overrides = {}) {
  ...
  class EngineBook {
    ...
    setImage(key, bytes) { state.calls.push(["image", key, [...bytes]]); }
    setFont(key, bytes) { state.calls.push(["font", key, [...bytes]]); }
    ...
    free() { assert.equal(this.dead, false, "double free"); this.dead = true; state.frees++; }
  }
  Object.assign(EngineBook.prototype, overrides);
  const api = createBookBindings(async () => { state.loads++; return EngineBook; });
  return { state, api };
}

test("one retained book renders all formats without reparsing", async () => {
  const { api, state } = harness();
  const session = await api.createBook(FILES, { title: "Manual", lang: "fr", toc: true, pageNumbers: true });
  ...
  assert.deepEqual(state.calls.find(call => call[0] === "metadata"), ["metadata", "Manual", undefined, "fr"]);
});

test("input validation rejects bad requests before engine loading", async () => {
  const { api, state } = harness();
  for (const files of [[], null, {}, [{ path: "a.md", source: 1 }], new Array(4097).fill(FILES[0])]) {
    await assert.rejects(api.createBook(files));
  }
  for (const options of [null, [], { toc: "false" }, { title: 4 }, { font: "unknown" }, { darkMode: "unknown" }]) {
...
test("image views preserve byte offsets and do not send the whole backing buffer", async () => {
  ...
  assert.deepEqual(state.calls.find(call => call[0] === "image"), ["image", "guide/figure.png", [1, 2, 3]]);
...
test("invalid font weights and empty images fail before allocating a book", async () => {
  ...
  for (const weight of [0, 1001, 1.5, Infinity, "700"]) {
    await assert.rejects(api.createBook(FILES, { fontAssets: [{ slot: "body-regular", bytes: new Uint8Array([1]), weight }] }));
  }
  await assert.rejects(api.createBook(FILES, { images: [{ destination: "image.png", bytes: new Uint8Array() }] }));
  assert.equal(state.constructions, 0);
});
...
test("asset byte budgets are enforced before entering the engine", async () => {
  ...
  await assert.rejects(api.createBook(FILES, { images: [{ destination: "huge.png", bytes: tooLarge }] }), /32 MiB/);
...
test("a partially configured book is freed when font validation fails", async () => {
  ...
  const { api, state } = harness({ setFont() { throw failure; } });
  await assert.rejects(api.createBook(FILES, {
...
test("one-shot helpers dispose after success and render failure", async () => {
  ...
  const broken = harness({ renderPdf() { throw failure; } });
...
test("disposed sessions refuse all operations and getters", async () => {
  ...
  for (const operation of [
    () => session.chapterCount, () => session.sourceLength,
    () => session.renderPdf(), () => session.renderEpub(), () => session.renderSite(),
...
test("non-finite and f32-inexpressible scales fail before allocation", async () => {
  ...
  assert.deepEqual(state.calls.find(call => call[0] === "css"), ["css", ""]);
  assert.deepEqual(state.calls.find(call => call[0] === "scale"), ["scale", 1.125]);
  session.dispose();
});

test("one-shot output validation failures still dispose the raw book", async () => {
  const { api, state } = harness({ renderSite() { return "not binary"; } });
