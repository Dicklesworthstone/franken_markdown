...
test("only exact selected local names and their explicit encoded aliases are authorized", async () => {
  ...
  for (const url of ["a b.png", "a%20b.png", "./a%20b.png"]) assert.deepEqual(await sources.load({ url }, context()), new Uint8Array([1, 2]));
  for (const url of ["../a%20b.png", "/a%20b.png", "//host/a%20b.png", "https://host/a%20b.png", "a%20b.png?q=1", "a%2520b.png"]) {
    assert.equal(await sources.load({ url }, context()), null);
  }
  assert.deepEqual(sources.references, ["![a b.png](a%20b.png)"]); assert.equal(sources.count, 1);
});

test("filename ambiguity and traversal-shaped names are rejected atomically", () => {
  for (const names of [["a.png", "a.png"], ["a b.png", "a%20b.png"], ["../a.png"], ["x:y.png"], ["a\\b.png"], [".."]]) {
    assert.throws(() => createLocalImageSources(names.map(n => file(n))), e => e.code === "INVALID_LOCAL_IMAGES");
  }
});
...
test("file and aggregate limits reject before reading any bytes", async () => {
  ...
  assert.throws(() => createLocalImageSources(Array.from({ length: 129 }, (_, i) => file(`${i}.png`))));
  ...
  await assert.rejects(sources.load({ url: "a.png" }, { ...context(), signal: AbortSignal.abort() }));
});

test("source references escape alt delimiters and loader grants survive caller array mutation", async () => {
  const files = [file("a[b].png")], sources = createLocalImageSources(files); files.length = 0;
