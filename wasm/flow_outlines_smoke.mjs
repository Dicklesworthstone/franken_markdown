...
try {
  ...
  for (const page of direct.pages({ glyphs: true, limit: 256 }))
  ...
  for (const [fontId, set] of requests) {
    ...
    assert.equal(metrics.unitsPerEm, a.unitsPerEm);
    assert.equal(metrics.glyphs.length, 0);
    if (ids.length)
      assert.deepEqual(direct.glyphOutlines(fontId, [ids[0], ids[0]]).glyphs, [
        a.glyphs[0],
        a.glyphs[0],
      ]);
  }
  ...
  await assert.rejects(
    remote.glyphOutlines([...requests.keys()][0], new Uint16Array(257)),
    (error) => error.code === "BUDGET_EXCEEDED",
  );
  assert.equal(await remote.getSource(), source, "a refused request leaves the worker usable");
  console.log(
    "flow-outlines: actual glyph paths, blank glyphs, font metrics, batch order and worker parity: PASS",
  );
} finally {
  remote?.dispose();
  direct.dispose();
