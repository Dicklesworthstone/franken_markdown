...
const endpoint = {
  postMessage(message, transfer = []) {
    if (message.ok && message.value?.format && workerData?.corrupt) {
      ...
      if (workerData.corrupt === "format") {
        value.format = "html";
        value.mimeType = "text/html; charset=utf-8";
      }
      if (workerData.corrupt === "size") {
        value.bytes = new Uint8Array(100);
  ...
  removeEventListener(kind, fn) {
    parentPort.off(kind, listeners.get(fn));
    listeners.delete(fn);
  },
};
class NativeDouble {
  ...
  fence(revision, layout = this.layoutRevision) {
    ...
    if (layout !== this.layoutRevision)
      throw new FlowError("STALE_LAYOUT", "native layout changed");
  }
  ...
  snapshotJson(revision, layout, offset, limit) {
    ...
    const items = workerData?.image
      ? [{ kind: "image", requestId: "1", destination: "local.png", isResolved: true }]
      : [];
...
installFlowWorker(endpoint, async (source, options) => {
  ...
  return withFlowExports(
    createFlowAdapter(raw, layout),
    { html: renderer("html"), pdf: renderer("pdf") },
    font,
  );
});
