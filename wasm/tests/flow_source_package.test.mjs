...
const demo = [
  ...
  "flow_draft_session.mjs",
  "flow_draft_controls.mjs",
];
test("manifest and both assemblers ship the complete source/recovery entrypoint", async () => {
  ...
  assert(manifest.files.includes("SOURCE.md"));
  assert(await read("wasm/SOURCE.md"));
  for (const name of demo) {
    assert(manifest.files.includes(`demo/${name}`));
    assert(await read(`wasm/demo/${name}`));
  }
  for (const name of ["check-wasm-package.sh", "dsr-wasm-package.sh"]) {
    ...
    assert(
      loops.some(
        (loop) => loop[1].split(/\s+/).includes("SOURCE.md") && loop[2].includes('cp "wasm/$file"'),
      ),
    );
    for (const file of demo)
      assert(
        loops.some(
          (loop) => loop[1].split(/\s+/).includes(file) && loop[2].includes('cp "wasm/demo/$file"'),
        ),
      );
  }
});
test("source controls have a separate script and their import graph never reaches WASM or preview code", async () => {
  const html = await read("wasm/demo/flow-canvas.html"),
    source = await read("wasm/demo/flow-source.js");
  ...
  for (const match of source.matchAll(/querySelector\("#([^"]+)"\)/g))
    assert(html.includes(`id="${match[1]}"`), match[1]);
  ...
  async function inspect(path) {
    ...
    for (const match of code.matchAll(/^import .*? from "([^"]+)";/gm)) {
      ...
      assert(next.href.startsWith(root.href));
      await inspect(next.href.slice(root.href.length));
    }
  }
  await inspect("wasm/demo/flow-source.js");
  assert(visited.has("wasm/flow_session.mjs"));
...
test("source replacement revokes native image ownership and image insertion notifies source observers", async () => {
  ...
  const handler = code.slice(
    code.indexOf('source.addEventListener("fmd-document-replaced"'),
    code.indexOf('source.addEventListener("input", update)'),
  );
  assert(handler.includes('files.value = ""'));
  assert(handler.includes("exportControls?.invalidate()"));
