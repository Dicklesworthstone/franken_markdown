...
export function createExportControls({
  ...
  urls = URL,
  readOptions = (format) =>
    format === "pdf" ? { pageNumbers: true, metadataEpochSeconds: 0 } : {},
}) {
  let state = null,
    pending = false,
    disposed = false,
    ...
    url = null;
  const available = () =>
    !disposed && !pending && state?.status === "ready" && state.images?.status !== "loading";
  const buttons = () => {
    html.disabled = pdf.disabled = !available();
  };
  function revoke() {
  ...
  async function prepare(format) {
    ...
    try {
      ...
      if (!sameOptions(options, currentOptions))
        throw new FlowError("STALE_OPTIONS", "publishing settings changed during export");
      validateFlowExportResult(result, expected, options.maxOutputBytes);
      if (result.format !== format)
        throw new FlowError("INVALID_WASM_RESPONSE", "unexpected export format");
      ...
      download.href = url;
      download.download = `document.${format}`;
      download.textContent = `Download ${format.toUpperCase()} (${result.bytes.length.toLocaleString()} bytes)`;
      download.hidden = false;
      const first = result.diagnostics[0];
      status.textContent =
        `Prepared source ${result.revision}. ${result.assetCount} embedded asset payloads; ${result.diagnostics.length} renderer diagnostics.` +
        (first
          ? ` First ${first.severity}: ${first.message}`
          : " Click Download to save the document.");
    } catch (error) {
      if (!disposed && epoch === ticket) {
        revoke();
        status.textContent = `${typeof error?.code === "string" ? error.code : "EXPORT_FAILED"}: ${error instanceof Error ? error.message : "export failed"}. Source remains in the editor.`;
      }
    } finally {
      pending = false;
      buttons();
  ...
  const onDownload = (event) => {
    if (
      ...
      sourceEditor.value !== published.source ||
      !same(published.token, state?.frame) ||
      ["idle", "disposed", "error"].includes(state?.status)
    ) {
      event.preventDefault();
      invalidate("The prepared download is stale; prepare it again.");
      return;
    }
    try {
      const [, options] = normalizeFlowExport(
        published.format,
        readOptions(published.format),
        published.token,
      );
      if (!sameOptions(options, published.options))
        throw new FlowError("STALE_OPTIONS", "publishing settings changed");
    } catch {
      event.preventDefault();
      invalidate("Publishing settings changed or are unapplied; prepare a fresh export.");
    }
  };
  html.addEventListener("click", onHtml);
  ...
  return Object.freeze({
    update(next) {
      ...
      const changed =
        state &&
        (!same(state.frame, next.frame) || ["idle", "disposed", "error"].includes(next.status));
      state = next;
    ...
    dispose() {
      ...
      revoke();
      buttons();
      html.removeEventListener("click", onHtml);
