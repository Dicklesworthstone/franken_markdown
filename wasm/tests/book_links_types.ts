import { checkBookLinks, createBook, parseBookLinkReport, type BookLinkReport } from '../book.js';
import { createBookWorker, type BookWorkerFormat } from '../book-worker.js';
...
async function exercise(format: BookWorkerFormat) {
  ...
  try { const retained = book.validateLinks(); retained.blob(); } finally { book.dispose(); }
  const worker = createBookWorker();
  try {
    const result = await worker.render(files, 'links', options, { signal: new AbortController().signal });
