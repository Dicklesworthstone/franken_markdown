...
if (!packageArg || !wasmArg) throw new Error("usage: node wasm/flow_smoke.mjs <assembled-package> <generated.wasm>");
const { init, createFlowSession } = await import(pathToFileURL(join(resolve(packageArg), "flow.js")));
await init(await readFile(resolve(wasmArg)));
const check = (id, fn) => { fn(); console.error(`flow-wasm: ${id} PASS`); };
...
try {
  ...
  check("generated-class-and-glyphs", () => {
    ...
    assert(originalItems.some(item => item.kind === "text" && item.fontRun.glyphs.some(glyph => glyph.glyphId > 0)));
  ...
  check("selection-domains", () => {
    ...
    assert(session.copySource(selected.enclosingSourceSpan.startByte, selected.enclosingSourceSpan.endByte, shown.revision).includes("**bold**"));
  });
  const link = originalItems.find(item => item.kind === "anchor" && !item.isHeading);
  check("wrapped-link-hit", () => {
    assert.equal(session.hitTest(link.bounds.x + 1, link.bounds.y + 1, shown).linkTarget, "#editor");
  });
  ...
  check("accessible-table-cells", () => {
    ...
    assert(reading.nodes.some(node => node.role === "table-header-row" && node.children.some(child => child.role === "table-header-cell")));
  });
  ...
  check("utf16-edit-and-stale-source", () => {
    ...
    assert.throws(() => session.edit(0, 0, "bad", { expectedRevision: shown.revision }), error => error.code === "STALE_REVISION");
    assert.throws(() => session.hitTest(1, 1, shown), error => error.code === "STALE_REVISION");
  });
  ...
  check("layout-fence-keeps-document-generation", () => {
    ...
    assert.throws(() => session.hitTest(1, 1, beforeResize), error => error.code === "STALE_LAYOUT");
  });
  const beforeFailure = session.token;
  const goodSource = session.source;
  check("failed-shaping-is-atomic", () => {
    assert.throws(() => session.replaceSource("😀", { expectedRevision: session.revision }), error => error.code === "LAYOUT_ERROR");
  ...
  session.provideAsset({ requestId: request.id, generation: request.generation, width: 640, height: 480 });
  ...
  try {
    ...
    fresh.provideAsset({ requestId: freshRequest.id, generation: freshRequest.generation, width: 640, height: 480 });
    check("edited-session-equals-fresh-layout", () => assert.deepEqual(items(session), items(fresh)));
  ...
  check("resource-generation-rejects-late-completion", () => {
    ...
    assert.throws(() => session.provideAsset({ requestId: request.id, generation: request.generation, width: 1, height: 1 }), error => error.code === "STALE_ASSET_GENERATION");
  });
} finally { session.dispose(); }
check("dispose", () => {
  session.dispose(); assert(session.disposed);
  assert.throws(() => session.snapshot(), error => error.code === "SESSION_DISPOSED");
});
