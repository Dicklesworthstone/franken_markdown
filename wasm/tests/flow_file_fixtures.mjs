...
export class MemoryFileHandle {
  ...
  set source(value) { this.entry.bytes = new TextEncoder().encode(value); }
  get source() { return new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(this.entry.bytes); }
  async stage(name, value) {
    this.log.push(name); this.counts[name] = (this.counts[name] ?? 0) + 1;
  ...
  async isSameEntry(other) { await this.stage("identity"); return this.entry === other.entry; }
  async requestPermission(options) { await this.stage("permission", options); return this.permission; }
  async createWritable(options) {
    await this.stage("create", options);
    let staged = null, aborted = false;
    return {
      write: async blob => {
      ...
      abort: async () => { aborted = true; await this.stage("abort"); }
    };
  }
}
