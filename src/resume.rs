...
//!
//! Each feed reclassifies unresolved source in bounded byte windows. Only
...
//!
//! Large transport chunks need not fit in the replay window. Oversized feeds
//! stage their output and publish it only after the entire feed succeeds.
//! Rejected feeds leave source bytes, offsets, output, and EOF state unchanged.
//! An unresolved construct that exhausts the replay window is refused.
//!
//! Output accumulation is optional: drain [`ResumableLexer::take_spans`] after
//! each feed to avoid retaining the entire stream. Output for an individual
//! accepted feed still scales with its token count. Checkpoints store replay
//! bytes, not emitted output, and preserve absolute source offsets.

use crate::highlight::{Span, Tok, highlight, is_supported};

#[path = "resume/checkpoint.rs"]
mod checkpoint;
#[path = "resume/input.rs"]
mod input;
...
pub enum ResumeError {
    ...
    UnsupportedLanguage,
    /// Unresolved lexical context fills the replay window before the feed
    /// can complete. Large feeds with bounded tokens are accepted.
    SuffixTooLong {
        /// Required replay-buffer byte count. Rejected bytes are not retained.
        held: usize,
...
impl ResumableLexer {
    ...
    /// Accept a byte chunk, including a partial final UTF-8 scalar. Large
    /// chunks are processed in bounded replay windows, not copied wholesale.
    ///
    /// Every typed refusal is transactional, including failures after internal
    /// windows have produced provisional spans. The same lexer can accept
    /// corrected input afterward. The limit bounds lexical replay storage,
    /// not the number of output spans generated by an accepted feed.
    pub fn feed(&mut self, chunk: &[u8]) -> Result<FeedReport, ResumeError> {
        input::feed(self, chunk)
    }
