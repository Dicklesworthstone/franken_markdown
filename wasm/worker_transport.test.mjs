...
import { OwnedWorkerRpc, FlowWorkerError, workerLimits } from "./worker_transport.mjs";
...
const invoke = (rpc, method, args = [], controls = {}) => rpc.request(method, 128, () => ({ args }), controls);
const rejects = (promise, code) => assert.rejects(promise, error => error instanceof FlowWorkerError && error.code === code);

test("real worker executes FIFO mutations and acknowledges state before resolving", async t => {
  let count = -1;
  const rpc = channel(t, {}, state => { count = state.count; });
...
test("pre-aborted requests never snapshot inputs", async t => {
  ...
  let snapshots = 0;
  await rejects(rpc.request("increment", 5, () => { snapshots++; return { args: [] }; },
...
test("pending count admission happens before copying and recovers after completion", async t => {
  ...
  let snapshots = 0;
  await rejects(rpc.request("increment", 1, () => { snapshots++; return { args: [] }; }), "WORKER_QUEUE_FULL");
  assert.equal(snapshots, 0);
  await running;
...
for (const [method, code] of [["crash", "WORKER_FAILED"], ["badReply", "WORKER_PROTOCOL_ERROR"],
  ["badValue", "WORKER_PROTOCOL_ERROR"], ["untypedError", "WORKER_OPERATION_FAILED"]]) {
...
test("invalid acknowledgments cannot publish new local state", async t => {
  const rpc = channel(t, {}, () => { throw new Error("invalid state"); });
...
test("failed argument preparation does not leak queue capacity", async t => {
  const rpc = channel(t);
  await assert.rejects(rpc.request("count", 100, () => { throw new Error("snapshot failed"); }));
  assert.equal(rpc.pendingOperations, 0);
  assert.equal(rpc.pendingBytes, 0);
  assert.equal(await invoke(rpc, "count"), 0);
});
...
test("worker options and controls reject impossible budgets", async t => {
  for (const options of [{ maxPendingOperations: 0 }, { maxPendingBytes: Infinity },
    { timeoutMs: -1 }, { timeoutMs: 2 ** 31 }, { surprise: true }]) {
    assert.throws(() => workerLimits(options), error => error.code === "INVALID_OPTIONS");
  }
