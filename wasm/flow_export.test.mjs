...
import {
  FLOW_EXPORT_LIMIT,
  normalizeFlowExport,
  validateFlowExportResult,
  withFlowExports,
} from "./flow_export.mjs";
...
const image = (id, destination = `image-${id}.png`, resolved = true) => ({
  kind: "image",
  requestId: String(id),
  destination,
  isResolved: resolved,
...
export class ExportSession {
  ...
  layoutRevision = "9007199254740997";
  ...
  snapshot(options) {
    ...
    const { offset, limit } = options,
      end = Math.min(offset + limit, this.items.length);
    return {
      schemaVersion: 1,
      ...this.token,
      offset,
      total: this.items.length,
      nextOffset: end < this.items.length ? end : null,
...
function fixture() {
  ...
  const render = (format) => async (source, options) => {
    ...
    return (
      result ?? {
        format,
        mimeType: format === "html" ? "text/html; charset=utf-8" : "application/pdf",
        bytes: utf8(
          format === "html" ? "<!doctype html><p>Native double</p>" : "%PDF-native-double",
        ),
        diagnostics: [],
      }
    );
...
test("exports original Markdown through the chosen core with data-only provenance", async () => {
  ...
  const result = await f.api.exportDocument(
    "pdf",
    { title: "Document", toc: true, pageNumbers: true },
    token,
  );
  assert.equal(f.calls.length, 1);
  assert.equal(f.calls[0].source, f.session.source);
  assert.equal(f.calls[0].options.font, "serif");
  assert.equal(f.calls[0].options.allowRawHtml, false);
  assert.equal(f.calls[0].options.metadataEpochSeconds, 0);
  assert.equal(f.calls[0].options.pageNumbers, true);
  assert.deepEqual(f.calls[0].options.pdfImages, []);
  ...
  assert.equal(f.session.revision, token.revision);
...
test("rejects unknown/coerced settings, unsafe HTML/font/asset injection, and invalid identities before work", async () => {
  ...
  for (const [format, options] of [
    ...
    ["pdf", { maxOutputBytes: FLOW_EXPORT_LIMIT + 1 }],
    ["pdf", { baseFontSize: 100 }],
    ["pdf", { fitToPages: 0 }],
    ["html", { lang: "en\nX" }],
  ]) {
  ...
  for (const revision of [1, "01", "18446744073709551616"]) {
    await assert.rejects(
      f.api.exportDocument("pdf", {}, { revision, layoutRevision: "1" }),
      code("INVALID_IDENTITY"),
    );
  }
  assert.equal(f.calls.length, 0);
  assert.equal(f.session.reads.length, 0);
  const normalized = normalizeFlowExport(
    "pdf",
    {},
...
test("collects late-page images, deduplicates equal destinations and copies exact owned payloads", async () => {
  const f = fixture();
  f.session.items = [
    ...Array.from({ length: 257 }, () => ({ kind: "text" })),
    image(1, "same.png"),
    image(2, "same.png"),
    image(3),
  ];
  const backing = new Uint8Array([99, 1, 2, 99]);
  f.session.assets.set("1", backing.subarray(1, 3));
  f.session.assets.set("2", new Uint8Array([1, 2]));
  f.session.assets.set("3", new Uint8Array([3]));
  const result = await f.api.exportDocument("html", {}, f.session.token);
  assert.deepEqual(
    f.session.reads.map((p) => p.offset),
    [0, 256],
  );
  assert(
    f.session.reads.every((p) => p.glyphs === false && p.token.revision === f.session.revision),
  );
  assert.deepEqual(
    f.calls[0].options.pdfImages.map((v) => [v.destination, [...v.bytes]]),
    [
      ["same.png", [1, 2]],
      ["image-3.png", [3]],
    ],
...
test("fails rather than dropping unresolved, dimension-only, empty, or ambiguous images", async () => {
  for (const mode of ["pending", "dimensions", "empty", "ambiguous"]) {
    const f = fixture();
    f.session.items = [image(1, "x.png", mode !== "pending")];
    if (mode === "empty") f.session.assets.set("1", new Uint8Array());
    if (mode === "ambiguous") {
      f.session.items.push(image(2, "x.png"));
      f.session.assets.set("1", new Uint8Array([1]));
      f.session.assets.set("2", new Uint8Array([2]));
    }
    await assert.rejects(
      f.api.exportDocument("html", {}, f.session.token),
      code(mode === "ambiguous" ? "AMBIGUOUS_EXPORT_ASSET" : "UNRESOLVED_EXPORT_ASSET"),
    );
    assert.equal(f.calls.length, 0);
  }
});
...
test("fences before work and before publishing a delayed export after source/layout changes or disposal", async () => {
  for (const mutation of ["source", "layout", "dispose"]) {
    ...
    assert.equal(f.calls.length, 1);
    if (mutation === "source") {
      f.session.source = "new";
      f.session.revision = "9007199254740994";
  ...
  const f = fixture();
  await assert.rejects(
    f.api.exportDocument("pdf", {}, { revision: "1", layoutRevision: "1" }),
    code("STALE_REVISION"),
  );
  assert.equal(f.calls.length, 0);
  assert.equal(f.session.reads.length, 0);
...
test("owns source, options and image bytes across renderer awaits; rejects concurrent physical exports", async () => {
  const f = fixture(),
    hold = gate();
  f.setGate(hold);
  const source = f.session.source,
    options = { title: "before", maxOutputBytes: 128 };
  f.session.items = [image(1)];
  const payload = new Uint8Array([1, 2]);
  f.session.assets.set("1", payload);
  ...
  assert.equal(f.calls[0].options.title, "before");
  assert.deepEqual([...f.calls[0].options.pdfImages[0].bytes], [1, 2]);
  hold.resolve();
  await promise;
  await f.api.exportDocument("pdf", {}, f.session.token);
...
test("recoverable renderer failure releases the export slot without changing source", async () => {
  ...
  f.setGate(hold);
  const promise = f.api.exportDocument("pdf", {}, f.session.token);
  hold.reject(new Error("render rejected"));
  await assert.rejects(promise, code("EXPORT_FAILED"));
  f.setGate(null);
  const result = await f.api.exportDocument("html", {}, f.session.token);
  assert.equal(result.revision, f.session.revision);
...
test("WebAssembly traps remain fatal-class errors rather than ordinary export failures", async () => {
  ...
  f.setGate(hold);
  const promise = f.api.exportDocument("pdf", {}, f.session.token);
  hold.reject(new WebAssembly.RuntimeError("unreachable"));
...
test("rejects stalled, inconsistent and oversized display inventories without rendering", async () => {
  for (const change of [
    ...
    (p) => ({ ...p, revision: "1" }),
    (p) => ({ ...p, items: [] }),
    (p) => ({ ...p, total: 500001, nextOffset: 256 }),
  ]) {
    const f = fixture();
    f.session.items = Array.from({ length: 300 }, () => ({ kind: "text" }));
    const original = f.session.snapshot.bind(f.session);
    f.session.snapshot = (options) => change(original(options));
    await assert.rejects(f.api.exportDocument("pdf", {}, f.session.token));
    assert.equal(f.calls.length, 0);
  }
  const f = fixture();
  f.session.items = Array.from({ length: 1025 }, (_, i) => image(i, "same.png"));
...
test("rejects shared/detached image memory and both per-image and aggregate payload overflow", async () => {
  ...
  f.session.items = Array.from({ length: 5 }, (_, i) => image(i));
  f.session.assetBytes = () => payload;
  await assert.rejects(f.api.exportDocument("pdf", {}, f.session.token), code("BUDGET_EXCEEDED"));
  assert.equal(f.calls.length, 0);
});
...
test("checks output format, size, diagnostics and copies render-owned result bytes", async () => {
  ...
  f.setResult(good);
  await assert.rejects(
    f.api.exportDocument("pdf", { maxOutputBytes: 2 }, f.session.token),
    code("BUDGET_EXCEEDED"),
  );
  const output = await f.api.exportDocument("pdf", {}, f.session.token);
  assert.notEqual(output.bytes.buffer, good.bytes.buffer);
  good.bytes.fill(0);
  assert.equal(new TextDecoder().decode(output.bytes), "%PDF-double");
  assert.deepEqual(output.diagnostics, good.diagnostics);
  assert.notEqual(output.diagnostics, good.diagnostics);
  assert.throws(
    () => validateFlowExportResult({ ...output, layoutRevision: "1" }, f.session.token),
...
test("preserves live getter descriptors while wrapping a frozen production-style facade", async () => {
  ...
  await assert.rejects(
    api.exportDocument("pdf", {}, { revision: "2", layoutRevision: "1" }),
