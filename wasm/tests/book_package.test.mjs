...
test("the public worker and complete publishing workbench are in the package manifest", () => {
  assert.deepEqual(manifest.exports["./book-worker"], {
    types: "./book-worker.d.ts",
    import: "./book-worker.js",
  });
  ...
  for (const path of ["./book_worker_entry.js", "./demo/book.js"])
    assert(manifest.sideEffects.includes(path), path);
});
for (const script of ["dsr-wasm-package.sh", "check-wasm-package.sh"])
  test(`${script} copies every added runtime and demo dependency`, () => {
    const source = read(`../../scripts/${script}`);
    const loops = [...source.matchAll(/for file in ([\s\S]*?); do([\s\S]*?)done/g)];
    const copies = (prefix) =>
      loops
        .filter((match) => match[2].includes(`cp "wasm/${prefix}$file"`))
