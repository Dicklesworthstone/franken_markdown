...
function harness(load) {
  ...
  class EngineBook {
    ...
    setImage(destination, bytes) {
      state.calls.push(["image", destination, [...bytes]]);
    }
    setFont(slot, bytes) {
      state.calls.push(["font", slot, [...bytes]]);
    }
...
test("default expansion delegates ordered chapters and include-only sources to Rust once", async () => {
  ...
  const files = [chapter(), { path: "end.md", source: "# End" }],
    shared = resource();
  const session = await api.createBook(files, { includeSources: [shared] });
  assert.deepEqual(state.bundles[0], {
    paths: files.map((file) => file.path),
    sources: files.map((file) => file.source),
    includePaths: [shared.path],
    includeSources: [shared.source],
  });
  ...
  for (const output of [session.renderPdf(), session.renderEpub(), session.renderSite()])
    assert.equal(output.sourceLength, bytes);
...
test("code examples and unused resources still reach Rust for semantic validation", async () => {
  ...
  assert.equal(state.bundles[0].sources[0], source);
  literal.dispose();
  const unused = await api.createBook([{ path: "plain.md", source: "# Plain" }], {
    includeSources: [resource()],
  });
  assert.equal(state.bundles[1].includePaths[0], resource().path);
  unused.dispose();
});
...
test("explicit parse-only mode preserves directives and rejects silently ignored resources", async () => {
  ...
  assert.equal(state.calls[0][2][0], chapter().source);
  session.dispose();
  const loads = state.loads;
  await assert.rejects(
    api.createBook([chapter()], { expandIncludes: false, includeSources: [resource()] }),
    /includeSources requires/,
  );
  assert.equal(state.loads, loads);
});

test("outdated engines never silently render an unexpanded book", async () => {
  const { api, state, EngineBook } = harness();
  delete EngineBook.fromSources;
  await assert.rejects(api.createBook([chapter()]), /FmdBook.fromSources.*rebuild/);
  assert.equal(state.instances.length, 0);
  const plain = await api.createBook([{ path: "a.md", source: "# Plain" }]);
  plain.dispose();
  const literal = await api.createBook([chapter()], { expandIncludes: false });
  literal.dispose();
...
test("chapter/resource/options snapshots survive changes during asynchronous engine loading", async () => {
  ...
  const options = {
    includeSources: shared,
    title: "Original",
    images: [{ destination: "x.png", bytes: image }],
  };
  const pending = api.createBook(files, options);
  files[0].source = "Different";
  files.push(resource());
  shared[0].path = "wrong.md";
  shared[0].source = "Changed";
  shared.push(chapter());
  ...
  assert.deepEqual(state.bundles[0], {
    paths: expected.files.map((file) => file.path),
    sources: expected.files.map((file) => file.source),
    includePaths: expected.shared.map((file) => file.path),
    includeSources: expected.shared.map((file) => file.source),
  });
  assert.deepEqual(
    state.calls.find((call) => call[0] === "image"),
    ["image", "x.png", [1, 2, 3]],
  );
  assert.equal(state.calls.find((call) => call[0] === "metadata")[1], "Original");
  session.dispose();
});
...
test("resource shape, malformed Unicode, and expansion-option types fail before loading", async () => {
  const { api, state } = harness();
  for (const includeSources of [
    {},
    "x",
    [null],
    [{ path: "x", source: 1 }],
    [{ path: "bad\ud800", source: "text" }],
    [{ path: "x", source: "bad\udc00" }],
  ])
    await assert.rejects(api.createBook([chapter()], { includeSources }));
  for (const expandIncludes of [null, 0, 1, "true", {}])
    await assert.rejects(api.createBook([chapter()], { expandIncludes }));
...
test("4096-source budget is combined across chapters and resources", () => {
  ...
  assert.equal(
    prepareBookInput([chapter()], { includeSources: resources }).options.includeSources.length,
    4095,
  );
  assert.throws(
    () => prepareBookInput([chapter(), resource()], { includeSources: resources }),
    /4096/,
  );
  assert.throws(
    () => prepareBookInput([chapter()], { includeSources: new Array(4096).fill(resource()) }),
    /4096/,
  );
});
...
test("64 MiB UTF-8 budget includes resource paths and chapter source together", async () => {
  ...
  await assert.rejects(
    api.createBook([{ path: "a.md", source: half }], {
      includeSources: [{ path: "b.txt", source: half }],
    }),
    /64 MiB|budget/,
  );
  assert.equal(state.loads, 0);
...
test("one-shot helpers retain source bundles across every format and dispose each book", async () => {
  ...
  for (const method of ["renderBookPdf", "renderBookEpub", "renderBookSite"]) {
    const output = await api[method]([chapter()], {
      includeSources: [resource()],
      fontScale: 1.125,
      toc: true,
    });
  ...
  assert.equal(state.bundles.length, 3);
  assert.equal(state.frees, 3);
...
test("raw Rust expansion errors become bounded Errors without allocating a book", async () => {
  const { api, state } = harness();
  state.failure = "include_cycle: " + "x".repeat(10000);
  await assert.rejects(
    api.createBook([chapter()]),
    (error) =>
      error instanceof Error &&
      error.message.startsWith("include_cycle:") &&
...
test("post-expansion configuration failure still releases the new WASM book", async () => {
  const { api, state, EngineBook } = harness();
  const failure = new Error("bad font");
  EngineBook.prototype.setFont = () => {
    throw failure;
  };
  await assert.rejects(
    api.createBook([chapter()], {
  ...
  const workerFactory = () => {
    ...
    const scope = {
      ...
      postMessage(value, transfer) {
        ...
        queueMicrotask(() => {
          if (!dead) listeners.get("message")?.({ data });
        });
      },
    };
...
test("production worker endpoints forward resources and immutable snapshots to the real facade", async () => {
  ...
  const files = [chapter()],
    shared = [resource()],
    expected = structuredClone(shared);
  const image = new Uint8Array([1, 2, 3]);
  const pending = channel.client.render(files, "site", {
    includeSources: shared,
    images: [{ destination: "x.png", bytes: image }],
  });
  shared[0].source = "Different";
  shared.length = 0;
  files[0].source = "No include";
  image.fill(9);
  const output = await pending;
  assert.deepEqual(
    state.bundles[0].includeSources,
    expected.map((file) => file.source),
  );
  assert.equal(state.bundles[0].sources[0], chapter().source);
  assert.equal(
    output.sourceLength,
    bookTextBytes(chapter().source) + bookTextBytes(expected[0].source),
  );
  assert.deepEqual(
    state.calls.find((call) => call[0] === "image"),
    ["image", "x.png", [1, 2, 3]],
  ...
  assert.equal(channel.state.terminated, 1);
  assert.equal(state.frees, 1);
  assert.equal(channel.client.busy, false);
...
test("include failures keep their reason through worker diagnostics and leave the client reusable", async () => {
  ...
  await assert.rejects(
    channel.client.render([chapter()], "pdf", { includeSources: [resource()] }),
  ...
  state.failure = null;
  assert.equal(
    (await channel.client.render([chapter()], "epub", { includeSources: [resource()] })).extension,
    "epub",
  );
  assert.equal(channel.state.terminated, 2);
  assert.equal(state.frees, 1);
  channel.client.dispose();
});
