...
class NativeWireDouble {
  ...
  source = "source";
  constructor() {
    if (workerData === "unsupported") this.viewportJson = undefined;
  }
  free() {}
  ...
  reflow(revision, layoutRevision) {
    if (revision !== this.revision || layoutRevision !== this.layoutRevision)
      throw new Error("unexpected test reflow token");
  ...
  viewportJson(revision, layoutRevision, x, y, width, height, afterIndex) {
    ...
    return JSON.stringify({
      ...
      afterIndex,
      total: 100000,
      totalBounds: { x: 0, y: 0, width: 800, height: 2000000 },
      visitedEntries: 32,
      nextIndex: null,
      items:
        afterIndex > 95000
