...
export const BOOK_INSPECTION_LIMITS = Object.freeze({
  chapters: 128,
...
const invalid = () =>
  fail("INVALID_INSPECTION", "The engine returned an invalid inspection report.");
function text(value, maximum) {
  if (typeof value !== "string") throw invalid();
  let size = 0;
  if (value.length > maximum)
    throw fail("INSPECTION_LIMIT", "Inspection text exceeds its byte budget.");
...
function array(value, maximum) {
  ...
  if (value.length > maximum)
    throw fail("INSPECTION_LIMIT", "Inspection contains too many findings or headings.");
  return value;
}
function sourceSnapshot(files) {
  array(files, L.chapters);
  if (!files.length) throw invalid();
  let size = 0;
  const seen = new Set();
  return Array.from(files, (file) => {
    const path = file?.path,
      source = file?.source;
    text(path, 1024);
    const sourceBytes = text(source, L.chapterBytes);
    if (!path || seen.has(path)) throw invalid();
    seen.add(path);
...
async function fingerprint(source) {
  if (!globalThis.crypto?.subtle)
    throw fail(
      ...
      "Source fingerprinting requires Web Crypto on a secure origin.",
    );
...
function finding(value, accessibility = false) {
  ...
  text(value.code, 128);
  const message = accessibility ? value.detail : value.message;
  text(message, 4096);
  if (!value.code) throw invalid();
...
function statistics(value, bytes) {
  if (
    !value ||
    value.schema !== "fmd-document-stats-v1" ||
    !value.structure ||
  ...
  for (const key of ["flesch_reading_ease", "flesch_kincaid_grade"]) {
    if (!Number.isFinite(value[key])) throw invalid();
    metrics[key] = value[key];
  }
  text(value.reading_ease_label, 256);
  metrics.reading_ease_label = value.reading_ease_label;
  const structure = Object.fromEntries(
    structures.map((key) => [key, integer(value.structure[key])]),
  );
  const outline = Array.from(array(value.outline, L.headings), (heading) => {
    if (!heading || !Number.isInteger(heading.level) || heading.level < 1 || heading.level > 6)
      throw invalid();
    text(heading.text, 4096);
    text(heading.slug, 4096);
...
function accessibility(value) {
  ...
  return stageBudget({
    status: "ok",
    findings: Array.from(array(value.findings, L.findings), (value) => finding(value, true)),
  });
}
  ...
  for (const chapter of chapters) {
    ...
    if (chapter.statistics.status === "ok") {
      ...
      for (const key of counters)
        total[key] = integer(total[key] + chapter.statistics.metrics[key]);
    }
  }
  return {
    total,
...
export async function inspectBook(engine, files) {
  ...
  for (const file of input) {
    const chapter = {
      ...
      sourceSha256: await fingerprint(file.source),
    };
    try {
      chapter.statistics = statistics(await engine.documentStats(file.source), file.sourceBytes);
    } catch (error) {
      chapter.statistics = failure(error);
    }
    try {
      chapter.accessibility = accessibility(await engine.accessibilityAudit(file.source));
    } catch (error) {
      chapter.accessibility = failure(error);
    }
  ...
  return {
    ...
    sourceLength: input.reduce((sum, file) => sum + file.sourceBytes, 0),
  };
}
function checkedStage(value, bytes, kind) {
  ...
  return stageBudget({
    status: "ok",
    findings: Array.from(array(value.findings, L.findings), (value) => finding(value)),
  });
}
...
export async function readBookInspection(bytes, files) {
  ...
  if (
    ...
    throw invalid();
  if (!bytes.byteLength || bytes.byteLength > L.reportBytes)
    throw fail("INSPECTION_LIMIT", "Inspection report exceeds its byte budget.");
  let value;
  try {
  ...
  if (
    !value ||
    value.schema !== "fmd-book-inspection-v1" ||
    value.scope !== "unexpanded-source-chapters" ||
    !Array.isArray(value.chapters) ||
    value.chapters.length !== input.length
  )
    throw invalid();
  const chapters = [];
  for (const [index, file] of input.entries()) {
    ...
    if (
      !chapter ||
      chapter.path !== file.path ||
      chapter.sourceBytes !== file.sourceBytes ||
      chapter.sourceSha256 !== (await fingerprint(file.source))
    )
      throw fail("STALE_INSPECTION", "The report belongs to different book source.");
    chapters.push({
      path: file.path,
      sourceBytes: file.sourceBytes,
