...
test("a subsequent reflow may advance layout before an earlier image acknowledgment is observed", async () => {
  const session = new Session(),
    original = session.provideAsset.bind(session),
    bitmap = image();
  session.provideAsset = async (result) => {
    const ack = original(result);
    session.reflow();
    return ack;
  };
  const assets = new FlowImageAssets(session, {
    load: () => png(),
    decode: () => bitmap,
    timeoutMs: 0,
  });
  try {
    ...
    assert.equal(
      assets.resolveImage(
        { requestId: "1", destination: "1.png", isResolved: true },
        session.token,
      ),
      bitmap,
    );
