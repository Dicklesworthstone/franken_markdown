...
import { FLOW_ASSET_LIMIT, FlowError, identity, normalizeFlowError } from "./flow_session.mjs";
...
export function normalizeAssetBatch(input) {
  ...
  if (input.length > FLOW_ASSET_BATCH_COUNT)
    fail("BUDGET_EXCEEDED", "asset batch exceeds 1024 results");
  const ids = new Set(),
    results = [];
  let retained = 0,
    generation = null;
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (
      ...
      Object.keys(value).some(
        (key) => !["requestId", "generation", "width", "height", "bytes"].includes(key),
      )
    ) {
    ...
    if (ids.has(result.requestId))
      fail("UNKNOWN_ASSET_REQUEST", "an asset request occurs twice in the batch");
    ...
    if (data !== undefined) {
      ...
      if (result.bytes.byteLength > FLOW_ASSET_LIMIT)
        fail("BUDGET_EXCEEDED", "asset exceeds the 8 MiB payload limit");
      retained += result.bytes.byteLength;
      if (retained > FLOW_ASSET_BATCH_BYTES)
        fail("BUDGET_EXCEEDED", "asset batch exceeds 32 MiB of payloads");
    }
    results.push(result);
  }
  return results;
...
export function packAssetBatch(input) {
  ...
  const payload = new Uint8Array(
    results.reduce((sum, result) => sum + (result.bytes?.byteLength ?? 0), 0),
  );
  let offset = 0;
  const rows = results.map((result) => {
    const length = result.bytes === undefined ? "-" : String(result.bytes.byteLength);
    if (result.bytes !== undefined) {
      payload.set(result.bytes, offset);
...
export function withAssetBatches(session, backend) {
  ...
  const supported = () => {
    alive();
    return typeof backend()?.provideAssetsPacked === "function";
  };
      ...
      value(input) {
        ...
        if (!supported())
          fail(
            ...
            "this WASM artifact lacks atomic asset batches; rebuild from matching source",
          );
        ...
        if (session.token.revision !== expected.revision)
          fail("STALE_ASSET_GENERATION", "resource generation changed while preparing the batch");
        if (!results.length) return session.token;
        try {
          backend().provideAssetsPacked(packed.metadata, packed.payload);
        } catch (error) {
