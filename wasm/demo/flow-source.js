...
function start() {
  ...
  retained = null;
  controls = createSourceControls({ sourceEditor: source, filename,
    open: document.querySelector("#open-markdown"), prepare: document.querySelector("#prepare-markdown"),
    download: document.querySelector("#source-download"), status, initialDocument: initial,
    confirmReplace: next => window.confirm(`Replace the current editor with ${next.filename}? Download the current Markdown first to keep a separate copy.`),
  ...
  drafts = createDraftControls({ sourceEditor: source, filename,
    ...
    save: document.querySelector("#save-draft"), status: document.querySelector("#draft-status"),
    readDocument: () => controls.snapshot(), restoreDocument: next => controls.replace(next),
    confirmRestore: next => window.confirm(`Restore ${next.filename} from browser draft storage and replace the current editor? Current image access will be revoked.`),
    confirmForget: name => window.confirm(`Forget the stored draft ${name ?? ""}? This removes only its stored source, not the current editor. Download Markdown first for a separate copy.`)
  });
}
