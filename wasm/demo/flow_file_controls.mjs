...
const buttons = Object.freeze({
  "file-open": "Open editable Markdown",
  "file-save": "Save current file",
  "file-save-as": "Save Markdown as…",
  "file-reload": "Reload file version",
  "file-keep": "Keep editing",
  "file-backup": "Prepare recovery download",
  "file-disconnect": "Disconnect file",
});
function mount(root) {
  let panel = root.querySelector("#file-controls");
  if (!panel) {
    panel = root.createElement("fieldset");
    panel.id = "file-controls";
    const legend = root.createElement("legend");
    legend.textContent = "Direct Markdown file saving";
    const help = root.createElement("p");
    help.id = "file-help";
    help.textContent =
      "These optional controls can overwrite a file you explicitly choose. No automatic disk saves occur. Open/reload replaces source after confirmation; saving preserves edits and image access. Handles are never stored and are disconnected when this page is suspended.";
  ...
  const elements = Object.fromEntries(
    [...Object.keys(buttons), "file-status"].map((id) => [id, root.querySelector(`#${id}`)]),
  );
  if (Object.values(elements).some((value) => !value))
    throw new Error("File controls markup is incomplete.");
...
export function createFileControls({
  ...
  const { elements: el } = mount(root),
    listeners = [];
  const canOpen =
    window.isSecureContext === true && typeof window.showOpenFilePicker === "function";
  const canChoose =
    window.isSecureContext === true && typeof window.showSaveFilePicker === "function";
  ...
  let disposed = false,
    ...
    session;
  const types = [
    { description: "UTF-8 Markdown", accept: { "text/plain": [".md", ".markdown", ".txt"] } },
  ];
  ...
  function refresh(state = session.state) {
    ...
    const phases = {
      ...
      saved:
        "Current source matches the last verified disk baseline. External changes are checked when saving, not monitored continuously.",
      ...
      conflict:
        "Disk conflict: current-file saving is paused. Keep editing, save to a different file, or reload explicitly.",
      uncertain:
        "Save outcome uncertain: do not assume this file was saved. Keep a separate copy before reloading.",
    ...
    el["file-status"].textContent =
      `${availability}${state.filename ? `Connected file: ${state.filename}. ` : ""}${phases[state.phase] ?? ""} ${state.message}`;
    const warn =
      touched && (state.dirty || !!state.busy || ["conflict", "uncertain"].includes(state.phase));
    if (warn && !unloadListening) {
      window.addEventListener("beforeunload", beforeUnload);
      unloadListening = true;
    } else if (!warn && unloadListening) {
  ...
  session = createFileDocumentSession({
    ...
    replaceDocument: (next) => controls.replace(next),
    renameDocument: (name) => {
      filename.value = name;
      filename.dispatchEvent(new Event("input", { bubbles: true }));
    },
    isBlocked: () =>
      composing || sourceEditor.disabled || sourceEditor.readOnly || !!ordinaryOpen?.disabled,
    onState: refresh,
  });
  const confirmation = ({ filename: name }) =>
    confirm(
      `Replace the editor with ${name}? Current unsaved edits and undo history will be replaced, and image access revoked. Prepare a recovery download first to keep a separate copy.`,
    );
  function open() {
    return session.open(
      canOpen
        ? () => window.showOpenFilePicker({ multiple: false, excludeAcceptAllOption: true, types })
        : undefined,
      confirmation,
    );
  }
  function saveAs() {
    return session.saveAs(
      canChoose
        ? (name) => {
            const suggestedName = documentName(
              /\.(md|markdown|txt)$/i.test(name) ? name : `${name}.md`,
            );
      ...
      ({ filename: name }) =>
        confirm(
          `Overwrite the existing contents of ${name} with this Markdown source? This changes the selected file on disk.`,
        ),
    );
  }
  function backup() {
    ...
    el["file-status"].textContent =
      "Recovery Markdown prepared. Click the source Download link to save it. This has not overwritten a file or marked the current file saved.";
  }
  function save() {
    const state = session.state;
  ...
  function report(error) {
    ...
    refresh();
    el["file-status"].textContent +=
      ` ${error instanceof FlowError ? `${error.code}: ${error.message}` : "File controls failed; prepare a source download to keep your work."}`;
  }
  ...
  listen(sourceEditor, "input", changed);
  listen(filename, "input", changed);
  ...
  listen(
    el["file-reload"],
    "click",
    run(() => session.reload(confirmation)),
  );
  listen(
    el["file-disconnect"],
    "click",
  ...
  return Object.freeze({
    ...
    save,
    saveAs,
    reload: () => session.reload(confirmation),
    backup,
    ...
    dispose() {
      ...
      el["file-status"].textContent =
        "File access disconnected. Source is retained; reopen explicitly after returning to the page.";
    },
  });
}
