...
import { BOOK_WORKBENCH_LIMITS as B, normalizeBookProject } from "./book_collection.mjs";
export const BOOK_SEARCH_LIMITS = Object.freeze({
  queryBytes: 4096,
  replacementBytes: 65536,
  matches: 10000,
});
const L = BOOK_SEARCH_LIMITS,
  searches = new WeakMap();
...
export function findBookSource(project, query, { matchCase = true, chapter = null } = {}) {
  ...
  for (const [index, file] of value.files.entries()) {
    ...
    for (let found; (found = pattern.exec(file.source)) !== null; ) {
      if (matches.length === L.matches)
        throw bookError(
          "SEARCH_LIMIT",
          "More than 10,000 matches; narrow the text or search one chapter. Nothing was changed.",
        );
      ...
      matches.push(
        Object.freeze({ chapter: index, start: found.index, end: pattern.lastIndex, line, column }),
      );
    }
  }
  const result = Object.freeze({
    ...
    matchCase,
    chapter,
    files: freezeFiles(value.files),
    matches: Object.freeze(matches),
  });
  searches.set(result, value);
  return result;
}
...
export function planBookReplacement(search, replacement, matchIndex = null) {
  ...
  if (!original)
    throw bookError("INVALID_SEARCH", "Run a source search before preparing a replacement.");
  bookTextBytes(replacement, L.replacementBytes);
  if (
    matchIndex !== null &&
    (!Number.isInteger(matchIndex) || matchIndex < 0 || matchIndex >= search.matches.length)
  ) {
  ...
  for (const [index, file] of original.files.entries()) {
    const all = groups.get(index) ?? [],
      newline = file.source.match(/\r\n|\r|\n/)?.[0] ?? "\n";
    const insert = lines(replacement).replace(/\n/g, () => newline);
    const insertBytes = bookTextBytes(insert, L.replacementBytes * 2),
      oldBytes = bookTextBytes(file.source, B.chapterBytes);
    ...
    for (const match of edits)
      newBytes +=
        insertBytes - bookTextBytes(file.source.slice(match.start, match.end), B.chapterBytes);
    if (newBytes > B.chapterBytes)
      throw bookError(
        "BOOK_LIMIT",
        "Replacement would exceed the 4 MiB chapter limit. Nothing was changed.",
      );
    total += newBytes + bookTextBytes(file.path, 1024);
    if (total > B.sourceBytes)
      throw bookError(
        "BOOK_LIMIT",
        "Replacement would exceed the 16 MiB book limit. Nothing was changed.",
      );
  ...
  const files = admitted.map(({ file, index, edits, insert, oldBytes, newBytes }) => {
    ...
    let at = 0;
    for (const match of edits) {
      parts.push(file.source.slice(at, match.start), insert);
      at = match.end;
    }
