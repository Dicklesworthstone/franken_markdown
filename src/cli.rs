...
fn print_robot_docs() -> ExitCode {
    emit_stdout(
        "fmd agent guide\n\nCanonical commands:\n  fmd README.md --out README.html\n  fmd README.md --to pdf --out README.pdf\n  fmd README.md --to pdf --pdf-line-numbers --out README.pdf\n  fmd README.md --to pdf --pdf-image images/chart.png=./chart.png --out README.pdf\n  fmd README.md --to pdf --pdf-font body-regular=./Var.ttf --pdf-font-weight 650 --out README.pdf\n  fmd README.md --to pdf --title 'Quarterly Memo' --author 'FMD' --out README.pdf\n  SOURCE_DATE_EPOCH=1700000000 fmd README.md --to pdf --out README.pdf\n  fmd --max-input-bytes 1048576 README.md --out README.html\n  fmd - --out stdin.html < README.md\n  fmd --text '# Hello' --out hello.html\n  fmd --text '# Hello' --out - > piped.html\n  fmd render README.md --to both --out README.html\n  fmd render README.md --to both --out README.html\n  fmd --font serif README.md --out README.html\n  fmd --css my.css README.md --out README.html\n  fmd --title 'Quarterly Memo' --author 'FMD' README.md --to pdf --out README.pdf\n  fmd --allow-html trusted.md --out trusted.html\n  fmd --pdf-line-numbers README.md --to pdf --out README.pdf\n  fmd --max-pdf-image-bytes 1048576 README.md --to pdf --out README.pdf\n  fmd --no-remote-images README.md --to pdf --out README.pdf\n  fmd --max-input-bytes 1048576 README.md --out README.html\n\nDiscovery:\n  fmd capabilities --json   # commands, examples, feature flags, theme, conformance number\n  fmd doctor --json          # subsystem availability, dependency posture, license\n  fmd robot-docs guide       # this file\n  fmd --robot-triage         # one-shot JSON envelope: quick-ref + health + next actions\n\nConfig (native, ~/.config/fmd/config by default; --no-config disables):\n  font=sans|serif\n  dark_mode=auto|disabled\n  custom_css=/path/to.css   (or 'none')\n  page_size=letter          (only 'letter' is honored in v1)\n  margin_top_pt|right_pt|bottom_pt|left_pt=<f32>\n  emoji_strategy=warning|noto_subset|drawn\n    See docs/EMOJI_FALLBACK.md. v1 honors only `warning` (current behavior — .notdef\n    boxes for unrendered emoji and a `missing_glyphs` warning). The other two are\n    accepted for forward compatibility and resolve to `warning` until a curated\n    Noto Sans Symbols subset (or a drawn fallback) ships. Set the key explicitly to\n    declare intent; an undeclared key is the v1 default.\n\nPDF and HTML:\n  PDF: deterministic, byte-stable for fixed input + options. Bundled curated font\n    subsets; real metrics + GPOS kerning + GSUB ligatures; Knuth-Plass line break;\n    selectable text; tagged-PDF structure; compressed streams; measured-column tables;\n    syntax-highlighted code; ASCII / SVG / JPEG asset drawing; frankenmermaid SVG.\n  HTML: self-contained <style>; font subsets as data URLs; local PNG/SVG/JPEG images\n    inlined; safe raw-HTML escaping (use --allow-html for trusted input); responsive\n    tables; polished blockquotes/code blocks; dark mode via prefers-color-scheme.\n\nWarnings are non-fatal. Each surface to stderr (PDF) or a JSON envelope (--json).\n  missing_glyphs: {count, sample} — character(s) had no glyph in the bundled faces.\n  unresolved_image: image dest had no --pdf-image mapping; rendered as alt text.\n  unsupported_image: supplied asset could not be decoded; rendered as alt text.\n  font_weight_ignored_static: --pdf-font-weight pinned but face has no wght axis.\n\nVerify (bead yo83): fmd verify doc.md --to pdf --json emits a stable-schema report\n  (per-page text runs, anchor audit, render warnings, FNV-1a digest, verdict).\n  Exit codes: 0 clean, 1 findings, 64 usage, 66 input, 70 font load. Human mode\n  (no --json) writes caret blocks for each finding pointing back to the source\n  span; --no-color and NO_COLOR/TERM=dumb/CI force Never mode.\n\nImage and font limits:\n  --max-input-bytes <n>           (default 64 MiB) refuse file/stdin/--text above n\n  --max-pdf-image-bytes <n>        (default 32 MiB) cap per-asset decode and remote fetch\n  --no-remote-images               skip remote http(s) image fetches; degrade to alt text\n  --remote-image-timeout-secs <n>  (default 20) per-image remote fetch timeout\n  --pdf-font PATH                  (or SLOT=PATH, repeatable) bundle a custom font face\n  --pdf-font-weight WEIGHT or SLOT=WEIGHT (1..=1000). Variable wght faces instance at the pin; static faces ignore it with warning font_weight_ignored_static. When body-bold is omitted and body-regular is variable, bold instances from that same file at 700. Flags apply to HTML and PDF. The emoji_strategy config key controls how missing-glyph warnings name emoji codepoints vs. other unsupported symbols (see docs/EMOJI_FALLBACK.md).",
    )
}
