...
function factory() {
  ...
  let closes = 0;
  const db = {
    objectStoreNames: { contains: (name) => name === "draft" },
    close() {
      closes++;
    },
...
test("save checks inside one readwrite transaction and acknowledges only transaction completion", async () => {
  const idb = factory(),
    store = await openDraftStore({ indexedDB: idb });
  const doc = { filename: "a.md", source: "captured" },
    pending = store.save(doc, 0);
  doc.source = "changed";
  const tx = idb.transactions[0];
  ...
  assert.equal(tx.writes[0].document.source, "captured");
  tx.commit();
...
test("conflicts and corrupt records abort without issuing any write", async () => {
  for (const [record, expectedCode] of [
    ...
    const idb = factory(),
      store = await openDraftStore({ indexedDB: idb });
    const pending = store.save({ filename: "a.md", source: "new" }, 0);
    const rejected = assert.rejects(pending, code(expectedCode));
    idb.transactions[0].read(record);
    await rejected;
    assert.equal(idb.transactions[0].writes.length, 0);
    store.dispose();
  }
});
test("forget writes an incremented null-document tombstone, not a deletion vulnerable to ABA", async () => {
  const idb = factory(),
    store = await openDraftStore({ indexedDB: idb });
  const pending = store.clear(9),
    tx = idb.transactions[0];
  tx.read({ schemaVersion: 1, version: 9, document: { filename: "a.md", source: "old" } });
  tx.commit();
  assert.deepEqual(await pending, { schemaVersion: 1, version: 10, document: null });
  store.dispose();
});
test("transaction quota/abort errors never return a successful prepared record", async () => {
  const idb = factory(),
    store = await openDraftStore({ indexedDB: idb });
  const pending = store.save({ filename: "a.md", source: "x" }, 0);
  const rejected = assert.rejects(pending, code("STORAGE_FULL")),
    tx = idb.transactions[0];
  tx.read(undefined);
...
test("read-only retrieval validates data and treats only missing records as empty", async () => {
  ...
  tx.read(undefined);
  tx.commit();
  assert.equal((await pending).version, 0);
  const bad = store.read(),
    rejected = assert.rejects(bad, code("CORRUPT_DRAFT"));
  idb.transactions[1].read(null);
  await rejected;
  store.dispose();
});
test("disposal aborts admitted work and rejects subsequent operations", async () => {
  const idb = factory(),
    store = await openDraftStore({ indexedDB: idb });
  const pending = store.read();
  const rejected = assert.rejects(pending, code("STORAGE_CLOSED"));
  store.dispose();
  await rejected;
  await assert.rejects(store.read(), code("STORAGE_CLOSED"));
  assert.equal(idb.closes, 1);
});
test("a transaction that never completes times out and aborts", async () => {
  const idb = factory(),
    store = await openDraftStore({ indexedDB: idb, timeoutMs: 5 });
  await assert.rejects(store.read(), code("STORAGE_TIMEOUT"));
  store.dispose();
});
...
test("malformed input and unavailable storage fail before creating a transaction", async () => {
  ...
  await assert.rejects(
    store.save({ filename: "a.md", source: "\ud800" }, 0),
    code("INVALID_UNICODE"),
  );
  await assert.rejects(store.clear(-1), code("INVALID_REVISION"));
  assert.equal(idb.transactions.length, 0);
  store.dispose();
