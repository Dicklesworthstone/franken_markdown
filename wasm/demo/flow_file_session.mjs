...
const sourceBlob = (source) =>
  new Blob([source], { type: "text/markdown; charset=utf-8", endings: "transparent" });
function handleName(handle) {
  ...
  if (!/\.(md|markdown|txt)$/i.test(name)) {
    throw error(
      "INVALID_FILENAME",
      "Direct file access accepts only .md, .markdown or .txt files.",
    );
  }
  return name;
}
...
function publicError(cause) {
  ...
  const messages = {
    ...
    NotAllowedError: [
      ...
      "File permission was denied. Use Save Markdown as or prepare a download.",
    ],
    SecurityError: [
      "FILE_PERMISSION_DENIED",
      "File access requires a secure page and an explicit user action.",
    ],
    NotFoundError: [
      "FILE_MISSING",
      "The selected file is unavailable. Keep editing or save to a different file.",
    ],
    NoModificationAllowedError: [
      "FILE_LOCKED",
      "Another writer may be using this file. Close it or save to a different file.",
    ],
    QuotaExceededError: [
      "FILE_WRITE_FAILED",
      "There is not enough storage to save the file. Your editor source is retained.",
    ],
  };
  const detail = Object.hasOwn(messages, cause?.name)
    ? messages[cause.name]
    : [
        "FILE_OPERATION_FAILED",
        "The file operation failed. Your editor source is retained; prepare a download as a backup.",
      ];
  return error(...detail);
}
 ...
 */
export function createFileDocumentSession({
  readDocument,
  replaceDocument,
  renameDocument,
  isBlocked = () => false,
  ...
  if (
    ![readDocument, replaceDocument, renameDocument, isBlocked, onState].every(
      (fn) => typeof fn === "function",
    )
  ) {
    throw error(
      "INVALID_ARGUMENT",
      "File sessions require document, rename, state and lifecycle callbacks.",
    );
  }
  let connection = null,
    active = null,
  ...
  let revision = 0,
    identity = 0,
    problem = null,
    message = "No file is connected. Import/download remains available.";
  const read = () => documentSnapshot(readDocument());
  const alive = () => {
    if (disposed) throw error("SESSION_DISPOSED", "File session is disposed.");
  };
  function state() {
    let current = null;
    if (!disposed) {
    ...
    return Object.freeze({
      filename: connection?.diskName ?? null,
      busy: active?.kind ?? null,
      phase: disposed
        ? "disposed"
        : (problem ??
          (!current
  ...
  function announce() {
    if (!disposed) {
      try {
        ...
        /* Observers cannot decide a disk transaction's outcome. */
      }
    }
  }
  function abort(job) {
    if (job?.stream && !job.closing && !job.abortPromise) {
      try {
        job.abortPromise = Promise.resolve(job.stream.abort()).catch(() => {});
      } catch {
  ...
  function fence(job) {
    ...
    if (!unchanged(job))
      throw error(
        "STALE_SOURCE",
        "The editor or document changed during file access. No older source will be installed or committed.",
      );
  }
  async function run(kind, work) {
    alive();
    if (active || isBlocked())
      throw error("FILE_BUSY", "Finish the current file operation or text composition first.");
    const job = {
      kind,
      document: read(),
      revision,
    ...
    active = job;
    message = `${kind} in progress. Source editing remains available.`;
    announce();
    try {
      ...
      if (!job.closing && !unchanged(job)) {
        detail = disposed
          ? error("SESSION_DISPOSED", "File session is disposed.")
          : error(
              "STALE_SOURCE",
              "The editor or document changed during file access. No older source was committed.",
            );
      }
      if (job.closing && !job.verified) {
        detail = error(
          "FILE_SAVE_UNCERTAIN",
          "The file may have changed, but this save could not be verified. Keep your edits; reload explicitly or save to a different file. Do not assume it was saved.",
        );
      }
      if (currentJob(job)) {
  ...
  async function compareDisk(job, handle, expected) {
    ...
    fence(job);
    if (!same(disk, expected))
      throw error(
        "FILE_CHANGED",
        "The file changed on disk. Reload explicitly to use that version, or save your edits to a different file.",
      );
  }
  function install(job, handle, document) {
    ...
    try {
      ...
      if (!same(read(), document))
        throw error(
          "STALE_SOURCE",
          "The host did not install the selected source exactly; no file was connected.",
        );
    } catch (cause) {
      connection = null;
      problem = null;
  ...
  async function load(job, handle, confirm) {
    handleName(handle);
    const document = await readHandle(handle);
    fence(job);
    if (
      typeof confirm !== "function" ||
      (await confirm(Object.freeze({ filename: document.filename }))) !== true
    ) {
      fence(job);
      message = "Open cancelled. The editor and current file connection were retained.";
  ...
  async function writablePermission(handle) {
    if (
      ...
      throw error(
        ...
        "This browser cannot save through this handle. Prepare a Markdown download instead.",
      );
    }
    if ((await handle.requestPermission({ mode: "readwrite" })) !== "granted") {
      throw error(
        "FILE_PERMISSION_DENIED",
        "Write permission was not granted. Your source remains in the editor.",
      );
    }
  }
  async function write(job, handle, expected, adopt) {
    ...
    if (
      !job.stream ||
      !["write", "close", "abort"].every((name) => typeof job.stream[name] === "function")
    ) {
      throw error(
        "FILE_WRITE_FAILED",
        "The browser did not provide a complete writable file stream.",
      );
    }
    await compareDisk(job, handle, expected);
    await job.stream.write(sourceBlob(job.document.source));
    fence(job);
    await compareDisk(job, handle, expected);
    job.closing = true;
    ...
    if (owned && job.associated) {
      connection = { ...connection, source: job.document.source };
      problem = null;
    }
    ...
    if (adopt && current) {
      ...
      try {
        ...
        if (now.source !== job.document.source || now.filename !== saved.filename) {
          throw error(
            "STALE_SOURCE",
            "The saved file was verified, but the editor changed; reopen it to connect.",
          );
        }
        connection = {
          handle,
          source: saved.source,
          localName: saved.filename,
          diskName: saved.filename,
        };
        problem = null;
        revision++;
        identity++;
    ...
    if (!disposed && owned)
      message = current
        ? `Saved and verified ${saved.filename}.`
  ...
  const api = {
    ...
    open(pick, confirm) {
      return run("Open", async (job) => {
        if (typeof pick !== "function")
          throw error("FILE_ACCESS_UNAVAILABLE", "Use the ordinary file input in this browser.");
        const handles = await pick();
        fence(job);
        if (!Array.isArray(handles) || handles.length !== 1)
          throw error("INVALID_FILE_HANDLE", "Choose exactly one Markdown file.");
    ...
    save() {
      return run("Save", async (job) => {
        if (!connection)
          throw error("NO_CONNECTED_FILE", "Use Save Markdown as to choose a file first.");
        if (problem)
          throw error(
            "FILE_CHANGED",
            "Resolve the disk conflict by reloading, or save to a different file.",
          );
        if (job.document.filename !== connection.localName)
          throw error(
            "FILENAME_CHANGED",
            "The source filename changed. Use Save Markdown as rather than overwrite the previously connected file.",
          );
        job.associated = true;
        const handle = connection.handle,
          expected = { filename: connection.diskName, source: connection.source };
        await writablePermission(handle);
        fence(job);
    ...
    saveAs(pick, confirmOverwrite) {
      return run("Save as", async (job) => {
        if (typeof pick !== "function")
          throw error("FILE_ACCESS_UNAVAILABLE", "Prepare a Markdown download in this browser.");
        const handle = await pick(job.document.filename);
        fence(job);
        handleName(handle);
        ...
        if (connection && !associated) {
          if (typeof handle.isSameEntry !== "function")
            throw error(
              "INVALID_FILE_HANDLE",
              "The browser cannot verify that this is a different file.",
            );
          associated = await handle.isSameEntry(connection.handle);
          fence(job);
          if (typeof associated !== "boolean")
            throw error("INVALID_FILE_HANDLE", "Invalid file identity response.");
        }
        job.associated = associated;
        if (associated && problem)
          throw error(
            "FILE_CHANGED",
            "Save as cannot bypass a conflict on the same file. Choose a different file.",
          );
        await writablePermission(handle);
        fence(job);
        const expected = associated
          ? { filename: connection.diskName, source: connection.source }
          : await readHandle(handle);
        fence(job);
        if (expected.source.length > 0) {
          if (
            typeof confirmOverwrite !== "function" ||
            (await confirmOverwrite(Object.freeze({ filename: expected.filename }))) !== true
          ) {
            fence(job);
            message = "Save as cancelled. No source bytes were written.";
