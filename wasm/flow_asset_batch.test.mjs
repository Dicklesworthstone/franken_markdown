...
import {
  FLOW_ASSET_BATCH_BYTES,
  FLOW_ASSET_BATCH_COUNT,
  normalizeAssetBatch,
  packAssetBatch,
} from "./flow_asset_batch.mjs";
import { normalizeFlowRequest, requestWeight, snapshotArguments } from "./flow_worker_protocol.mjs";
import { decodeBatch, facade, NativeDouble, result } from "./tests/flow_asset_batch_fixture.mjs";
...
test("packing retains exact binary views and distinguishes absent and empty payloads", () => {
  ...
  const packed = packAssetBatch([
    result("3"),
    result("1", backing.subarray(1, 5)),
    result("2", new Uint8Array()),
  ]);
  ...
  assert(!Object.hasOwn(values[0], "bytes"));
  assert.equal(values[2].bytes.length, 0);
...
test("pooled Node buffers are copied by view, without their unrelated backing bytes", () => {
  ...
  assert.equal(payload.buffer.byteLength, 3);
  assert.equal(input.byteLength, 3);
  assert.notEqual(payload.buffer, input.buffer);
});

test("lossless u64 identities round-trip without Number coercion", () => {
  const values = [
    { ...result(9007199254740993n), generation: 18446744073709551615n },
...
test("count, per-payload and aggregate admission apply before packed allocation", () => {
  assert.throws(
    () => normalizeAssetBatch(new Array(FLOW_ASSET_BATCH_COUNT + 1)),
  ...
  assert.equal(
    normalizeAssetBatch(four).reduce((n, r) => n + r.bytes.length, 0),
    FLOW_ASSET_BATCH_BYTES,
  );
  assert.throws(
    () => normalizeAssetBatch([...four, result("5", new Uint8Array(1))]),
    code("BUDGET_EXCEEDED"),
  );
  assert.throws(
    () => normalizeAssetBatch([result("1", new Uint8Array(big.length + 1))]),
    code("BUDGET_EXCEEDED"),
  );
});
...
test("the complete batch is validated before any native write", () => {
  ...
  const bad = [
    ...
    [new Array(1), "INVALID_OPTIONS"],
    [null, "INVALID_ARGUMENT"],
  ];
  for (const [input, expected] of bad)
    assert.throws(() => session.provideAssets(input), code(expected));
  assert.equal(raw.calls.length, 0);
  assert.equal(raw.singleCalls, 0);
...
test("batch facade publishes one token, retains getters and never invokes single writes", () => {
  const { raw, session } = facade();
  assert(session.supportsAssetBatches);
  ...
  assert.equal(raw.singleCalls, 0);
  assert.equal(session.source, "authoritative source");
  assert.equal(input.byteLength, 2);
  input.fill(8);
  assert.deepEqual(session.assetBytes("1", "1"), new Uint8Array([1, 2]));
  assert.deepEqual(
    session.pendingAssets().requests.map((r) => r.id),
    ["3"],
  );
  session.dispose();
  assert(raw.freed);
...
test("empty batches are no-ops and legacy binaries refuse rather than emulate atomicity", () => {
  const { raw, session } = facade();
  assert.deepEqual(session.provideAssets([]), session.token);
  assert.equal(raw.calls.length, 0);
  const legacy = new NativeDouble();
  legacy.provideAssetsPacked = undefined;
  const old = facade(legacy).session;
  assert.equal(old.supportsAssetBatches, false);
  for (const input of [[], [result()]])
    assert.throws(() => old.provideAssets(input), code("UNSUPPORTED_WASM_PACKAGE"));
  assert.equal(legacy.singleCalls, 0);
  assert.equal(old.source, "authoritative source");
});

test("late native rejection preserves pending state and permits a retry", () => {
  const { raw, session } = facade();
  raw.rejectBatch = true;
  assert.throws(() => session.provideAssets([result(), result("2")]), code("LAYOUT_ERROR"));
  assert.deepEqual(session.token, { revision: "1", layoutRevision: "1" });
  assert.equal(session.pendingAssets().total, 3);
  raw.rejectBatch = false;
  session.provideAssets([result(), result("2")]);
  assert.equal(session.layoutRevision, "2");
...
test("user iterators and arrays growing through getters cannot defeat the count bound", () => {
  ...
  values[Symbol.iterator] = () => {
    throw new Error("must not call a user iterator");
  };
...
test("worker snapshots isolate every view only after normalization and preserve absent payloads", () => {
  ...
  const args = normalizeFlowRequest("provideAssets", [
    [result("1", input.subarray(1, 3)), result("2"), result("3", new Uint8Array())],
  ]);
