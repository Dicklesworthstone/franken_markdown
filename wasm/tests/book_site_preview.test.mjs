...
import {
  decodeBookSiteArchive,
  parseBookPreview,
  renderBookPreview,
  resolveBookPreviewLink,
} from "../book_site_preview.mjs";
...
function archive(entries, method = 8) {
  ...
  for (const [name, source] of entries) {
    ...
    const a = Buffer.alloc(30),
      b = Buffer.alloc(46);
    a.writeUInt32LE(0x04034b50);
    a.writeUInt16LE(20, 4);
    a.writeUInt16LE(0x800, 6);
    a.writeUInt16LE(method, 8);
    a.writeUInt32LE(crc, 14);
    a.writeUInt32LE(payload.length, 18);
    a.writeUInt32LE(body.length, 22);
    a.writeUInt16LE(nameBytes.length, 26);
    b.writeUInt32LE(0x02014b50);
    b.writeUInt16LE(20, 4);
    b.writeUInt16LE(20, 6);
    b.writeUInt16LE(0x800, 8);
    b.writeUInt16LE(method, 10);
    b.writeUInt32LE(crc, 16);
    b.writeUInt32LE(payload.length, 20);
    b.writeUInt32LE(body.length, 24);
    b.writeUInt16LE(nameBytes.length, 28);
    b.writeUInt32LE(offset, 42);
    local.push(a, nameBytes, payload);
    central.push(b, nameBytes);
    offset += a.length + nameBytes.length + payload.length;
  }
...
function entries(chapters = [chapter, other]) {
  return [
    [
      chapter.page,
      '<!doctype html><html lang="en"><head><style>p{color:red}</style></head><body><h1 id="start">Start 🚀</h1><a href="end.html#end">End</a></body></html>',
    ],
...
const rejects = (bytes, code = "INVALID_BOOK_PREVIEW") =>
  assert.rejects(decodeBookSiteArchive(bytes), { code });
const central = (bytes) => bytes.readUInt32LE(bytes.length - 6);
for (const method of [0, 8])
  test(`method ${method}: exact engine HTML and generated chapter order survive decoding`, async () => {
    const source = entries([other, chapter]),
      parsed = await decodeBookSiteArchive(archive(source, method));
    assert.deepEqual(
      parsed.pages.map((page) => [page.path, page.source, page.title]),
      [
        [other.page, other.source, other.title],
        [chapter.page, chapter.source, chapter.title],
      ],
    );
    assert.equal(parsed.pages[1].html, source[0][1]);
    assert.equal(parsed.pages.length, 2);
    assert(Object.isFrozen(parsed.pages[0]));
    assert.equal(
      parsed.pages.some((page) => page.path === "index.html"),
      false,
    );
  });
test("archive input is captured before asynchronous decompression", async () => {
  const bytes = archive(entries()),
    pending = decodeBookSiteArchive(bytes);
  bytes.fill(0);
...
test("renderer adapter delegates the complete ordered collection and asset options", async () => {
  const files = [{ path: "source.md", source: "untouched" }],
    options = { images: [{ destination: "x.png", bytes: new Uint8Array([1]) }] };
  let calls = 0;
  const result = await renderBookPreview(
    {
      async renderBookSite(f, o) {
        calls++;
        assert.equal(f, files);
        assert.equal(o, options);
        return { bytes: archive(entries()), sourceLength: 77 };
      },
    },
    files,
    options,
  ...
  assert.equal(result.sourceLength, 77);
  assert.equal(parseBookPreview(result.bytes).pages.length, 2);
});
for (const [name, mutate] of [
  ...
  [
    "encrypted flags",
    (b) => {
      b.writeUInt16LE(0x801, central(b) + 8);
      return b;
    },
  ],
  [
    "data descriptor",
    (b) => {
      b.writeUInt16LE(0x808, central(b) + 8);
      return b;
    },
  ],
  [
    "ZIP64 version",
    (b) => {
      b.writeUInt16LE(45, central(b) + 6);
      return b;
    },
  ],
  [
    "extra fields",
    (b) => {
      b.writeUInt16LE(1, central(b) + 30);
      return b;
    },
  ],
  [
  ...
  [
    "different local length",
    (b) => {
      b.writeUInt32LE(3, 22);
      return b;
    },
  ],
  [
    "overlapping local offset",
    (b) => {
      b.writeUInt32LE(1, central(b) + 42);
      return b;
    },
  ],
  [
    ...
    (b) => {
      ...
      b.writeUInt32LE(0, central(b) + 16);
      return b;
    },
  ],
  [
    "central directory gap",
    (b) => {
      b.writeUInt32LE(central(b) + 1, b.length - 6);
      return b;
    },
  ],
...
test("all declared sizes are admitted before any decompression starts", async () => {
  const bytes = archive(entries());
  bytes.writeUInt32LE(9 * 1024 * 1024, 22);
  bytes.writeUInt32LE(9 * 1024 * 1024, central(bytes) + 24);
...
test("expansion beyond admitted size and malformed deflate streams fail closed", async () => {
  ...
  bytes.writeUInt32LE(1, central(bytes) + 24);
  await rejects(bytes);
  const broken = archive(entries());
  broken.fill(0xff, 30 + chapter.page.length, 35 + chapter.page.length);
...
for (const name of [
  ...
  test(`rejects output path ${name}`, async () => {
    const e = entries();
    e[0][0] = name;
    await rejects(archive(e));
  });
...
test("malformed UTF-8 is rejected even with valid sizes and CRC", async () => {
  const e = entries();
  e[0][1] = new Uint8Array([0xc0, 0x80]);
  await rejects(archive(e));
});
test("malformed JSON, schema drift and malformed Unicode metadata are rejected", async () => {
  ...
  e[2][1] = "not json";
  await rejects(archive(e));
  e[2][1] = JSON.stringify({ schema: "future", chapters: [chapter, other] });
  await rejects(archive(e));
...
test("preview wire messages are bounded, validated, and immutable", async () => {
  const value = await decodeBookSiteArchive(archive(entries())),
    bytes = encode(JSON.stringify(value));
  assert.deepEqual(parseBookPreview(bytes), value);
  for (const bad of [
    encode("[]"),
    encode('{"schema":"fmd-book-preview-v1","pages":[null]}'),
    encode(JSON.stringify({ ...value, pages: [value.pages[0], value.pages[0]] })),
  ]) {
    assert.throws(() => parseBookPreview(bad), { code: "INVALID_BOOK_PREVIEW" });
  }
  assert.throws(() => parseBookPreview(new Uint8Array(new SharedArrayBuffer(22))), {
    code: "INVALID_BOOK_PREVIEW",
  });
});
test("navigation uses only engine-attested flat pages and decoded fragments", async () => {
  ...
  for (const href of ["end.html#end", "./end.html#end"])
    assert.deepEqual(resolveBookPreviewLink(preview, chapter.page, href), {
      path: "end.html",
      fragment: "end",
    });
  assert.deepEqual(resolveBookPreviewLink(preview, chapter.page, "#%E4%B8%AD"), {
    path: chapter.page,
    fragment: "中",
  });
...
for (const method of [0, 8])
  test(`method ${method}: current search-enabled sites retain only chapter pages`, async () => {
    const source = entries([other, chapter]);
    source.splice(3, 0, [
      "~fmd-search.html",
      '<!doctype html><script>throw new Error("never execute search here")</script>',
    ]);
    const bytes = archive(source, method);
    const preview = await decodeBookSiteArchive(bytes);
    assert.deepEqual(
      preview.pages.map((page) => page.path),
      [other.page, chapter.page],
    );
    ...
    const output = await renderBookPreview(
      {
        async renderBookSite() {
...
test("all 128 chapters plus the three generated auxiliary files are admitted", async () => {
  const chapters = Array.from({ length: 128 }, (_, i) => ({
    source: `${i}.md`,
    page: `${i}.html`,
    title: `Chapter ${i}`,
  }));
...
test("auxiliary pages cannot masquerade as indexed chapters or bypass integrity checks", async () => {
  ...
  source.push(["~fmd-search.html", "Search"]);
  source[2][1] = JSON.stringify({
    schema: "fmd-book-search-index-v1",
    chapters: [chapter, { ...other, page: "~fmd-search.html" }],
  });
  await rejects(archive(source));
  const extra = entries();
  extra.push(["unindexed.html", "Not attested"]);
  await rejects(archive(extra));
  const duplicate = entries();
  duplicate.push(["~fmd-search.html", "One"], ["~fmd-search.html", "Two"]);
  await rejects(archive(duplicate));
  const malformed = entries();
  malformed.push(["~fmd-search.html", new Uint8Array([0xc0, 0x80])]);
  await rejects(archive(malformed));
  const preview = await decodeBookSiteArchive(archive(entries()));
  assert.throws(
    () =>
      parseBookPreview(
        encode(
          JSON.stringify({
            ...preview,
            pages: [{ ...preview.pages[0], path: "~fmd-search.html" }],
          }),
        ),
      ),
    { code: "INVALID_BOOK_PREVIEW" },
