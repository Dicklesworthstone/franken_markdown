...
export function createPreviewExport(readState) {
  ...
  return async function exportDocument(format, options, source) {
    ...
    if (
      ...
      source !== before.desiredSource
    ) {
      fail(
        "STALE_REVISION",
        "export requires the current editor source to be applied successfully",
      );
    }
    if (before.layoutPending)
      fail("STALE_LAYOUT", "export is awaiting the requested preview typography");
    if (before.imagesBusy)
      fail("ASSET_BUSY", "finish the current authorized image batch before exporting");
    const expected = Object.freeze({ ...current.token });
    if (!same(expected, before.frame))
      fail("STALE_LAYOUT", "export does not match the displayed document");
    const ticket = {};
    job = ticket;
    try {
