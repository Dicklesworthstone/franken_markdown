...
const MIB = 1024 * 1024;
export const BOOK_WORKBENCH_LIMITS = Object.freeze({ chapters: 128, chapterBytes: 4 * MIB,
...
function chapters(value) {
  ...
  return Array.from(value, file => {
    ...
    paths.add(path); total += bookTextBytes(file.source, L.chapterBytes) + bookTextBytes(path, 1024);
...
export function createBookCollection() {
  ...
  return Object.freeze({
    ...
    append(batch) {
      alive(); const next = chapters([...sources(), ...batch.chapters]);
    ...
    snapshot() {
      alive(); const ordered = chapters(sources());
      if (!ordered.length) throw bookError("EMPTY_BOOK", "Add at least one chapter before exporting.");
      return prepareBookInput(ordered, { ...settings(options), images });
    },
    chapterDownload(index) {
      alive(); const file = files[indexOf(index, files.length)]; bookTextBytes(file.source, L.chapterBytes);
      let filename = `chapter-${index + 1}.md`;
      try { const path = bookPath(file.path); if (/\.(md|markdown)$/i.test(path)) filename = path.split("/").at(-1); }
 ...
 */
export async function readBookFiles(selected, { folder = false } = {}) {
  ...
  for (const file of input) {
    ...
    const kind = /\.(md|markdown)$/i.test(path) ? "chapter" : /\.(png|jpe?g|svg)$/i.test(path) ? "image" : null;
    if (!kind) { ignored++; continue; }
    bookPath(path); localFile(file, kind === "chapter" ? L.chapterBytes : L.imageBytes);
    if (kind === "chapter") { sourceBytes += file.size + bookTextBytes(path, 1024); chapterCount++; }
    else { imageBytes += file.size; imageCount++; }
    admitted.push({ file, path, kind });
  }
  if (chapterCount > L.chapters || imageCount > L.images || sourceBytes > L.sourceBytes || imageBytes > L.totalImageBytes) {
    throw bookError("BOOK_LIMIT", "The selected collection exceeds workbench source or image limits.");
  }
  const result = { chapters: [], images: [], ignored };
  for (const { file, path, kind } of admitted) {
    ...
    if (kind === "image") result.images.push({ destination: path, bytes: new Uint8Array(bytes) });
      ...
      catch { throw bookError("INVALID_UNICODE", "A chapter is not UTF-8; no files were imported."); }
      result.chapters.push({ path, source });
    }
  }
  chapters(result.chapters); imageList(result.images); return result;
}
