...
test("inspection modules, guide and their relative imports are declared for publication", () => {
  for (const path of ["book_inspection.mjs", "demo/book_inspection_controls.mjs", "INSPECTION.md"])
    assert(manifest.files.includes(path), path);
  for (const path of [
    "book_inspection.mjs",
    "demo/book_inspection_controls.mjs",
    "book_worker_entry.js",
  ]) {
...
test("both real package assemblers copy the inspection core, controller and guide", () => {
  for (const path of ["scripts/check-wasm-package.sh", "scripts/dsr-wasm-package.sh"]) {
    ...
    const root = loops.find((v) => v[2].startsWith("$file")),
      demo = loops.find((v) => v[2].startsWith("demo/"));
    for (const name of ["book_inspection.mjs", "INSPECTION.md"])
      assert(root?.[1].split(/\s+/).includes(name), `${path}: ${name}`);
...
test("actual publisher and worker entry retain previous features and attach the source-only inspector", () => {
  const entry = read("wasm/demo/book.js"),
    worker = read("wasm/book_worker_entry.js");
  assert(entry.includes("createBookInspectionPanel(document)"));
  assert(entry.includes("maxOutputBytes: 2 * 1024 * 1024"));
  assert(
    entry.includes(
      "inspection?.dispose(); search?.dispose(); preview?.dispose(); library?.dispose(); controls.dispose();",
    ),
  );
  for (const name of [
    "createBookSearchControls",
    "createBookPreviewControls",
    "createBookLibraryControls",
  ])
    assert(entry.includes(name));
  assert(worker.includes("inspectBook: files => inspectBook(documents, files)"));
  assert(worker.includes('from "./franken_markdown.js"'));
