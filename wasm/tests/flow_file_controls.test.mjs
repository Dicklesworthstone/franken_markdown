...
class Element extends EventTarget {
  constructor(tag, document) {
    ...
    this._value = "";
    this._text = "";
    this.disabled = this.readOnly = false;
    this.selectionStart = this.selectionEnd = 0;
    this.selectionDirection = "none";
  }
  set value(value) {
    this._value =
      this.tagName === "textarea" ? String(value).replace(/\r\n?/g, "\n") : String(value);
  }
  get value() {
    return this._value;
  }
...
function setup({ supported = true } = {}) {
  const root = new Document(),
    ...
    urls = new Set();
  const add = (id, tag = "button") => {
    const el = root.createElement(tag);
    el.id = id;
    root.append(el);
    return el;
  };
  const source = add("source", "textarea"),
    filename = add("source-filename", "input");
  source.value = "# Initial\n";
  filename.value = "draft.md";
  const open = add("open-markdown", "input"),
    prepare = add("prepare-markdown"),
    download = add("source-download", "a"),
    status = add("source-status", "p");
  let images = true,
    replacements = 0,
    confirmation = true,
    picked = new MemoryFileHandle(),
    saved = new MemoryFileHandle("new.md", "");
  let pickerCalls = 0,
    savePickerCalls = 0;
  ...
  if (supported)
    Object.assign(window, {
      ...
      showSaveFilePicker(options) {
        savePickerCalls++;
        assert.ok(options.suggestedName);
        return Promise.resolve(saved);
      },
    });
  const controls = createSourceControls({
    sourceEditor: source,
    filename,
    open,
    prepare,
    ...
    onReplace() {
      images = false;
      replacements++;
      source.dispatchEvent(new Event("fmd-document-replaced"));
    },
  });
  const editing = createSourceEditingControls({
    sourceEditor: source,
    undo: add("source-undo"),
    redo: add("source-redo"),
    query: add("source-find", "input"),
    ignoreCase: add("source-find-insensitive", "input"),
    previous: add("source-find-previous"),
    next: add("source-find-next"),
    replacement: add("source-replacement", "input"),
    replace: add("source-replace"),
    replaceAll: add("source-replace-all"),
    status: add("source-edit-status", "p"),
  });
...
async function settled(h) {
  for (let i = 0; i < 100 && h.files.state.busy; i++)
    await new Promise((resolve) => setImmediate(resolve));
  assert.equal(h.files.state.busy, null);
}
function key(window, values = {}) {
  const event = new Event("keydown", { cancelable: true });
  Object.assign(event, { key: "s", ctrlKey: true, ...values });
...
test("native panel is labeled, mounted beside source downloads and reused after suspension", () => {
  ...
  const panel = h.el("file-controls");
  h.suspend();
  h.resume();
  assert.equal(h.el("file-controls"), panel);
  assert.equal(h.files.state.filename, null);
  h.dispose();
});
...
test("unsupported browser retains ordinary source I/O and shortcut prepares explicit download", async () => {
  ...
  assert.equal(h.el("file-save-as").disabled, true);
  ...
  assert.equal(h.files.state.phase, "unlinked");
...
test("actual source controls preserve imported BOM/CRLF through direct file save", async () => {
  const h = setup(),
    file = new MemoryFileHandle("unicode.md", "\uFEFF# A\r\n日本語\r");
  h.pick(file);
  await h.files.open();
  assert.equal(h.source.value, "\uFEFF# A\n日本語\n");
  const bytes = file.entry.bytes.slice();
  await h.files.save();
  assert.deepEqual(file.entry.bytes, bytes);
...
test("saving leaves undo/redo and document identity intact", async () => {
  ...
  await h.files.save();
  assert.equal(h.replacements, replacements);
  assert.equal(h.files.state.phase, "saved");
  assert.equal(h.editing.undo(), true);
  assert.equal(h.source.value, "# Disk\n");
  assert.equal(h.files.state.phase, "edited");
  assert.equal(h.editing.redo(), true);
  assert.equal(h.files.state.phase, "saved");
  h.dispose();
});
...
test("Save as renames source without revoking image authority or resetting history", async () => {
  const h = setup();
  h.edit("# New document");
  await h.files.saveAs();
  assert.equal(h.filename.value, "new.md");
  assert.equal(h.replacements, 0);
  assert.equal(h.images, true);
  assert.equal(h.editing.undo(), true);
  assert.equal(h.source.value, "# Initial\n");
  assert.equal(h.files.state.phase, "edited");
  h.dispose();
});
...
test("conflict workflow retains edits, offers native Blob recovery download and explicit reload", async () => {
  ...
  await h.files.open();
  h.edit("# My recovery");
  h.picked.source = "# External";
  await assert.rejects(h.files.save(), { code: "FILE_CHANGED" });
  assert.equal(h.el("file-save").disabled, true);
  assert.match(h.el("file-status").textContent, /Disk conflict/);
  h.files.backup();
  assert.equal(await (await fetch(h.download.href)).text(), "# My recovery");
  h.el("file-keep").dispatchEvent(new Event("click"));
  assert.equal(h.root.activeElement, h.source);
  assert.equal(h.files.state.phase, "conflict");
  h.confirm(false);
  assert.equal((await h.files.reload()).opened, false);
  assert.equal(h.source.value, "# My recovery");
  h.confirm(true);
  await h.files.reload();
  assert.equal(h.source.value, "# External");
  assert.equal(h.files.state.phase, "saved");
  assert.equal(h.urls.size, 0);
  h.dispose();
});
...
test("composition disables file commands and does not steal IME shortcuts", async () => {
  const h = setup();
  await h.files.open();
  h.source.dispatchEvent(new Event("compositionstart"));
  assert.equal(h.el("file-open").disabled, true);
  assert.equal(key(h.window).defaultPrevented, false);
  await assert.rejects(h.files.save(), { code: "FILE_BUSY" });
  h.source.dispatchEvent(new Event("compositionend"));
  assert.equal(h.el("file-open").disabled, false);
...
test("draft restoration and ordinary import detach file ownership without losing source", async () => {
  ...
  assert.equal(h.files.state.filename, null);
  assert.equal(h.source.value, "recovery");
  await h.files.open();
  await h.controls.openFile(new File(["imported"], "imported.md"));
  assert.equal(h.files.state.filename, null);
  assert.equal(h.source.value, "imported");
  h.dispose();
});
...
test("pending ordinary import blocks native access without disabling it permanently", async () => {
  ...
  await assert.rejects(h.files.open(), { code: "FILE_BUSY" });
  assert.equal(h.pickerCalls, 0);
  held.resolve(new TextEncoder().encode("imported").buffer);
  await opening;
  await h.files.open();
...
test("button failure shows bounded diagnostic, not native exception text", async () => {
  ...
  h.el("file-open").dispatchEvent(new Event("click"));
  await settled(h);
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(h.el("file-status").textContent.includes("/private/user/path"), false);
  assert.match(h.el("file-status").textContent, /FILE_OPERATION_FAILED/);
  h.dispose();
});

test("file-name text is inserted as text, not interpreted as HTML", async () => {
  const h = setup();
  h.pick(new MemoryFileHandle("&amp;.md", "# Good"));
  await h.files.open();
  assert.match(h.el("file-status").textContent, /&amp;\.md/);
  h.dispose();
});
