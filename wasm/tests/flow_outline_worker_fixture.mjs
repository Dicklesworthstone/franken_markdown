...
const endpoint = {
  ...
  addEventListener(type, listener) {
    const adapted = (data) => listener({ data });
    listeners.set(listener, adapted);
    parentPort.on(type, adapted);
  },
...
installFlowWorker(endpoint, async (source, layout) =>
  withGlyphOutlines(
    Object.freeze({
      disposed: false,
    ...
    (id, glyphs) => {
      if (id === "0")
        throw JSON.stringify({ code: "UNKNOWN_FONT", message: "fixture unknown font" });
