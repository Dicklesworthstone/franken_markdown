...
const run = (startByte, endByte, extra = {}, link = null) => ({
  startByte,
  endByte,
  style: style(extra),
  link,
...
const linked = () =>
  inline("paragraph", "one TWO three", [
...
export async function runTests() {
  ...
  async function view(roots, options = {}) {
    ...
    const reader = new FlowReaderView(container, options);
  ...
  await test("semantic inline tags survive headings, quotes, task markers and table cells", async () => {
    const { container } = await view([
      ...
      node("table-header-row", "head", {
        children: [inline("table-header-cell", "head", [run(0, 4, { italic: true })])],
      }),
      node("table-row", "cell", {
        children: [inline("table-cell", "cell", [run(0, 4, { code: true })])],
      }),
    ]);
  ...
  await test("native selection crosses Unicode and style boundaries without losing text", async () => {
    const { reader, snapshot } = await view([
      inline("paragraph", "A é😀z tail", [
        run(2, 4, { bold: true }),
        run(4, 8, { italic: true }),
        run(8, 9, { code: true }),
      ]),
  ...
  await test("one keyboard link spans adjacent bold italic code and strike fragments", async () => {
    const calls = [],
      { container, snapshot } = await view([linked()], { onLink: (event) => calls.push(event) });
    const links = container.querySelectorAll('[role="link"]');
    equal(links.length, 1);
    const anchor = links[0];
    equal(anchor.tabIndex, 0);
    equal(anchor.textContent, "one TWO three");
    assert(
      anchor.querySelector("strong") &&
        anchor.querySelector("em code") &&
        anchor.querySelector("s"),
    );
    anchor.focus();
    equal(document.activeElement, anchor);
    click(anchor.querySelector("code"));
    enter(anchor);
    equal(calls.length, 2);
    equal(calls[0].target, "#section");
    equal(calls[0].startUtf16, 0);
    equal(calls[0].endUtf16, 13);
    equal(calls[0].location.revision, snapshot.token.revision);
    equal(calls[0].location.nodeIndex, 0);
    ...
    enter(anchor, { repeat: true });
    enter(anchor, { key: " " });
    equal(calls.length, 2);
  });
  await test("unsafe targets and absent callbacks remain inert without URL attributes", async () => {
    const calls = [],
      blocked = inline("paragraph", "bad", [
        run(0, 3, { bold: true }, target("javascript:bad", null)),
      ]);
    const { container } = await view([blocked], { onLink: (event) => calls.push(event) });
    equal(container.querySelectorAll('[role="link"]').length, 0);
    click(container.querySelector("strong"));
    equal(calls.length, 0);
    const inert = await view([linked()]);
    equal(inert.container.querySelectorAll('[role="link"]').length, 0);
    equal(inert.container.textContent, "one TWO three");
    assert(
      !document.querySelector(
        "section [href], section [src], section [onclick], section img, section script",
      ),
    );
  });
  await test("linked image descriptions work with empty alt and never load an image", async () => {
    const calls = [],
      { container, reader, snapshot } = await view(
        [
    ...
    const links = container.querySelectorAll('figure [role="link"]');
    equal(links.length, 2);
    equal(links[0].textContent, "Image: ");
    enter(links[0]);
    equal(calls[0].target, "https://example.com/image");
    equal(calls[0].startUtf16, null);
    equal(calls[0].endUtf16, null);
    equal(reader.selectMatch(snapshot.find("diagram").matches[0]), "diagram");
    equal(document.getSelection().toString(), "diagram");
  ...
  await test("source, layout and disposed-session fences refuse old links and matches", async () => {
    for (const action of ["edit", "reflow", "dispose"]) {
      const calls = [],
        { container, reader, snapshot, session } = await view([linked()], {
          onLink: (event) => calls.push(event),
        });
      const match = snapshot.find("TWO").matches[0];
      if (action === "dispose") session.disposed = true;
      else session[action]();
      click(container.querySelector('[role="link"]'));
      enter(container.querySelector('[role="link"]'));
      equal(calls.length, 0);
      throws(
        () => reader.selectMatch(match),
        action === "edit"
          ? "STALE_REVISION"
          : action === "reflow"
            ? "STALE_LAYOUT"
  ...
  await test("changed, inserted and reordered DOM text cannot alter search Copy", async () => {
    for (const mutate of [
      (el) => {
        el.querySelector("strong").firstChild.data = "changed";
      },
      (el) => {
        el.querySelector("em").before(document.createTextNode("injected"));
      },
      (el) => {
        el.querySelector("strong").after(el.querySelector("s"));
      },
      (el) => {
      ...
      const calls = [],
        { container, reader, snapshot } = await view([linked()], {
          onLink: (event) => calls.push(event),
        });
      const anchor = container.querySelector('[role="link"]');
      mutate(anchor);
      throws(
        () => reader.selectMatch(snapshot.find("one TWO three").matches[0]),
        "READING_DOM_CHANGED",
      );
      click(anchor);
      equal(calls.length, 0);
  ...
  await test("replacement, clear and disposal revoke old link listeners", async () => {
    const calls = [],
      { container, reader } = await view([linked()], { onLink: (event) => calls.push(event) });
    ...
    const next = container.querySelector('[role="link"]');
    reader.clear();
    container.append(next);
    click(next);
    equal(calls.length, 0);
    reader.render(await readFlowDocument(new ReadingSession([linked()])));
    const last = container.querySelector('[role="link"]');
    reader.dispose();
  ...
  await test("rejected snapshot keeps the previous readable DOM and selection", async () => {
    ...
    reader.selectMatch(snapshot.find("TWO").matches[0]);
    const old = container.firstChild;
    throws(() => reader.render({ ...snapshot }), "INVALID_ARGUMENT");
    equal(container.firstChild, old);
    equal(document.getSelection().toString(), "TWO");
    const session = new ReadingSession([linked()]),
      stale = await readFlowDocument(session);
    session.edit();
    throws(() => reader.render(stale), "STALE_REVISION");
    equal(container.firstChild, old);
  });
  ...
  await test("constructor validates callback options without mutating host children", async () => {
    ...
    for (const options of [null, { onLink: null }, { onLink: 3 }, { unknown: true }]) {
      throws(() => new FlowReaderView(container, options), "INVALID_OPTIONS");
      equal(container.textContent, "keep");
    }
  });
  return passed;
}
