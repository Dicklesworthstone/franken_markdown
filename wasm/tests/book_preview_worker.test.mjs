...
class LoopbackWorker extends EventTarget {
    ...
    installBookWorker(
      {
        ...
        postMessage: (data, transfer) => {
          if (!this.terminated)
            this.dispatchEvent(
              Object.assign(new Event("message"), { data: structuredClone(data, { transfer }) }),
            );
        },
      },
      engine,
  ...
  postMessage(data, transfer) {
    ...
    queueMicrotask(() => {
      if (!this.terminated) this.receive({ data: copied });
    });
  }
  terminate() {
...
const payload = () => ({
  bytes: new TextEncoder().encode('{"schema":"fmd-book-preview-v1","pages":[]}'),
  sourceLength: 5,
});
function client(t, engine, options = {}) {
  const workers = [],
    instance = createBookWorkerClient({
      ...options,
      workerFactory: () => {
        const worker = new LoopbackWorker(engine);
        workers.push(worker);
        return worker;
      },
    });
...
test("preview has a distinct byte envelope and transfers only private input/output copies", async (t) => {
  const images = [{ destination: "a.png", bytes: new Uint8Array([1, 2, 3]) }];
  let captured;
  const { instance, workers } = client(t, {
    async renderBookPreview(f, options) {
      captured = { f, options };
      return payload();
    },
  });
  const pending = instance.render(files, "preview", { images });
  images[0].bytes[0] = 9;
  const result = await pending;
  assert.equal(result.format, "book-preview");
  assert.equal(result.mimeType, "application/json");
  assert.equal(result.extension, "json");
  assert.deepEqual(captured.f, files);
  assert.equal(captured.options.images[0].bytes[0], 1);
  assert.equal(images[0].bytes.length, 3);
...
for (const [format, method, mime] of [
  ...
  test(`existing ${format} exports still use the original engine method and MIME`, async (t) => {
    ...
    const result = await instance.render(files, format);
...
test("cancelling a separate preview client does not cancel an in-flight publication", async (t) => {
  const a = gate(),
    b = gate(),
    preview = client(t, { renderBookPreview: () => a.promise }),
    publication = client(t, { renderBookPdf: () => b.promise });
  const p = preview.instance.render(files, "preview"),
    q = publication.instance.render(files, "pdf");
  await tick();
  const rejected = assert.rejects(p, { code: "EXPORT_CANCELLED" });
  preview.instance.cancel();
  await rejected;
  assert(preview.workers[0].terminated);
  assert(!publication.workers[0].terminated);
  assert(publication.instance.busy);
  a.resolve(payload());
  b.resolve(payload());
  assert.equal((await q).mimeType, "application/pdf");
});
test("preview limits and failures terminate the job and leave the client reusable", async (t) => {
  let fail = true;
  const { instance, workers } = client(t, {
    renderBookPreview() {
      if (fail) throw Object.assign(new Error("too large"), { code: "PREVIEW_LIMIT" });
      return payload();
    },
  });
  await assert.rejects(instance.render(files, "preview"), { code: "PREVIEW_LIMIT" });
  assert(workers[0].terminated);
  fail = false;
  assert.equal((await instance.render(files, "preview")).format, "book-preview");
});
test("preview still observes output ceilings, cancellation and invalid-input validation", async (t) => {
  ...
  controller.abort();
  await assert.rejects(h.instance.render(files, "preview", {}, { signal: controller.signal }), {
    code: "EXPORT_CANCELLED",
  });
  await assert.rejects(
    h.instance.render([{ path: "a.md", source: "bad\ud800" }], "preview"),
    /Unicode/,
  );
