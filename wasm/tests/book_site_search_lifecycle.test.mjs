...
function fixture(body, title = "Guide") {
  ...
  elements["search-data"].textContent = JSON.stringify({
    ...
    chapters: [
      {
        ...
        index: {
          ...
          entries: [{ kind: "paragraph", text: body, anchor: "details" }],
        },
      },
    ],
  });
  const document = {
    elements,
    location: { href: "file:///book/search.html" },
...
test("recorded source offsets make snippet extraction independent of document length", () => {
  ...
  const prefix = "İ🙂".repeat(100000),
    content = prefix + "target" + "x".repeat(500000);
...
test("production snippets center normalized phrase hits without reprocessing whole entries", async () => {
  ...
  assert.ok(
    f.context.largest <= 4097,
    `renormalized ${f.context.largest} characters while rendering`,
  );
  controller.dispose();
});
