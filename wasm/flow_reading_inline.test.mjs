...
const style = (value = {}) => ({ bold: false, italic: false, code: false, strikethrough: false, ...value });
const run = (startByte, endByte, flags = {}, link = null) => ({ startByte, endByte, style: style(flags), link });
const link = (target, activeTarget = target) => ({ target, activeTarget });
const leaf = (text, inlineRuns = [], extra = {}) => ({ role: "paragraph", text, inlineRuns,
  bounds: { x: 0, y: 0, width: 100, height: 20 }, enclosingSourceSpan: { startByte: 99, endByte: 199 },
  children: [], ...extra });
const sessionFor = nodes => ({ disposed: false, token: { revision: "1", layoutRevision: "1" },
  readingOrder({ offset, limit }) {
    return { schemaVersion: 1, ...this.token, offset, total: nodes.length,
      nextOffset: offset + limit < nodes.length ? offset + limit : null, nodes: nodes.slice(offset, offset + limit) };
  } });
const read = (nodes, options) => readFlowDocument(sessionFor(nodes), options);
const rejects = (nodes, code = "INVALID_READING_DATA", options) => assert.rejects(read(nodes, options), { code });

test("Unicode reading ranges convert exactly, independently of enclosing source spans", async () => {
  const source = leaf("A é😀z", [run(0, 2), run(2, 4, { bold: true }), run(4, 8, { italic: true, code: true }), run(8, 9)]);
  const document = await read([source]), node = document.nodes[0];
  assert.deepEqual(node.inlineRuns.map(r => [r.startUtf16, r.endUtf16]), [[0, 2], [2, 3], [3, 5], [5, 6]]);
  ...
  assert.ok(Object.isFrozen(node.inlineRuns) && Object.isFrozen(node.inlineRuns[1]) && Object.isFrozen(node.inlineRuns[1].style));
});
...
test("safe links, intentionally inert links, and blocked targets are retained immutably", async () => {
  const nodes = ["#part", "../guide.md", "https://example.com", "HTTP://example.com", "mailto:a@b", "//example.com/x", "/a:b"]
  ...
  nodes.push(leaf("trim", [run(0, 4, {}, link("\u0085 https://example.com \n", "https://example.com"))]));
...
test("claimed activation cannot authorize schemes, controls, or a substituted target", async () => {
  for (const target of ["javascript:x", "data:x", "file:///x", "vbscript:x", "java\tscript:x", "https:\\evil", "x\u007fy", "x\u009fy", "httpſ:x", ""]) {
...
test("ranges reject splits, bounds, overlaps, inversions, empties, and missing styles or links", async () => {
  for (const runs of [[run(1, 2)], [run(0, 3)], [run(2, 7)], [run(0, 2), run(1, 6)],
    [run(3, 2)], [run(0, 0)], [run(-1, 2)], [run(0.5, 2)], [run(0, Infinity)],
    [{ startByte: 0, endByte: 2 }], [{ ...run(0, 2), link: undefined }],
    [run(0, 2, { bold: "yes" })], [run(0, 2, { color: "red" })]]) {
...
test("all page metadata counts toward independently lowerable retention budgets", async () => {
  ...
  await rejects([leaf("x", [run(0, 1, {}, link("#x"))])], "READING_LIMIT", { limits: { maxLinkUnits: 3 } });
  assert.equal((await read([leaf("x", [run(0, 1, {}, link("#x"))])], { limits: { maxLinkUnits: 4 } })).linkUnits, 4);
  await rejects([leaf("", [], { role: "image", imageLink: link("abc", null) })], "READING_LIMIT", { limits: { maxLinkUnits: 2 } });
...
test("legacy plain pages remain accepted without inventing formatting or links", async () => {
  const old = leaf("legacy"); delete old.inlineRuns;
...
test("formatting never weakens source/layout identity fences", async () => {
  ...
  const document = await readFlowDocument(session), match = document.find("one two").matches[0];
