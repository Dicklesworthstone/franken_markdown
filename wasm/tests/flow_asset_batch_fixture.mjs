...
const error = (code) => {
  throw JSON.stringify({ code, message: code });
};
export const result = (requestId = "1", bytes) => ({
  requestId,
  generation: "1",
  width: 20,
  height: 10,
...
export function decodeBatch(metadata, payload) {
  ...
  const body = metadata.slice(magic.length);
  let offset = 0;
  const values = body
    ? body.split("\n").map((row) => {
        ...
        if (length !== "-") {
          ...
          value.bytes = payload.slice(offset, end);
          offset = end;
        }
        return value;
      })
...
export class NativeDouble {
  ...
  pending = new Set(["1", "2", "3"]);
  accepted = new Map();
  calls = [];
  singleCalls = 0;
  freed = false;
  rejectBatch = false;
  constructor(source = "authoritative source") {
    this.source = source;
  }
  provideAssetsPacked(metadata, payload) {
    this.calls.push({ metadata, payload: new Uint8Array(payload) });
    const results = decodeBatch(metadata, payload),
      remaining = new Set(this.pending);
