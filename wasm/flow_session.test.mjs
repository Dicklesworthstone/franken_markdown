...
function transport() {
  ...
  const tick = () => {
    b.layoutRevision = (BigInt(b.layoutRevision) + 1n).toString();
  };
  const rev = (r) => {
    if (r !== b.revision) throw '{"code":"STALE_REVISION","message":"stale"}';
  };
  const envelope = () => ({
    schemaVersion: 1,
    revision: b.revision,
    layoutRevision: b.layoutRevision,
  });
  const page = (offset, limit, key, count) =>
    JSON.stringify({
      ...envelope(),
      offset,
      total: count,
      nextOffset: offset + Math.min(limit, count - offset) < count ? offset + limit : null,
      [key]: Array.from({ length: Math.min(limit, count - offset) }, (_, i) => ({
        index: offset + i,
      })),
    });
  b.snapshotJson = (...args) => {
    log("snapshotJson", args);
    return page(args[2], args[3], "items", b.count);
  };
  b.readingJson = (...args) => {
    log("readingJson", args);
    return page(args[2], args[3], "nodes", 1);
  };
  b.pendingAssetsJson = (...args) => {
    log("pendingAssetsJson", args);
    return page(args[1], args[2], "requests", 1);
  };
  for (const name of ["editUtf16", "editBytes", "replaceSource"])
    b[name] = (...args) => {
      log(name, args);
      rev(args[0]);
      if (b.reject) throw '{"code":"LAYOUT_ERROR","message":"unsupported glyph"}';
      b.revision = (BigInt(b.revision) + 1n).toString();
      tick();
    };
  b.reflow = (...args) => {
    log("reflow", args);
    rev(args[0]);
    if (b.reject) throw '{"code":"LAYOUT_ERROR","message":"failed"}';
    tick();
  };
  b.provideAsset = (...args) => {
    log("provideAsset", args);
    rev(args[1]);
    tick();
  };
  b.reloadAssets = (r) => {
  ...
  b.hitTestJson = (...args) => {
    log("hitTestJson", args);
    return JSON.stringify({
      ...envelope(),
      hit: null,
      linkTarget: 'https://example.test/"quoted"',
    });
  };
  b.selectItemJson = (...args) => {
    log("selectItemJson", args);
    return JSON.stringify({ ...envelope(), text: "café", rectangles: [] });
  };
  b.copySource = (...args) => {
    log("copySource", args);
    return "**bold**";
...
test("creation validates source, options and positive f32 layout before initialization", () => {
  assert.deepEqual(validateCreation("hello").layout, {
    viewportWidth: 800,
    bodySize: 14,
    codeSize: 13,
    lineHeight: 20,
...
test("Unicode ingress rejects lone surrogates without replacing valid scalar pairs", () => {
  ...
  for (const text of ["\ud800", "\udfff", "a\ud800x", "\ud800\ud800"])
    assert.throws(() => sourceText(text), errorCode("INVALID_UNICODE"));
  const raw = transport(),
    api = createFlowAdapter(raw);
  assert.throws(
    () => api.edit(0, 0, "\ud800", { expectedRevision: "1" }),
...
test("source budget counts UTF-8 bytes rather than UTF-16 units", () => {
  ...
  assert.throws(
    () => sourceText("é".repeat(FLOW_SOURCE_LIMIT / 2 + 1)),
...
test("u64 revisions preserve all bits and reject Number or coercible strings", () => {
  for (const value of ["0", "9007199254740993", "18446744073709551615", 18446744073709551615n])
    assert.equal(identity(value), String(value));
  for (const value of [
    1,
    9007199254740992,
    -1n,
...
test("failed reflow leaves JS layout and native revision unchanged", () => {
  ...
  assert.deepEqual(api.layoutOptions, before);
  assert.equal(api.layoutRevision, "1");
  raw.reject = false;
  api.reflow({ viewportWidth: 100 }, api.token);
  assert.equal(api.layoutOptions.viewportWidth, 100);
...
test("pagination carries one frozen snapshot token and refuses mixed generations", () => {
  const raw = transport(),
    api = createFlowAdapter(raw);
  assert.deepEqual(
    [...api.pages({ limit: 2 })].flatMap((p) => p.items.map((i) => i.index)),
    [0, 1, 2],
  );
...
test("empty documents yield one terminating page and invalid limits are rejected", () => {
  ...
  for (const limit of [0, -1, 2049, 1.5, "1"])
    assert.throws(() => api.snapshot({ limit }), FlowError);
});
...
test("retrieved font and asset arrays are independently owned", () => {
  const raw = transport(),
    api = createFlowAdapter(raw);
  const font = api.fontBytes("18446744073709551615");
  font[0] = 9;
  const asset = api.assetBytes("1", "1");
  asset[1] = 9;
...
test("WASM errors retain stable codes and unexpected traps retain their cause", () => {
  const raw = transport(),
    api = createFlowAdapter(raw);
  raw.replaceSource = () => {
    throw '{"code":"STALE_REVISION","message":"stale source"}';
  };
  assert.throws(
    () => api.replaceSource("x", { expectedRevision: "1" }),
  ...
  assert.throws(
    () => api.replaceSource("x", { expectedRevision: "1" }),
    (e) => e.code === "WASM_ERROR" && e.cause === trap,
  );
});
...
async function entrypointFixture(classSource) {
  ...
  for (const file of ["flow.js", "flow_session.mjs"])
...
test("public entrypoint constructs the generated persistent class", async () => {
  const { createFlowSession } = await entrypointFixture(
    'export class FmdFlowSession { constructor(source) { this.source = source; this.revision = "1"; this.layoutRevision = "1"; } free() {} }',
  );
  const api = await createFlowSession("hello");
  assert.equal(api.source, "hello");
  api.dispose();
});
test("public entrypoint rejects mismatched old artifacts with an actionable code", async () => {
  ...
  await assert.rejects(
    createFlowSession("hello"),
    (e) => e.name === "FlowError" && e.code === "UNSUPPORTED_WASM_PACKAGE",
  );
});
