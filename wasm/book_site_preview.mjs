...
export const BOOK_PREVIEW_LIMITS = Object.freeze({ archiveBytes: 64 * 1024 * 1024,
...
function ownedBytes(value, maximum) {
  if (!(value instanceof Uint8Array) || Object.prototype.toString.call(value.buffer) !== "[object ArrayBuffer]") throw invalid();
...
function directory(bytes) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const method = u16(at + 10), crc = u32(at + 16), compressed = u32(at + 20), length = u32(at + 24);
    const nameLength = u16(at + 28), local = u32(at + 42);
    if (u16(at + 6) > 20 || u16(at + 8) !== 0x800 || ![0, 8].includes(method)
    ...
    if (u32(local) !== 0x04034b50 || u16(local + 4) !== u16(at + 6) || u16(local + 6) !== 0x800
        || u16(local + 8) !== method || u32(local + 14) !== crc || u32(local + 18) !== compressed
        || u32(local + 22) !== length || u16(local + 26) !== nameLength || u16(local + 28)
        || decode(bytes.subarray(local + 30, local + 30 + nameLength)) !== name) throw invalid();
    const offset = local + 30 + nameLength;
    localEnd = offset + compressed;
    if (localEnd > start || (method === 0 && compressed !== length)) throw invalid();
    entries.push({ name, method, crc, offset, compressed, length }); at += 46 + nameLength;
  }
  if (at !== end || localEnd !== start || !names.has("index.html") || !names.has("search-index.json")) throw invalid();
  return entries;
}
...
async function inflate(bytes, expected) {
  ...
  catch { throw error("PREVIEW_UNAVAILABLE", "This browser cannot decompress book previews. Download a site export instead."); }
  ...
  const output = new Uint8Array(expected); let written = 0, complete = false;
  try {
    for (;;) {
      const part = await reader.read();
      if (part.done) break;
      if (written + part.value.byteLength > expected) throw invalid();
      output.set(part.value, written); written += part.value.byteLength;
    }
    if (written !== expected) throw invalid();
    complete = true; return output;
  } catch { throw invalid(); }
  finally {
    if (!complete) { try { await reader.cancel(); } catch { /* Preserve the admission/decompression error. */ } }
    reader.releaseLock();
  }
}
...
export async function decodeBookSiteArchive(value) {
  ...
  const bytes = new Uint8Array(ownedBytes(value, L.archiveBytes)), entries = directory(bytes), contents = new Map();
...
function checkedPreview(value) {
  if (!value || value.schema !== "fmd-book-preview-v1" || !Array.isArray(value.pages)
      || !value.pages.length || value.pages.length > L.chapters) throw invalid();
  ...
  const pages = Array.from(value.pages, page => {
    ...
    const path = pageName(page.path); text(page.source, 4096); text(page.title, 4096);
    if (!page.source || paths.has(path.toLowerCase()) || sources.has(page.source)) throw invalid();
    paths.add(path.toLowerCase()); sources.add(page.source);
...
export async function renderBookPreview(engine, files, options) {
  ...
  const parts = ['{"schema":"fmd-book-preview-v1","pages":[']; let total = parts[0].length;
  for (const [i, page] of preview.pages.entries()) {
    const part = (i ? "," : "") + JSON.stringify(page);
    total += text(part, L.messageBytes - total - 2); parts.push(part);
  }
  parts.push("]}");
...
export function parseBookPreview(value) {
  ...
  let parsed; try { parsed = JSON.parse(decode(bytes)); } catch { throw invalid(); }
...
export function resolveBookPreviewLink(preview, current, href) {
  ...
  const split = href.indexOf("#"), rawPath = split < 0 ? href : href.slice(0, split);
  const path = rawPath.startsWith("./") ? rawPath.slice(2) : rawPath;
  if (/[\/%?:]/.test(path) || path === "." || path === "..") return null;
  const page = path === "index.html" ? preview.pages[0] : preview.pages.find(item => item.path === (path || current));
  if (!page) return null;
  let fragment;
  try { fragment = split < 0 ? "" : decodeURIComponent(href.slice(split + 1)); } catch { return null; }
