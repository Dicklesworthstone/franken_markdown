...
#[test]
fn render_warnings_quiet_when_svg_image_asset_is_valid() {
    use franken_markdown::{PdfImageAsset, PdfOptions, parse_markdown, render_warnings};

    let svg = br##"
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 40">
  <rect x="4" y="4" width="72" height="32" rx="4" fill="#ffffff" stroke="#94a3b8"/>
  <text x="40" y="25" text-anchor="middle" font-size="12" fill="#1a1a2e">SVG</text>
</svg>
"##;
    let opts = PdfOptions {
        image_assets: vec![PdfImageAsset::new("ok.svg", svg.to_vec())],
        ..PdfOptions::default()
    };

    assert!(render_warnings(&parse_markdown("![x](ok.svg)"), &opts).is_empty());
}

#[test]
fn render_warnings_quiet_when_svg_image_asset_has_preamble() {
    use franken_markdown::{PdfImageAsset, PdfOptions, parse_markdown, render_warnings};

    let svg = br##"<?xml version="1.0" encoding="UTF-8"?>
<!-- generated by an SVG tool -->
<!DOCTYPE svg>
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 40">
  <rect x="4" y="4" width="72" height="32" fill="#ffffff" stroke="#94a3b8"/>
</svg>
"##;
    let opts = PdfOptions {
        image_assets: vec![PdfImageAsset::new("preamble.svg", svg.to_vec())],
        ..PdfOptions::default()
    };

    assert!(
        render_warnings(&parse_markdown("![x](preamble.svg)"), &opts).is_empty(),
        "ordinary SVG preambles should not make a valid asset fall back to alt text"
    );
}

#[test]
fn render_warnings_include_svg_image_text_missing_glyphs() {
    use franken_markdown::{
        PdfImageAsset, PdfOptions, RenderWarning, parse_markdown, render_warnings,
    };
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 40">
  <text x="10" y="25" font-size="14" fill="#1a1a2e">ASCII 😀</text>
</svg>
"##;
    let opts = PdfOptions {
        image_assets: vec![PdfImageAsset::new("glyphs.svg", svg.as_bytes().to_vec())],
        ..PdfOptions::default()
    };
    let warns = render_warnings(&parse_markdown("![x](glyphs.svg)"), &opts);

    assert!(
        warns.iter().any(
            |w| matches!(w, RenderWarning::MissingGlyphs { sample, .. } if sample.contains('😀'))
        ),
