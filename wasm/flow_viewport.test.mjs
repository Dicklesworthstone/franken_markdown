...
import { createFlowAdapter, viewportOptions, validateViewportPage, coveringViewport } from "./flow_session.mjs";
...
const item = index => ({ index, kind: "text", bounds: rect(), effectiveClip: rect(),
  text: "é🙂", fontSize: 14, fontRun: null, colorRole: "text", enclosingSourceSpan: { startByte: 0, endByte: 6 } });
function page(query, patch = {}) {
  return { schemaVersion: 1, ...token, queryKind: "viewport-v1", shapingProfile: "bundled-simple-ltr",
    viewport: query.viewport, totalBounds: rect(0, 0, 800, 2000000), afterIndex: query.afterIndex,
    total: 100000, visitedEntries: 32, nextIndex: null, items: [item(95000)], ...patch };
}
class NativeDouble {
  revision = "1"; layoutRevision = "2"; calls = []; freed = 0;
  constructor(rewrite = value => value) { this.rewrite = rewrite; }
...
test("native viewport is called with canonical tokens and original sparse item IDs", () => {
  const raw = new NativeDouble(), session = createFlowAdapter(raw);
  ...
  assert.deepEqual(raw.calls[0], { ...token, viewport: rect(), afterIndex: 90000, limit: 16, glyphs: true });
});

test("missing native capability remains explicit and disposal is live", () => {
  const raw = { ...token, free() {} }, session = createFlowAdapter(raw);
...
test("stale requests are refused before any native query", () => {
  const raw = new NativeDouble(), session = createFlowAdapter(raw);
  assert.throws(() => session.viewport({ ...options(), token: { revision: "0", layoutRevision: "2" } }), { code: "STALE_REVISION" });
  assert.throws(() => session.viewport({ ...options(), token: { revision: "1", layoutRevision: "1" } }), { code: "STALE_LAYOUT" });
...
test("bad viewport requests never reach the native boundary", () => {
  const raw = new NativeDouble(), session = createFlowAdapter(raw);
  for (const value of [
    {}, { ...options(), extra: true }, { ...options(), afterIndex: -1 },
    { ...options(), limit: 0 }, { ...options(), limit: 2049 }, { ...options(), glyphs: 1 },
    { ...options(), viewport: rect(0, NaN) }, { ...options(), viewport: rect(0, 0, -1e-100) },
    { ...options(), viewport: rect(Infinity) }, { ...options(), viewport: rect(1e13) },
...
test("cursor pagination accepts gaps but never duplicate, reordered or fabricated indices", () => {
  ...
  for (const patch of [
    { items: [item(99)] }, { items: [item(120), item(120)], nextIndex: 121 },
    { items: [item(900), item(120)], nextIndex: 121 }, { items: [item(100000)] },
    { items: [item(120)], nextIndex: 121 }, { items: [item(120), item(900)], nextIndex: 999 },
    { items: [], nextIndex: 100 }, { items: [item(120), item(900), item(1000)] },
  ]) assert.throws(() => validateViewportPage({ ...valid, ...patch }, q, token), { code: "INVALID_WASM_RESPONSE" });
});

test("empty and final pages have a truthful null continuation", () => {
  const q = viewportOptions({ ...options(), afterIndex: 100000 });
  assert.deepEqual(validateViewportPage(page(q, { items: [], visitedEntries: 0 }), q, token).items, []);
  const empty = viewportOptions({ viewport: rect(0, 0, 0, 20) });
  assert.deepEqual(validateViewportPage(page(empty, { items: [], visitedEntries: 0 }), empty, token).items, []);
});

test("bad schemas, geometry, clips and work receipts are refused atomically", () => {
  const q = viewportOptions(options());
  for (const patch of [
    { queryKind: "snapshot" }, { shapingProfile: "unknown" }, { schemaVersion: 2 },
    { revision: "7" }, { layoutRevision: "7" }, { afterIndex: 1 },
    { total: 1000001 }, { total: -1 }, { visitedEntries: -1 }, { visitedEntries: 100001 },
  ...
  ]) assert.throws(() => validateViewportPage(page(q, patch), q, token), { code: "INVALID_WASM_RESPONSE" });
});
