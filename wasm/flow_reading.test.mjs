...
import { deferred, node, ReadingSession } from "./tests/flow_reading_fixtures.mjs";
...
test("collects multiple real-sized pages under one token for synchronous and async sessions", async () => {
  for (const asyncMode of [false, true]) {
    const session = new ReadingSession(
      Array.from({ length: 520 }, (_, i) => node("paragraph", `node ${i}`)),
    );
    if (asyncMode) {
      const read = session.readingOrder.bind(session);
      session.readingOrder = async (options) => read(options);
    }
    const doc = await readFlowDocument(session);
    assert.equal(doc.nodes.length, 520);
    assert.equal(doc.roots.length, 520);
    assert.deepEqual(
      session.calls.map((c) => c.offset),
      [0, 256, 512],
    );
    assert(session.calls.every((c) => c.token.revision === "1" && c.signal === undefined));
    assert.equal(doc.nodes[519].index, 519);
    assert(Object.isFrozen(doc.nodes[0].bounds));
    session.roots[0].text = "changed";
    assert.equal(doc.nodes[0].text, "node 0");
...
test("semantic leaves are searched once instead of duplicated container transcripts", async () => {
  ...
  const doc = await readFlowDocument(session);
  assert.equal(doc.headings[0].level, 2);
  assert.equal(doc.text, "Guide\n\nA\n\nB\n\nEntry");
  assert.equal(doc.find("A").matches.length, 1);
  assert.equal(doc.find("Entry").matches.length, 1);
  assert.deepEqual(
    doc.nodes.map((n) => n.index),
    [0, 1, 2, 3, 4, 5],
  );
});

test("literal search crosses visual wraps within logical text, preserves code and UTF-16 offsets", async () => {
  const doc = await readFlowDocument(
    new ReadingSession([
      node("paragraph", "A😀BC café long sentence"),
      node("code-block", "  a\n    b"),
    ]),
  );
  const match = doc.find("😀B").matches[0];
  assert.equal(match.startUtf16, 1);
  assert.equal(match.endUtf16, 4);
  assert.equal(doc.matchText(match), "😀B");
...
test("bounded search truthfully reports additional matches and rejects forged or cross-session hits", async () => {
  ...
  const result = doc.find("a", { maxMatches: 2 });
...
test("navigation/search/selection refuse source edits, reflows and disposed sessions", async () => {
  for (const action of ["edit", "reflow", "dispose"]) {
    const s = new ReadingSession(),
      doc = await readFlowDocument(s),
      match = doc.find("text").matches[0];
    if (action === "dispose") s.disposed = true;
    else s[action]();
    const expected =
      action === "edit"
        ? "STALE_REVISION"
    ...
    for (const call of [
      ...
      assert.throws(call, code(expected));
  }
});

test("reading cannot mix layouts or generations across asynchronous pages", async () => {
  const s = new ReadingSession(Array.from({ length: 300 }, () => node())),
    read = s.readingOrder.bind(s);
  s.readingOrder = async (options) => {
    const page = read(options);
    if (options.offset) s.reflow();
    return page;
  };
...
test("descendant/text/depth budgets apply to the complete tree, not just page roots", async () => {
  const tree = node("list", "summary", {
    children: [node("list-item", "one"), node("list-item", "two")],
  });
  await assert.rejects(
    readFlowDocument(new ReadingSession([tree]), { limits: { maxNodes: 2 } }),
    code("READING_LIMIT"),
  );
  await assert.rejects(
    readFlowDocument(new ReadingSession([tree]), { limits: { maxTextUnits: 3 } }),
    code("READING_LIMIT"),
  );
  let deep = node();
  for (let i = 0; i < 5; i++) deep = node("blockquote", "", { children: [deep] });
  await assert.rejects(
    readFlowDocument(new ReadingSession([deep]), { limits: { maxDepth: 2 } }),
    code("READING_LIMIT"),
  );
  for (const limits of [{ maxNodes: 0 }, { maxDepth: 65 }, { surprise: 1 }]) {
    await assert.rejects(
      readFlowDocument(new ReadingSession(), { limits }),
...
test("cycles, aliases, unknown roles, impossible structure and geometry are rejected", async () => {
  ...
  for (const roots of [
    ...
    [shared, shared],
    [node("script")],
    [node("heading", "x", { level: 7 })],
    [node("table-cell")],
    [node("table", "x", { children: [node()] })],
    [node("paragraph", "x", { children: [node()] })],
    [node("list", "x", { children: [node()] })],
    [node("paragraph", "\udc00")],
    [node("paragraph", "x", { bounds: { x: 0, y: NaN, width: 3, height: 4 } })],
...
test("abort stops waiting without forwarding cancellation or ignoring late rejections", async () => {
  ...
  const pending = readFlowDocument(s, { signal: controller.signal });
  controller.abort();
  await assert.rejects(pending, code("ABORTED"));
  assert(!s.disposed);
  gate.reject(new Error("late observed rejection"));
...
test("empty documents and >Number.MAX_SAFE_INTEGER revisions remain valid", async () => {
  const s = new ReadingSession([]);
  s.revision = "9007199254740993";
  const doc = await readFlowDocument(s);
  assert.equal(doc.token.revision, s.revision);
...
test("enclosing Markdown byte spans convert losslessly for textarea navigation", () => {
  ...
  assert.throws(
    () => sourceSpanToUtf16("\ud800", { startByte: 0, endByte: 0 }),
