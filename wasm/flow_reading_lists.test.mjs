...
const item = (listId = "1", itemIndex = 0, extra = {}) => ({
  listId,
  itemIndex,
  ordered: false,
  start: "1",
...
const node = (role = "list-item", text = "item", listPath = [item()], extra = {}) => ({
  role,
  text,
  listPath,
  ...
  return {
    ...
    readingOrder(options) {
      ...
      const { offset, limit } = options,
        end = Math.min(roots.length, offset + limit);
      const result = {
        ...this.token,
        schemaVersion: 1,
        offset,
        total: roots.length,
        nextOffset: end < roots.length ? end : null,
        nodes: roots.slice(offset, end),
      };
...
const reject = (roots, code = "INVALID_READING_DATA", options) =>
  assert.rejects(read(roots, options), { code });

test("nested ordered lists retain starts, item positions and task state without guessing from bounds", async () => {
  const outer = item("1", 0, { ordered: true, start: "7" }),
    child = item("2", 0, { task: true });
  const roots = [
    node("list-item", "first", [outer]),
    node("paragraph", "continuation", [outer]),
    node("list-item", "[x] child", [outer, child]),
    node("code-block", " code\n", [outer, child]),
    node("paragraph", "after child", [outer]),
...
test("lists and continuation ownership survive real-sized synchronous and async page boundaries", async () => {
  const roots = Array.from({ length: 520 }, (_, i) =>
    node(i === 0 ? "list-item" : "paragraph", `block ${i}`),
  );
  for (const asyncMode of [false, true]) {
    const s = session(roots, asyncMode),
      doc = await readFlowDocument(s);
    assert.deepEqual(
      s.calls.map((o) => o.offset),
      [0, 256, 512],
    );
...
test("separate nested lists keep distinct IDs even with identical source spans and list type", async () => {
  ...
  const doc = await read([
    node(),
    node("list-item", "a", [parent, a]),
    node("list-item", "b", [parent, b]),
  ]);
  assert.equal(
    doc.nodes[1].enclosingSourceSpan.startByte,
    doc.nodes[2].enclosingSourceSpan.startByte,
  );
...
test("empty/code-first items, images, headings and table rows retain one owner", async () => {
  ...
  const roots = [
    ...
    node("image", "alt"),
    node("heading", "heading", [item()], { level: 3 }),
    node("table-row", "cell", [item()], { children: [child] }),
  ];
  const doc = await read(roots);
...
test("malformed and sparse paths or list fields reject admission", async () => {
  for (const listPath of [
    ...
    [item("1", 0, { start: "03" })],
    [item("1", 0, { ordered: 1 })],
    [item("1", 0, { task: "checked" })],
    [item("1", 0, { extra: true })],
  ])
    await reject([node("list-item", "x", listPath)]);
  const missing = item();
  delete missing.task;
...
test("missing parents, missing item-leading blocks and duplicate markers are refused", async () => {
  ...
  await reject([
    node("list-item", "parent", [item()], { children: [node("paragraph", "child", [])] }),
  ]);
});
...
test("item positions cannot skip, move backward or change list metadata", async () => {
  ...
  await reject([
    node(),
    node("list-item", "next", [item("1", 1)]),
    node("list-item", "back", [item()]),
  ]);
...
test("closed lists, cross-parent IDs and cyclic ancestry cannot be resurrected", async () => {
  await reject([
    node(),
    node("paragraph", "outside", []),
    node("list-item", "reused", [item("1", 1)]),
  ]);
  await reject([node(), node("list-item", "cycle", [item(), item()])]);
  await reject([
    node(),
    node("list-item", "nested", [item(), item("2")]),
    node("list-item", "next parent", [item("1", 1)]),
    node("list-item", "foreign", [item("1", 1), item("2")]),
  ]);
  await reject([
    node(),
    node("list-item", "second list", [item("2")]),
    node("list-item", "first again", [item()]),
  ]);
});
...
test("cells and nested reading descendants cannot independently claim root-list membership", async () => {
  await reject([
    node(),
    node("table-row", "cell", [item()], { children: [node("table-cell", "cell")] }),
  ]);
...
test("ancestry budget counts repeated paths across all pages before retention", async () => {
  ...
  await reject([node(), node("list-item", "child", [item(), item("2")])], "READING_LIMIT", {
    limits: { maxDepth: 1 },
  });
});
...
test("large identities and ordinals remain exact decimal strings", async () => {
  ...
  await reject([
    node("list-item", "large", [large]),
    node("list-item", "overflow", [{ ...large, itemIndex: 1 }]),
  ]);
});

test("list metadata never weakens source or layout revision fencing", async () => {
  const s = session([node()]),
    doc = await readFlowDocument(s),
    match = doc.find("item").matches[0];
