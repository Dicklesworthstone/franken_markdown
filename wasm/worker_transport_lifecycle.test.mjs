...
const request = (rpc, controls = {}) =>
  track(rpc.request("render", 8, () => ({ args: [] }), controls));
...
test("failed preparation releases its reservation and unblocks a reentrant successor", async (t) => {
  ...
  assert.equal(rpc.pendingBytes, 8);
  assert.deepEqual(
    worker.sent.map((message) => message.method),
    ["render"],
  );
  worker.reply(0);
  await inner;
