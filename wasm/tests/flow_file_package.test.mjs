...
test("file saving and guide ship exactly once without changing package exports", () => {
  ...
  for (const key of [".", "./book", "./flow", "./book-worker"]) assert.ok(manifest.exports[key], key);
});
...
test("source entrypoint mounts saving before draft initialization and drops file authority on suspension", () => {
  ...
  assert.ok(source.indexOf("files = createFileControls") < source.indexOf("drafts = createDraftControls"));
  ...
  assert.match(source, /onReplace: \(\) => source\.dispatchEvent\(new Event\("fmd-document-replaced"\)\)/);
});
...
test("all source-entrypoint static dependencies are real shipped files, without renderer imports", () => {
  ...
  function visit(path) {
    ...
    visited.add(path);
    const source = read(path), packagePath = relative(resolve(root, "wasm"), resolve(root, path));
  ...
  for (const path of ["flow_file_session.mjs", "flow_file_controls.mjs", "flow_draft_store.mjs", "flow_document.mjs"]) {
