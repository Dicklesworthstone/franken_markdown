...
test("quoted phrases, Unicode, literal regex characters and term limits", () => {
  assert.deepEqual(plain(parseQuery(' RUST "memory safety" rust 中 .* ')), [
    "rust",
    "memory safety",
    "中",
    ".*",
...
test("reader search spans chapters, ranks headings, ANDs terms and groups section hits", async () => {
  const data = rows([
    chapter("Guide", [
      entry("Rust memory safety", "intro", "heading"),
      entry("More rust memory safety details", "intro"),
      entry("Rust alone", "other"),
    ]),
...
test("unknown schemas, traversal, remote destinations and duplicate pages are rejected", () => {
  ...
  assert.throws(
    () => rows([chapter("Bad", [{ kind: "unknown", text: "bad", anchor: "" }])]),
    /kind/,
  );
});
...
test("bounded best-results list reports the full match count, with stable ties", async () => {
  ...
  const result = await search(rows([chapter("Guide", entries)]), "needle", {
    yieldTask: async () => {},
  });
...
test("long searches yield and cancellation never produces a stale answer", async () => {
  ...
  const data = rows([
    chapter(
      "Guide",
      Array.from({ length: 500 }, (_, i) => entry("needle", `s${i}`)),
    ),
  ]);
...
function dom(data) {
  const elements = Object.fromEntries(
    ["query", "search-form", "status", "results", "previous", "next", "search-data"].map((id) => [
      id,
      new Element(),
    ]),
  );
  ...
  return {
    elements,
    location: { href: "file:///book/~fmd-search.html" },
...
test("production controller renders safe local links and inert text; no HTML parsing", async () => {
  ...
  const document = dom(
    index([chapter(title, [entry("</script> needle <svg/onload=bad>", 'a"<>')])]),
  );
